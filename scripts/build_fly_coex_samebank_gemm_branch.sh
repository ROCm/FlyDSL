...
# ---------------------------------------------------------------------------
BUILD_DIR="${FLY_BUILD_DIR:-${REPO_ROOT}/build-fly-coexec-samebank-gemm-branch}"
if [[ "${BUILD_DIR}" != /* ]]; then
...
if [ -z "${MLIR_PATH:-}" ]; then
  candidates=(
    "${BASE_DIR}/llvm-pipeline/mlir_install_coexec_samebank"
    "${BASE_DIR}/llvm-pipeline/build-coexec-samebank"
  )
...
if [ -z "${MLIR_PATH:-}" ]; then
  ...
  echo "Build LLVM/MLIR first:  bash scripts/build_llvm_coex_samebank.sh" >&2
...
echo "=============================================="
echo "FlyDSL Build (coexec-samebank)"
