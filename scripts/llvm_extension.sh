...
# The apply order lives in the LLVM_EXTENSIONS array in build_llvm.sh, so that is
# what is read here -- a second list would be a second source of truth.
#
...
pin() {
    python3 -c "import json; print(json.load(open('${REPO_ROOT}/thirdparty/llvm-build-info.json'))['upstream']['llvm_hash'])"
}
...
prepare_tree() {
    ...
    echo "Replaying extensions onto ${llvm_pin} ..."
