...
# ---------------------------------------------------------------------------
# Build directory (default: build-fly-internal/, overridable via FLY_BUILD_DIR)
# ---------------------------------------------------------------------------
BUILD_DIR="${FLY_BUILD_DIR:-${REPO_ROOT}/build-fly-internal}"
if [[ "${BUILD_DIR}" != /* ]]; then
...
if [ -z "${MLIR_PATH:-}" ]; then
  candidates=(
    "${BASE_DIR}/llvm-project-internal/mlir_install-${LLVM_SHORT}"
