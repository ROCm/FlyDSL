...
# A16W16 universe GEMM shapes (gfx950, NT layout):
# "dtype,M,N,K,block_m,block_n,block_k,stages,split_k,m_waves,n_waves,k_waves"
GEMM_A16W16_UNIVERSE_GFX950_SHAPES='
...
Supported ops:
  ...
  (gemm includes preshuffle, A16W16 universe, and FP8 8-wave row-scale GEMM)
USAGE
}
...
if [ "${RUN_PRESHUFFLE_GEMM}" -eq 1 ] && [ "${IS_CDNA}" = "true" ]; then
  ...
  case "${GPU_ARCH}" in
    gfx95*)
      for shape in $GEMM_A16W16_UNIVERSE_GFX950_SHAPES; do
        [ -z "$shape" ] && continue
        oldIFS=$IFS
        IFS=,
        # shellcheck disable=SC2086 # intentional word-splitting on IFS=,
        set -- $shape
        IFS=$oldIFS
        dtype=$1; M=$2; N=$3; K=$4; block_m=$5; block_n=$6; block_k=$7
        stages=$8; split_k=$9; m_waves=${10}; n_waves=${11}; k_waves=${12}
        shape_tag="${M}x${N}x${K}_nt_b${block_m}x${block_n}x${block_k}_s${stages}_sk${split_k}"
        log="${BENCH_LOG_DIR}/gemm_a16w16_universe_${shape_tag}_${dtype}.log"
        if python3 tests/kernels/test_gemm_a16w16_universe_gfx950.py \
          --dtype "$dtype" \
          --layout nt \
          --num_warmup 3 \
          --num_iters 50 \
          ...
          --m_waves "$m_waves" \
          --n_waves "$n_waves" \
          --k_waves "$k_waves" >"${log}" 2>&1; then
          SUCCESS_COUNT=$((SUCCESS_COUNT + 1))
        else
          _fail_or_skip "${log}" "gemm_a16w16_universe"
        fi
        row="$(_py_parse_and_emit gemm_a16w16_universe "${shape_tag}" "${dtype}" "${log}")"
        set -- $row
        _emit_row "$1" "$2" "$3" "$4" "$5"
      done
      ;;
