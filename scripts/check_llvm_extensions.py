...
# Exempt because they pre-date the switch requirement. Do not extend this list:
# a new extension without a switch should gain one, not an exemption.
GRANDFATHERED = {
...
def main() -> int:
    ...
    for ext in sorted(EXT_DIR.glob("*.patch")):
        if ext.name in GRANDFATHERED:
            print(f"  {ext.name}: skipped (grandfathered)")
            continue
