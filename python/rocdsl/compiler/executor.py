"""Executor for RocDSL via MLIR ExecutionEngine."""
...
def _default_mlir_lib_dir() -> Optional[Path]:
    ...
    env_dir = os.environ.get("ROCDSL_MLIR_LIB_DIR")
    if env_dir:
        p = Path(env_dir)
        if p.exists():
            return p
    ...
    # Next: try to locate a sibling llvm-project next to ROCDSL_BUILD_DIR (or its parents).
    # This matches common build layouts where dsl2/ and llvm-project/ live side-by-side.
    build_dir = os.environ.get("ROCDSL_BUILD_DIR")
    if build_dir:
...
def default_shared_libs(lib_dir: Optional[Path] = None) -> SharedLibs:
    ...
    if lib_dir is None:
        raise FileNotFoundError(
            "Could not locate MLIR runtime libraries. Set `ROCDSL_MLIR_LIB_DIR=/path/to/mlir/lib` "
...
class ExecutionEngineExecutor:
    """Execute host-side entrypoints compiled by RocDSL via MLIR ExecutionEngine."""
