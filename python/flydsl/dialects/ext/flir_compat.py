"""Compatibility layer for Flir dialect operations in the new flyc environment.

Provides the same Python API as flir.py but WITHOUT depending on the
``_mlir.dialects.flir`` C++ dialect (which only exists in the old build).

Layout algebra ops are forwarded to the ``fly`` dialect.
``swizzle_xor16`` is implemented with pure arithmetic.
``MlirModule``, ``kernel``, ``jit`` are NOT re-exported — use ``@flyc.kernel``
/ ``@flyc.jit`` from ``flydsl.compiler`` instead.
"""
...
def make_shape(*dims, loc: Optional[Location] = None, ip: Optional[InsertionPoint] = None) -> Value:
    ...
    with ip or InsertionPoint.current:
        return fly_ops.MakeShapeOp(result_type, operands, loc=loc).result
...
def make_stride(*strides, loc: Optional[Location] = None, ip: Optional[InsertionPoint] = None) -> Value:
    ...
    with ip or InsertionPoint.current:
        return fly_ops.MakeStrideOp(result_type, operands, loc=loc).result
...
def make_layout(shape, stride=None, loc: Optional[Location] = None, ip: Optional[InsertionPoint] = None) -> Value:
    ...
    with ip or InsertionPoint.current:
        return fly_ops.MakeLayoutOp(result_type, _unwrap_value(shape), _unwrap_value(stride), loc=loc).result
...
def make_coord(*coords: Value, loc: Optional[Location] = None, ip: Optional[InsertionPoint] = None) -> Value:
    ...
    with ip or InsertionPoint.current:
        ...
        return fly_ops.MakeCoordOp(result_type, operands, loc=loc).result
...
def crd2idx(coord: Value, layout: Value, loc: Optional[Location] = None, ip: Optional[InsertionPoint] = None) -> Value:
    ...
    with ip or InsertionPoint.current:
        op = fly_ops.Crd2IdxOp(result_type, _unwrap_value(coord), _unwrap_value(layout), loc=loc, ip=ip)
        return op.result
...
def idx2crd(idx: Value, layout: Value, loc: Optional[Location] = None, ip: Optional[InsertionPoint] = None) -> Value:
    ...
    with ip or InsertionPoint.current:
        op = fly_ops.Idx2CrdOp(result_type, _unwrap_value(idx), _unwrap_value(layout), loc=loc, ip=ip)
        return op.results[0]
...
def swizzle_xor16(row: Value, col: Value, k_blocks16: Value,
    ...
    Pure-arith implementation (no flir/fly dialect op needed).
    """
    loc = _get_location(loc)
    with ip or InsertionPoint.current:
        r = _unwrap_value(row)
...
def size(layout_or_tensor, mode: Optional[List[int]] = None, loc: Optional[Location] = None, ip: Optional[InsertionPoint] = None) -> Value:
    ...
    with ip or InsertionPoint.current:
        op = fly_ops.SizeOp(result_type, _unwrap_value(layout_or_tensor), loc=loc, ip=ip)
        return op.result
...
def cosize(layout: Value, loc: Optional[Location] = None, ip: Optional[InsertionPoint] = None) -> Value:
    ...
    with ip or InsertionPoint.current:
        op = fly_ops.CosizeOp(result_type, _unwrap_value(layout), loc=loc, ip=ip)
        return op.result
...
def rank(shape_or_layout: Value, loc: Optional[Location] = None, ip: Optional[InsertionPoint] = None) -> Value:
    ...
    with ip or InsertionPoint.current:
        raise NotImplementedError(
            "fly dialect has no RankOp; use type-level rank info or fx.rank() instead"
        )
...
def get_shape(layout: Value, loc: Optional[Location] = None, ip: Optional[InsertionPoint] = None) -> Value:
    ...
    with ip or InsertionPoint.current:
        return fly_ops.GetShapeOp(result_type, _unwrap_value(layout), loc=loc).result
...
def get_stride(layout: Value, loc: Optional[Location] = None, ip: Optional[InsertionPoint] = None) -> Value:
    ...
    with ip or InsertionPoint.current:
        return fly_ops.GetStrideOp(result_type, _unwrap_value(layout), loc=loc).result
...
def get(input: Value, index: Value, loc: Optional[Location] = None, ip: Optional[InsertionPoint] = None) -> Value:
    ...
    with ip or InsertionPoint.current:
        return fly_ops.GetOp(result_type, _unwrap_value(input), _unwrap_value(index), loc=loc, ip=ip).result
...
def composition(layout_a: Value, layout_b: Value, loc: Optional[Location] = None, ip: Optional[InsertionPoint] = None) -> Value:
    ...
    with ip or InsertionPoint.current:
        return fly_ops.CompositionOp(result_type, _unwrap_value(layout_a), _unwrap_value(layout_b), loc=loc).result
...
def complement(tiler: Value, target_size: Value, loc: Optional[Location] = None, ip: Optional[InsertionPoint] = None) -> Value:
    ...
    with ip or InsertionPoint.current:
        return fly_ops.ComplementOp(result_type, _unwrap_value(tiler), _unwrap_value(target_size), loc=loc).result
...
def logical_product(block: Value, tiler: Value, loc: Optional[Location] = None, ip: Optional[InsertionPoint] = None) -> Value:
    ...
    with ip or InsertionPoint.current:
        return fly_ops.LogicalProductOp(result_type, _unwrap_value(block), _unwrap_value(tiler), loc=loc).result
...
def zipped_product(block: Value, tiler: Value, loc: Optional[Location] = None, ip: Optional[InsertionPoint] = None) -> Value:
    ...
    try:
        ...
        result_type = LayoutType.get(-1)
    with ip or InsertionPoint.current:
        return fly_ops.ZippedProductOp(result_type, _unwrap_value(block), _unwrap_value(tiler), loc=loc).result
...
def tiled_product(block: Value, tiler: Value, loc: Optional[Location] = None, ip: Optional[InsertionPoint] = None) -> Value:
    ...
    try:
        ...
        result_type = LayoutType.get(-1)
    with ip or InsertionPoint.current:
        return fly_ops.TiledProductOp(result_type, _unwrap_value(block), _unwrap_value(tiler), loc=loc).result
...
def flat_product(block: Value, tiler: Value, loc: Optional[Location] = None, ip: Optional[InsertionPoint] = None) -> Value:
    ...
    try:
        ...
        result_type = LayoutType.get(-1)
    with ip or InsertionPoint.current:
        return fly_ops.FlatProductOp(result_type, _unwrap_value(block), _unwrap_value(tiler), loc=loc).result
...
def raked_product(block: Value, tiler: Value, loc: Optional[Location] = None, ip: Optional[InsertionPoint] = None) -> Value:
    ...
    try:
        ...
        result_type = LayoutType.get(-1)
    with ip or InsertionPoint.current:
        return fly_ops.RakedProductOp(result_type, _unwrap_value(block), _unwrap_value(tiler), loc=loc).result
...
def blocked_product(block: Value, tiler: Value, loc: Optional[Location] = None, ip: Optional[InsertionPoint] = None) -> Value:
    ...
    try:
        ...
        result_type = LayoutType.get(-1)
    with ip or InsertionPoint.current:
        return fly_ops.BlockedProductOp(result_type, _unwrap_value(block), _unwrap_value(tiler), loc=loc).result
...
def logical_divide(layout: Value, tiler: Value, loc: Optional[Location] = None, ip: Optional[InsertionPoint] = None) -> Value:
    ...
    with ip or InsertionPoint.current:
        return fly_ops.LogicalDivideOp(result_type, _unwrap_value(layout), _unwrap_value(tiler), loc=loc).result
...
def zipped_divide(layout_or_tensor, tiler_or_shape, loc: Optional[Location] = None, ip: Optional[InsertionPoint] = None):
    ...
    try:
        ...
        result_type = LayoutType.get(-1)
    with ip or InsertionPoint.current:
        return fly_ops.ZippedDivideOp(result_type, _unwrap_value(layout_or_tensor), _unwrap_value(tiler_or_shape), loc=loc).result
...
def tiled_divide(layout: Value, tiler: Value, loc: Optional[Location] = None, ip: Optional[InsertionPoint] = None) -> Value:
    ...
    try:
        ...
        result_type = LayoutType.get(-1)
    with ip or InsertionPoint.current:
        return fly_ops.TiledDivideOp(result_type, _unwrap_value(layout), _unwrap_value(tiler), loc=loc).result
...
def flat_divide(layout: Value, tiler: Value, loc: Optional[Location] = None, ip: Optional[InsertionPoint] = None) -> Value:
    ...
    try:
        ...
        result_type = LayoutType.get(-1)
    with ip or InsertionPoint.current:
        return fly_ops.FlatDivideOp(result_type, _unwrap_value(layout), _unwrap_value(tiler), loc=loc).result
...
def local_partition(layout: Value, tile: Value, index: Value, loc: Optional[Location] = None, ip: Optional[InsertionPoint] = None) -> Value:
    ...
    with ip or InsertionPoint.current:
        return fly_ops.LocalPartitionOp(result_type, _unwrap_value(layout), _unwrap_value(tile), _unwrap_value(index), loc=loc).result
...
def local_tile(layout: Value, tiler: Value, coord: Value, loc: Optional[Location] = None, ip: Optional[InsertionPoint] = None) -> Value:
    ...
    with ip or InsertionPoint.current:
        return fly_ops.LocalTileOp(result_type, _unwrap_value(layout), _unwrap_value(tiler), _unwrap_value(coord), loc=loc).result
...
# ─── Override layout algebra to use fly dialect (not flir) ─────────────────
# These replace the !flir.shape/stride/layout type-parsing implementations
# above with thin forwarders to flydsl.expr (fx), which emit fly.* ops.
# Lazy import to avoid circular import / double registration issues.
