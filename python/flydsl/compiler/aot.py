...
``compile_aot`` traces and lowers a launcher for concrete example arguments
...
from .backends import get_backend, get_backend_class
from .jit_argument import MemRefJitArg, PointerJitArg
from .jit_function import JitFunction, _BoundJitMethod, _create_mlir_context
from .kernel_function import CompilationContext
from .protocol import c_abi_spec

__all__ = ["AOTCompiledFunction", "AbiSlot", "CExportResult", "ReturnAbi", "compile_aot"]
...
def _leaf_semantic_slots(name: str, jit_arg) -> List[Dict[str, Any]]:
    ...
    if isinstance(jit_arg, Numeric):
        ...
        raise NotImplementedError(f"compile_aot: argument {name!r} has unsupported scalar type {cls.__name__}")
    raise NotImplementedError(
        f"compile_aot: argument {name!r} has unsupported type {type(jit_arg).__name__}; "
...
def _host_size_align(ty: ir.Type) -> Tuple[int, int]:
    ...
    if width is not None:
        ...
        if size is None:
            raise NotImplementedError(f"compile_aot: unsupported host integer type {ty}")
        return size, size
    ...
    raise NotImplementedError(f"compile_aot: unsupported host argument type {ty}")
...
def _struct_leaf_widths(ty: ir.Type) -> List[int]:
    ...
    width = _integer_width(ty)
    if width is None:
        raise NotImplementedError(f"compile_aot: unsupported tensor layout element type {ty}")
    return [width]
...
def _group_width(widths: List[int], what: str, name: str) -> Optional[int]:
    if not widths:
        return None
    if len(set(widths)) != 1:
        raise NotImplementedError(f"compile_aot: tensor {name!r} has mixed-width dynamic {what} values {widths}")
    return widths[0]
...
def _build_abi(sig, param_names, jit_args, has_user_stream, entry_arg_types: List[ir.Type]) -> Tuple[AbiSlot, ...]:
    ...
    if sum(isinstance(a, Stream) for a in user_args) > 1:
        raise NotImplementedError("compile_aot: a launcher may declare at most one stream argument")
    ...
    for name, jit_arg in zip(param_names, user_args):
        ...
        if len(ctypes_seq) != len(semantic):
            raise NotImplementedError(
                f"compile_aot: argument {name!r} ({type(jit_arg).__name__}) packs {len(ctypes_seq)} C ABI slots, "
    ...
    if len(pending) != len(entry_arg_types):
        raise RuntimeError(
            f"compile_aot: launcher packs {len(pending)} C ABI slots but the lowered entry "
    ...
    for (arg_index, arg_name, semantic, ctype), ty in zip(pending, entry_arg_types):
        ...
        if not _lowered_type_matches(semantic, ty):
            raise RuntimeError(f"compile_aot: {where} ({semantic['ctype']}) does not match lowered type {ty}")
        size, alignment = _host_size_align(ty)
        if ctypes.sizeof(ctype) != size:
            raise RuntimeError(
                f"compile_aot: {where} packs {ctypes.sizeof(ctype)} bytes but the lowered type {ty} has {size}"
            )
        extra = {}
        if semantic["kind"] == "tensor_layout":
            ...
            if len(widths) != n_shape + len(semantic["stride_dims"]):
                raise RuntimeError(f"compile_aot: tensor {arg_name!r} layout {ty} does not match its dynamic dims")
...
def compile_aot(launcher, *args, **kwargs) -> "AOTCompiledFunction":
    """Compile a ``@flyc.jit`` launcher for the given example arguments.

    ``launcher`` is a ``@flyc.jit`` function or method, bound (``obj.launch``)
    or unbound with the instance passed first. Arguments follow the same
    binding, specialization and compile-hint rules as a regular call, but the
    launcher is only traced and lowered: no GPU module is loaded and no kernel
    is launched. Constexpr and type arguments are baked into the result; the
    remaining arguments form the runtime C ABI. The result keeps no reference
    to the example arguments.
    """
    if isinstance(launcher, _BoundJitMethod):
        launcher, args = launcher.jit_function, (launcher.instance, *args)
    if not isinstance(launcher, JitFunction):
        raise TypeError(f"compile_aot() expects a @flyc.jit function, got {type(launcher).__name__}")

    bound_self, args = launcher._split_self(args)
    launcher._snapshot_or_check_globals(type(bound_self) if bound_self is not None else None)

    sig = launcher._sig
    bound = sig.bind(*args, **kwargs)
    bound.apply_defaults()
    hints = launcher._effective_compile_hints()
    backend = get_backend()
    entry = launcher.func.__name__

    with _create_mlir_context() as ctx, CompilationContext.compile_hints(hints) if hints else nullcontext():
        traced = launcher._trace_and_compile(ctx, sig, bound, bound_self)
        if traced.post_load_processors:
            raise NotImplementedError(
                "compile_aot: launchers with post-load processors (Python callbacks run after GPU "
                "module load) cannot be exported"
            )
        return _make_aot_compiled(
...
class AOTCompiledFunction:
    """A launcher traced and lowered by :func:`compile_aot`, ready for export.
