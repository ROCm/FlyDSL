...
class CompilationContext:
    ...
    def __init__(self):
        ...
        self.ktrace_buf_arg = None  # host-side SSA value for the trace buffer
...
class KernelLauncher:
    ...
    def launch(
        ...
        *,
        ...
        # The ktrace buffer is a kernel OPERAND, not an async dependency: the stream travels
        # through async_dependencies, and putting a pointer there is a type error.
        _ktrace_ctx = CompilationContext.get_current()
        if _ktrace_ctx is not None and _ktrace_ctx.ktrace_buf_arg is not None:
            kernel_operands.append(_ktrace_ctx.ktrace_buf_arg)
...
class KernelFunction:
    ...
    def _emit_kernel(
        ...
        # ktrace: a trailing implicit pointer parameter carrying the trace buffer. Declared
        ...
        ktrace_traced = ctx.ktrace_buf_arg is not None
        if ktrace_traced:
            # Match the host-side PointerJitArg's type exactly, or gpu.launch_func
            # rejects the operand.
            kernel_arg_types.append(ctx.ktrace_buf_arg.type)
