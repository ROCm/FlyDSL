...
class RocmBackend(BaseBackend):
    ...
    def lower_compile_hints(self, module, *, compile_hints: dict) -> None:
        """Materialize scalar WPE as an exact, module-wide AMDGPU option.

        ``N,N`` matches Triton semantics; ``0,0`` explicitly requests the
        compiler default.  Device helpers are not entry kernels and are skipped.
        """
        ...
        with module.context:
            for func_op in _iter_gpu_kernel_funcs(module):
                # The compile option is an overlay. Remove the native min-only
                # form so exactly one LLVM attribute survives lowering.
