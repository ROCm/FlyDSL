...
class MlirCompiler:
    ...
    def compile(cls, module: ir.Module, *, arch: str = "", func_name: str = "") -> ir.Module:
        ...
        # Apply scoped LLVM cl::opt overrides (e.g. enable-post-misched) around
        # the pass-pipeline execution so they take effect during code-gen.
...
def _compile_impl(func, *args) -> CompiledFunction:
    ...
    # compile_hints are already set on jf by CompileCallable.
    jf(*args)
...
class CompileCallable:
    """Subscriptable compile callable, inspired by CuTeDSL's CompileCallable.

    Usage::

        # Pre-compile without hints:
        compiled_fn = flyc.compile(launch, *args)

        # Pre-compile with hints:
        compiled_fn = flyc.compile[{"fast_fp_math": True}](launch, *args)

        # Set hints on a @flyc.jit function (no args → deferred compilation):
        hinted_fn = flyc.compile[{"llvm_options": {...}}](launch)
        hinted_fn(q, k, v, o, B, S)  # compiles on first call with hints
    """
