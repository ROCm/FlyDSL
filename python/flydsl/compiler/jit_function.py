...
def _dump_llvmir(*, dump_dir: Path, asm: str, stage_name: str = "16_llvm_ir"):
    """Dump LLVM IR (.ll) by extracting gpu.module content from MLIR.

    Extracts the ``llvm.func`` kernel from inside ``gpu.module``, wraps it
    in a standalone LLVM-dialect module, and translates to LLVM IR via
