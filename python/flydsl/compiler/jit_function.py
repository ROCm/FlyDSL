...
def _build_call_state(sig, args_tuple, func_exe, *, ktrace_enabled=None):
    ...
    *ktrace_enabled* must carry the decision the COMPILE path made, which every caller
    reads off the artifact (``_ktrace_traced``).  This function runs outside the
    ``compile_hints`` scope, so re-deriving it here would miss a ``ktrace`` hint and pack
    one slot fewer than the kernel declares -- the kernel would then read a trace-buffer
    pointer from past the end of the packed array.  None means "no compile happened on
    this path" and falls back to the ambient decision.
    """
    ...
    if _ktrace.tracing_enabled() if ktrace_enabled is None else ktrace_enabled:
        ...
        if not _env.compile.compile_only:
            ...
            slot_specs.append((-1, ctypes.c_void_p, None))
            # The address above is frozen into the ABI storage, so checking it only
            # here would miss every later launch: a cached CallState and
            # CompiledFunction.__call__ both reuse it without going near
            # ensure_buffer() again. Re-check per dispatch instead, which is cheap --
            # one attribute compare unless the device actually changed.
            predispatch = _ktrace_emit.require_dispatchable_buffer

    return CallState(slot_specs, func_exe, presets, predispatch)
...
class JitFunction:
    ...
    def __call__(self, *args, **kwargs):
        ...
        if cached_func is not None:
            ...
            # with. Restore them or the host decodes every record against {}. Before the
            # COMPILE_ONLY return, not after: that path still loads the artifact, and
            # leaving its ids unregistered lets the next compile hand the same id to a
            # different name.
            _ktrace.restore_names(cached_func)
            if env.compile.compile_only:
                return None
            _ktrace.require_matching_capacity(cached_func)
        ...
        with _compile_lock_ctx as (_lock_result, _cache_writer):
            if _lock_result is not None and not getattr(_lock_result, "_link_libs", None):
                ...
                _ktrace.restore_names(compiled_func)
                # Same artifact, same risk as the in-process cache hit: its store bound
                # was fixed by whichever process built it, not by the buffer we hold.
                _ktrace.require_matching_capacity(compiled_func)
                ...
                with _create_mlir_context() as ctx, _hints_ctx:
                    ...
                    _attach_ktrace_config(module, has_ktrace_buf)
                    ...
                    _capture_ktrace_names(compiled_func, has_ktrace_buf, compiled_module)
        ...
        state = _build_call_state(
            ...
            ktrace_enabled=getattr(compiled_func, "_ktrace_traced", None),
        )
        self._call_state_cache[cache_key] = state
        return state(args_tuple)
...
# The ktrace helpers that remain here do compiler work that happens to serve ktrace:
# they build MLIR attributes, read what the lowering pass published on the module, or
# pack the trace buffer into the launch ABI. The ones that only touched ktrace's own
# state -- the record cache, the name table, the capacity bound -- now live in
# expr/experimental/ktrace.py and are called from this file.


def _attach_ktrace_config(module, traced: bool) -> None:
    """Record the ktrace settings this compilation is using, on the module itself.

    `convert-fly-ktrace-to-rocdl` bakes the store-bounds capacity into the kernel, so it
    has to use the same value the frontend did. (The per-wave slot count is a fixed
    constant -- see ktrace.EVENTS_PER_WAVE -- and needs no channel.)

    Passing it through the IR rather than having the pass re-read the environment keeps
    one source of truth per compilation. A pass option would serve the pass equally well,
    but an attribute is also what the host reads back later, so the binary ends up
    self-describing: a cache hit does not have to re-derive what it was built with.
    """
    if not traced:
        return
    from .._mlir import ir
    ...
    # The bound the kernel checks against must describe the buffer it will actually be
    # handed, not what the environment currently asks for. The buffer is allocated once
    # per process (ensure_buffer) and never resized, so a later compile that read a
    # larger FLYDSL_KTRACE_BUFFER_BYTES would bake a bound past the end of the existing
    # allocation and store out of bounds. _ensure_ktrace_buffer_arg has already
    # allocated it by the time this runs, except under COMPILE_ONLY where nothing
    # launches and the environment value is the only thing available.
    buffer = _ktrace_emit.current_buffer()
    capacity = buffer.capacity_slots if buffer is not None else _ktrace_emit._buffer_capacity_slots()
    attrs["fly_ktrace.capacity_slots"] = ir.IntegerAttr.get(i32, capacity)
    # The arch the pass gates on. It normally reads gpu.module's target, but an
    # extern-linked kernel has its targets stripped before compile (see the
    # extern_linked branch in _build_compiled) and rocdl-attach-target only puts
    # them back afterwards. The chip is known either way, so publishing it keeps
    # tracing usable on those kernels instead of failing the compile.
    from .backends import get_backend

    attrs["fly_ktrace.chip"] = ir.StringAttr.get(get_backend().target.arch)
    # Bindings this process has already made. Records from several kernels share one
    # buffer and accumulate until a read, so an id has to identify a name across the
    # whole process: a new module numbers above these, and a name that appears again
    # keeps the id it already has rather than being renumbered out from under the
    # records that were written with it.
    known = _ktrace_emit.name_table()
    if known:
        attrs["fly_ktrace.known_event_names"] = ir.DictAttr.get(
            {name: ir.IntegerAttr.get(i32, ident) for name, ident in known.items()}
        )
    # Which workgroups record. The pass gates the per-wave slot claim on this, not
    # just the stores: with a filter over a large grid, every wave claiming its
    # range runs the cursor past the buffer even though only a handful record.
    blocks = (env.ktrace.blocks or "").strip()
    if blocks:
        attrs["fly_ktrace.blocks"] = ir.StringAttr.get(blocks)


def _ktrace_names_from_module(module) -> dict | None:
    """The name table `convert-fly-ktrace-to-rocdl` published, or None.

    When the expansion runs as a pass, the pass owns id assignment, so it writes the
    mapping onto the module as ``fly_ktrace.event_names``. Reading it here is what
    lets the host decode a record back to a phase name.

    Returns None when the attribute is absent, which now means the pass did not run:
    an untraced module, or a chip the pass is gated off for. A traced gfx942/gfx950
    compile always carries it.
    """
    try:
        attrs = module.operation.attributes
        if "fly_ktrace.event_names" not in attrs:
            return None
        return {
            str(named.name): int(str(named.attr).split(":")[0].strip()) for named in attrs["fly_ktrace.event_names"]
        }
    except Exception as exc:  # noqa: BLE001 - a malformed table must not fail the compile
        log().warning(f"ktrace: could not read the event-name table from the module: {exc}")
        return None


def _capture_ktrace_names(artifact, traced: bool, module=None) -> None:
    ...
    The conversion pass assigns the ids, so its ``fly_ktrace.event_names`` attribute is
    the authority and is also published into the emitter's table, which is what
    ``collect()`` and ``summary()`` decode against. The pass omits the attribute when it
    found no events -- a kernel traced by hint but carrying no fx.ktrace calls -- so the
    fallback below covers that case and leaves the artifact with an empty table rather
    than no table.
    ...
    *traced* comes from the compile path rather than being re-derived: this runs after the
    ...
    # What the binary was compiled to bound its stores by, so a later cache hit can
    # refuse a buffer that does not match instead of storing past the allocation.
    buffer = _ktrace_emit.current_buffer()
    artifact._ktrace_capacity = buffer.capacity_slots if buffer is not None else _ktrace_emit._buffer_capacity_slots()

    if module is not None:
        from_pass = _ktrace_names_from_module(module)
        if from_pass:
            artifact._ktrace_names = from_pass
            # collect() and summary() decode against the emitter's table, not the
            # artifact's, so publishing there too is what keeps a fresh traced
            # compile readable: the pass assigned these ids and the frontend never
            # saw them. Both allocate on first encounter in program order, so the
            # two agree and merge_names' collision check stays quiet.
            _ktrace_emit.merge_names(from_pass)
            return
...
class CompiledFunction:
    ...
    def __call__(self, *args):
        # Same reason as the jit path (see JitFunction.__call__): a second launch must
        # not let collect() hand back the first launch's records. Without this the stale
        # trace is well-formed and plausible, so nothing reveals the mistake. The check
        # is a module-global read, which the hot path can afford.
        from ..expr.experimental import ktrace as _ktrace
