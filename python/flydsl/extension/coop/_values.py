...
Numeric tiles use Vector; structured tiles use native Struct sequences or tuples. Struct values
...
def _is_struct_items(value):
    """Recognize an opt-in native Struct sequence, never just an indexable view."""
    cls = type(value)
    return (
        is_struct_type(cls)
        and callable(getattr(cls, "__len__", None))
        and callable(getattr(cls, "__getitem__", None))
        and callable(getattr(cls, "from_elements", None))
        and isinstance(getattr(value, "dtype", None), type)
    )
...
def _from_items(items, dtype=None, *, like=None):
    """Retain a Struct sequence when item dtype and count match its template."""
    items = _as_items(items)
    dtype = _item_dtype(items[0]) if dtype is None else dtype
    if _is_struct_items(like) and like.dtype is dtype and len(like) == len(items):
        result = type(like).from_elements(tuple(_record_cast(item, dtype) for item in items))
        if type(result) is not type(like) or len(_as_items(result)) != len(items):
            raise TypeError("from_elements must preserve the native Struct sequence type and length")
        return result
    if issubclass(dtype, Numeric):
