...
def warp_merge_sort(
    ...
    *,
    ...
    Args:
        keys: Nonempty outer tuple/list of complete keys in blocked order.
...
class WarpMergeSort(WarpPrimitive):
    ...
    Specialize with ``WarpMergeSort[dtype, width, items_per_thread]``. The parameters below
    are compile-time positional arguments to ``WarpMergeSort[...]``, in bracket order.
