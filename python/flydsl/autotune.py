...
def autotune(
    ...
    Builder mode (structural knobs are baked at module-build time, as every
    current FlyDSL kernel is written). Decorate a thin dispatcher and hand it a
    ``build_fn`` that rebuilds the module per config plus a ``default``
    heuristic::

        def _build(config, x, out, m):
            return build_rmsnorm_module(x.shape[1], "bf16",
                                        BLOCK_THREADS=config.kwargs["BLOCK_THREADS"])

        @autotune(configs=get_all_configs(), key=['x'],
                  build_fn=_build, default=get_default)
        def rmsnorm(x, out, m, stream): ...
