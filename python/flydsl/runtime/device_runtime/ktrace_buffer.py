...
def _runtime():
    ...
    Loaded lazily and cached, so importing flydsl never needs a GPU runtime. The
    mgpuTraceBuffer* entry points return hipError_t rather than reporting and
    continuing, which is what lets _check raise instead of handing a kernel a null
    pointer.

    Unguarded on purpose, unlike the buffer itself: CDLL on an already-loaded library
    returns a handle to the same one, and the argtypes assignments below are idempotent,
    so two threads racing here converge on the same state rather than leaking a second
    allocation.
    """
    global _RT
    if _RT is None:
        ...
        # Allocation reuses the generic wrapper; only the two calls that must be
        # synchronous are ktrace-specific.
...
class TraceBuffer:
    ...
    def claimed_slots(self) -> int:
        ...
        """
        # mgpuTraceBufferRead synchronises every stream before it copies, which is what
        # this read needs: FlyDSL kernels routinely launch on a user-supplied fx.Stream,
        # and that work is not ordered against a null-stream copy, so the cursor would be
        # read mid-flight.
        self._require_own_device()
...
def allocate(capacity_bytes: int | None = None) -> TraceBuffer:
    ...
    # mgpuMemAlloc reports a failure to stderr and returns null rather than an error
    # code, so the null is the signal to raise on -- a null here would otherwise reach
    # the kernel as its trace buffer and fault on the first store.
    rt = _runtime()
