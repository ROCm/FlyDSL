...
"""Turn iket records into a Chrome Trace JSON timeline.
...
def summarize(records: list[dict], names: dict[str, int]) -> dict:
    ...
    for group in waves.values():
        ...
        for rec in group:
            kind = rec["kind"]
            if kind == _iket.KIND_RANGE_PUSH:
                stack.append(rec)
            elif kind == _iket.KIND_RANGE_POP and stack:
                start = stack.pop()
...
def write_trace(records: list[dict], names: dict[str, int], *, kernel: str = "kernel", out_dir=None) -> str:
    """Write ``<kernel>_iket.trace.json`` plus the raw records; returns the trace path.
    ...
    path = _unique(os.path.join(out_dir, f"{kernel}_iket.trace.json"))
    with open(path, "w") as fh:
        json.dump(build_chrome_trace(records, names, kernel=kernel), fh)

    raw = _unique(os.path.join(out_dir, f"{kernel}_iket.json"))
