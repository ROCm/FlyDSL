...
* Records arrive in *cursor* order, which is global atomic order, not per-wave order.
  Group by the wave slot (see :func:`_wave_key`) and sort each group by timestamp before
...
def _wave_key(rec: dict) -> tuple[int, ...]:
    """Identify the wave slot a record came from.

    Keyed on the decoded identity fields, not the raw HW_ID: above bit 14 that register
    also carries TG_ID, VM_ID and QUEUE_ID, and TG_ID differs per workgroup. Keying on
    the raw value therefore splits one wave slot into several groups whose labels are
    byte-identical -- the timeline shows repeated rows and ``summarize`` overcounts
    waves, with nothing to indicate it. These are exactly the fields _wave_label prints.
    """
    return (rec["xcc_id"], rec["se"], rec["sh"], rec["cu"], rec["simd"], rec["wave"])
...
def _wave_events(group: list[dict], by_id: dict[int, str], tid: int, pid: int, t0: int) -> list[dict]:
    out = []
    # Name of the START each open slot belongs to. Chrome Trace pairs async b/e by
    # (id, name), so an end that names itself closes nothing when the token was
    # rebound across a branch or loop: RangeToken carries the name of whichever
    # exemplar the loop machinery reconstructed it from, not of the range actually
    # open. summarize() already reads the name off the START for this reason.
    ...
    for rec in group:
        ...
        if kind == _ktrace.KIND_MARK:
            ...
            if rec["start_slot"] != _ktrace.UNPAIRED_SLOT:
                # The START's name, not this record's: they differ when the token was
                # rebound, and Perfetto then leaves the range open.
                end_name = open_names.pop(rec["start_slot"], name)
                out.append({**base, "ph": "e", "id": rec["start_slot"], "cat": "range", "name": end_name, "args": args})
    return out
