...
__all__ = [
    ...
    # Convenience wrappers (override star-imported raw ops)
    "ballot_i64",
]
...
def ballot_i64(cond, *, loc=None):
    ...
    Args:
        cond: Condition value (i1, or wider integer truncated to i1).

    Returns:
        i64 bitmask where bit *lane* is set iff ``cond[lane]`` is true.
    """
...
def readlane(val, lane, *, loc=None):
    ...
    Args:
        val:  The value whose lane register content is read.
        lane: Source lane index (i32 or Python int).

    Returns:
        i32 value holding lane-*lane*'s value in all lanes.
    """
