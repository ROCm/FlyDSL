...
def TDMGather(index_size=32, pad_interval=0, pad_amount=0, cache_modifier=0):
    """Create a gfx1250 TDM gather Global<->LDS copy atom *type*.

    Loads/stores rows selected by explicit row indices (carried as atom state)
    rather than iterating a contiguous outer dim. ``index_size`` is the row-index
    width in bits: 32 (up to 8 indices) or 16 (up to 16 indices, 2 per lane).
    ``pad_interval`` / ``pad_amount`` (elements) add LDS row padding.

    The descriptor (base pointer, OOB extents, row stride, packed row indices,
    valid-index count, and K-advance byte offset) is runtime atom state set via
    ``fx.atom.set_value``. :func:`make_tdm_gather_atom` populates it from a tensor
    and an index list in one call.
    """
...
def _pack_gather_indices(row_indices, index_size):
    """Pack ``row_indices`` into a ``vector<8xi32>`` matching the TDM gather ABI.

    32-bit mode: up to 8 indices, one per lane. 16-bit mode: up to 16 indices,
    packed 2 per lane (lo | hi << 16). Missing slots are zero.
    """
    from .. import vector
    from ..typing import T as fxT

    def _i32(v):
        return v if isinstance(v, Int32) else Int32(v)

    n = len(row_indices)
    idx = [_i32(v) for v in row_indices]
    zero = Int32(0)
    if index_size == 32:
        if n > 8:
            raise ValueError(f"32-bit gather takes at most 8 indices, got {n}")
        words = [idx[i] if i < n else zero for i in range(8)]
    elif index_size == 16:
        if n > 16:
            raise ValueError(f"16-bit gather takes at most 16 indices, got {n}")
        words = []
        for w in range(8):
            lo = idx[2 * w] if 2 * w < n else zero
            hi = idx[2 * w + 1] if 2 * w + 1 < n else zero
            words.append((lo & 0xFFFF) | ((hi & 0xFFFF) << 16))
    else:
        raise ValueError(f"index_size must be 16 or 32, got {index_size}")
    return vector.from_elements(fxT.vec(8, fxT.i32), words)
...
def make_tdm_gather_atom(
    ...
    *,
    ...
    """Build a gfx1250 TDM gather copy atom carrying ``tensor`` + ``row_indices``.

    ``row_indices`` is a Python list of row indices (Python ints or fx integers).
    Issue the copy with ``fx.copy_atom_call(atom, global_tile, lds)`` where the
    global operand supplies the tile shape (outer = rows to gather, inner = per-row
    width) and the copy direction.

    Optional OOB / layout overrides (all runtime state):
      ``num_rows`` sets ``outer_extent`` (tensor_dim1, the matrix row count used for
      HW OOB on the indices; an index >= this is zero-filled on load). ``None``
      leaves it unclamped (INT32_MAX). ``row_width_bound`` sets ``inner_extent``
      (tensor_dim0, the matrix row width for column OOB); ``None`` leaves it
      unclamped. ``row_stride`` sets the row stride in elements; ``None`` falls back
      to the tile memref's static layout stride. ``gather_count`` sets the number of
      valid indices consumed; ``None`` uses ``len(row_indices)``.
    """
    from ..primitive import atom_set_value, make_copy_atom
    ...
    n = len(row_indices)
    packed = _pack_gather_indices(row_indices, index_size)

    outer_extent = Int32(NO_CLAMP) if num_rows is None else _i32(num_rows)
    inner_extent = Int32(NO_CLAMP) if row_width_bound is None else _i32(row_width_bound)
    stride = Int32(STRIDE_UNSET) if row_stride is None else _i32(row_stride)
    count = _i32(n if gather_count is None else gather_count)

    copy_op = CopyOpGFX1250TDMGatherType.get(
        index_size,
        pad_interval,
        pad_amount,
        cache_modifier,
    ...
    atom = make_copy_atom(copy_op, tensor.element_type)
    atom = atom_set_value(atom, "base", get_iter(tensor))
    atom = atom_set_value(atom, "outer_extent", outer_extent)
    atom = atom_set_value(atom, "inner_extent", inner_extent)
    atom = atom_set_value(atom, "outer_stride", stride)
    atom = atom_set_value(atom, "row_indices", packed)
    atom = atom_set_value(atom, "gather_count", count)
    if workgroup_mask != 0:
        atom = atom_set_value(atom, "workgroup_mask", _i32(workgroup_mask))
    return atom
