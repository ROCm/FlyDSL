...
```python
stage = fx.ktrace.sentinel_token("k_tile")
for i in fx.range(0, k_tiles, 1):
    ...
    fx.ktrace.range_end(stage)
    stage = fx.ktrace.range_start("k_tile")
    ...
fx.ktrace.range_end(stage)
```
...
An annotation is an MLIR op, not inlined code. `fx.ktrace.mark("x")` emits a single
...
| Piece | Where |
|---|---|
| Public API | [`python/flydsl/expr/ktrace.py`](../python/flydsl/expr/ktrace.py) |
...
| Host-side buffer and name table | [`ktrace_runtime.py`](../python/flydsl/utils/ktrace_runtime.py), [`ktrace_emit.py`](../python/flydsl/expr/rocdl/ktrace_emit.py) |
