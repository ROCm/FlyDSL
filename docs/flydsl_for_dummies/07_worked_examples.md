...
GEMM — exercising every concept in Chapters 3–6. Run any of them directly:
...
- `make_tiled_copy_tv(atom, thr_layout, val_layout)` builds a TiledCopy (§4.2)
...
`flat_divide(A, TileMN)` tiles the whole tensor (§3.3); indexing
...
`partition_S/D` (§4.3) hand each thread its float4 of A, B, C, and its four
coordinates (`thr_cC`).
...
`make_fragment_like` allocates registers shaped like a partition (§4.4). The
...
The three `fx.copy` calls (§5.3) are gated by `pred=thr_pC` (§5.4) so overhang
...
def copy_kernel(A, B):
    ...
    A = fx.rocdl.make_buffer_tensor(A)          # buffer tensors (§5.2)
...
`zipped_divide` + `slice(_, (None, bid))` (§3.3.2) selects this block's 8×24 tile
from A and B.
...
and this becomes the staging half of a GEMM (§5.6). No predication is needed
...
One `16×16×4` MFMA atom (§6.1), tiled `2×2×1` (§6.2) → a 32×32 warp tile; with 256
...
```python
    ...
    tiled_copy_C = fx.make_tiled_copy_C(copy_atom, tiled_mma)   #   (§4.2)
...
```python
    ...
    frag_B = thr_mma.make_fragment_B(bB)         #   (§4.4, §6.3)
...
`retile` (§4.4) is the key correctness step: it maps the copy's TV layout onto the
...
The compute is one `fx.gemm` (§6.3), lowering to `rocdl.mfma.*`. Because `K=8`
here fits in a single load, there is no K-loop — the GEMM puzzles add the
accumulate-over-K loop (§6.4), LDS staging, double-buffering, and swizzle to turn
...
Chapter 8 (debugging), Chapter 9's reference tables, and the puzzles are all you
need.
