...
**TiledCopy** (Chapter 4), and a single `fx.copy` / `fx.copy_atom_call` per
transfer.
...
**buffer tensor**, which carries the shape/stride (a Layout, §3.2) *and* the
...
From there you use the layout algebra of Chapters 3–4 (`zipped_divide`, `slice`,
...
Once a thread has a source partition and a destination partition (Chapter 4),
...
```python
# global -> registers (A and B operands), retiled to fragment order (§4.4)
...
Add an LDS staging tier (Chapter 7 / GEMM puzzles) and it becomes
...
optimizable. Chapter 6 does the same for the matrix multiply itself.
...
lanes hit distinct banks (Chapter 7 and the GEMM puzzles). The copy calls do not
