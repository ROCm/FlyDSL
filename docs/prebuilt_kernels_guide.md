...
| **Softmax** | `build_softmax_module(M, N, dtype)` | Layout API (`@flyc.kernel`) | f32, f16, bf16 | Online softmax, opt-in `BLOCK_THREADS` autotuning |
...
|---|---|---|
| `BLOCK_THREADS` | 256 by default; 64/128/256/512 when tuned | Threads per row block |
...
`BLOCK_THREADS` also selects the data-movement path: with
`tile_cols = BLOCK_THREADS * vec_width`, a row takes the vectorized fast path when
...
`BLOCK_THREADS ∈ {64,128,256,512}` × `waves_per_eu ∈ {none,1,2}`; each candidate is
numerically validated before it can be ranked. Artifacts use the name `softmax_fwd`; see
