...
**fence 速查**（图里 `[…]` 标的就是它）：`release`=把我之前的写**推出去**让别人能看见；`acquire`=把别人的写**拉进来**我能看见（成对用）。**agent**=只在本卡 GPU 内可见（便宜）；**system**=跨卡 xGMI 可见（贵，仅跨卡 payload 才用）。激活量化是本核之前的一个独立 FlyDSL 小核（`per_1x32_mx_quant`），把 bf16 激活转成 fp8/fp4 payload + e8m0 行 scale 落 HBM，本融合核再从 HBM 读它 dispatch（把这一步融进本核的方案见 §11）。
...
- **接 stage2 时**：stage1 走 atom 契约（a2@logical + srcmap，§9.2）。大 bs 自动走 **compact+atom combo**（§9.5）——compact dispatch 绕 4GB + GEMM1 写 atom-logical a2，stage2 接线不变、零额外动作、逐位一致。
...
**做法**（static-tiles，默认路径）：GEMM 不读紧凑列表，改走**静态固定槽位**——
...
> 守卫：`cu % gx != 0` 自动关 xcd。`xcd_swizzle` 为编译参数（WGM 组大小，0=关；由 `resolve_stage1_config` 给出）。

---

## 6. 性能数据

口径：8×MI355X，网络 native topk（v4=6,r1_v3=8），CUDAGraph device-time，8 卡均值，`scheme=fixedslot`（static-tiles 默认路径 / 大 bs 自动 compact）。测量入口 `tests/kernels/test_mega_moe.py`。baseline = ATOM 生产全栈（`per_1x32_mx_quant_hip` 量化 → aiter `moe_sorting_fwd` → aiter `mxfp4_moe_sort_hip` → `mixed_moe_gemm1` → 融合 GEMM2+combine）。激活量化两边都在计时圈外预量化（都不含 quant 耗时；mega 生产 `forward` 内的量化已换成原生 FlyDSL `per_1x32_mx_quant`，与 aiter 逐值等价、性能持平，见 §11）。

> **精度**：dequant 后 `relL2(mega,atom)≈0`（多随机种子 × 多 bs）、`relL2(mega,f16)=0.027(fp8)/0.143(fp4)`（纯量化噪声）。

**全 bs stage1-only speedup（mega / baseline，device-time，输出 fp8/fp4 a2）**:
...
### 6.1 端到端（stage1+stage2，全 bs 扫描）

口径：8×MI355X，a8w4，native topk（v4=6,r1_v3=8），CUDAGraph device-time，8 卡均值（`tests/kernels/test_mega_moe.py`）。compact/fixedslot 按 buffer 大小自动切（无 env）。每个 bs 独立进程跑（避免 mori 对称堆跨配置累积爆堆）。`mega-vs-base` = megav1 与 ATOM 生产全栈输出的 relL2（均 `~1e-5`，纯 fp8 量化抖动，逐位级一致，全 PASS）。
...
- **oracle = 端到端 mega vs ATOM 生产栈**（`tests/kernels/test_mega_moe.py::_run_full_e2e`，§10）：每 rank 把加权输出映射回源 token（同一把 32-bit 键 `(k_slot<<24)|(src_pe*mtpr+src_tok)`）后比 relL2：
  - mega：`srcmap_em[row]` 即键，值 = `out[row]`（real 行 `[le*cap, le*cap+count)`）。
  - atom：`FlyDSLDispatchCombineIntraNodeOp.dispatch` → `aiter.moe_sorting_fwd` → scale-sort → `mixed_moe_gemm1`；recv 槽 `s` 第 `k` 个 LOCAL expert → 键 `(k<<24)|tok_id_to_src[s]`。
- 判据：**键集一致**（无丢 tile / 多余行）**且**每键误差落在量化地板（`mega == atom-fp8`）。丢 tile ⇒ 键集不符；数值损坏 ⇒ 逐行误差 → 都判 FAIL；**全 rank PASS 才 PASS**。

### 7.1 量化输出口径（生产默认 fp8/fp4 + e8m0）

两边都输出 fp8/fp4 + e8m0 scale，按键 dequant 回 f32 再比 relL2。三组 relL2 同时报：
...
### 8.1 开关 / 配置（默认即最优，无需任何设置）

stage1 的优化开关**都已从 env 收敛为编译期参数**，由 `resolve_stage1_config` + tune 配置（`MegaStage1` JSON 分桶，`mega_tuned_tile` / `mega_gemm2_tuned_table`）解析，`compact` 由 buffer 大小自动决定：

- **static-tiles**：默认路径（readlane 现推 `(expert,k)` + reset-defer + agent-fence，§4.5/§4.6）；`epr>64` 自动回退 se/trb 路径。
- **compact**：`max_buf ≥ 3GB 或 epr>64` 自动启用（绕 4GB，§1.2），否则 fixedslot。接 stage2 时大 bs 走 compact+atom combo（§9.5），同为自动。
- **XCD swizzle**：`xcd_swizzle` 编译参数（组大小，默认 4）；`cu%gx≠0` 自动关（如 v4_pro gx=12，§5）。
- **tile / waves_per_eu / b_nt / gemm2 tile** 等：由 tune JSON 分桶给出，未命中用算子默认。
...
a2_noadapt  (5 行, dense, 行号 = recv 槽号)         sorted 元数据(也都按这 5 行槽序):
  ...
  行4 = A4   (e1)
```
这是 stage1 单独使用（standalone，未接 stage2）时的天然输出:**最紧凑、最快**(连续写,无散射)。
...
- **为何要**:combine 的 scatter-back 循环以「本卡收到多少个 distinct 源 token」为界。一个源 token 可能路由到本卡多个 local expert(占多个 recv 槽),要**去重**才得 distinct 数。否则需为拿 distinct 数额外跑一次 `comb_op.dispatch`,是冗余。
...
| | **fixed-slot atom**(小-中 bs) | **compact + atom combo**(大 bs，自动) |
| --- | --- | --- |
...
1. **disp 表共存**(`mega_moe.py` 的 disp 表构建)。compact_ag 不用 idx 19/20(那是非-ag compact 的 compact_base);故 combo 覆写:
...
- **端到端真链路**(`tests/kernels/test_mega_moe.py::MegaMoE`):`mega-vs-baseline relL2 ~1e-5`(逐位级复刻生产 atom 路径)——三网络全 bs、多随机种子全 PASS;compact/fixedslot 自动切,大 bs 走 compact+atom combo。
...
- **gemm2_combine 本体 bit-exact**:`tests/kernels/test_profiler_moe_gemm2_combine.py --mode verify` 三网络 `out_tok`/`out_wts` abs/rel max=0。
...
入口 `tests/kernels/test_mega_moe.py`（8-rank torchrun，EP8）。每个 `(network, quant, bs)` 跑一遍端到端 `_run_full_e2e`，同时出**精度门禁 + CUDAGraph 性能**，三条流水对比：

| 路径 | 说明 |
| --- | --- |
| **megav1** | `MegaMoE` 单算子（fp8/fp4 激活 → bf16） |
| **atom-fp8**（主基线） | production fp8 dispatch → `aiter.moe_sorting_fwd` → `aiter.mxfp4_moe_sort_hip` → `mixed_moe_gemm1` → 融合 GEMM2+combine |
| **atom-bf16**（oracle） | bf16 dispatch 参考路径，精度 sanity |

### 10.1 计时

CUDAGraph device-time（`_cg_time`：capture 一次 → 多次 replay → CUDA-event 计时 → 跳前若干 → 均值 → 跨 8 卡 `_all_mean`）。融合 stage1 下计时体走 `forward_prequant`（量化预算在计时圈外，两边都不含 quant）。

### 10.2 公平性

- **量化在计时圈外**：基线用 `per_1x32_mx_quant_hip` 预量化；mega 生产 `forward` 内的量化已换成原生 FlyDSL `per_1x32_mx_quant`（与 aiter 逐值等价、性能持平，见 §11），e2e 计时用 `forward_prequant` 排除它。
- baseline 的 buffer 复位（`a2_e.zero_()` 等，大 bs 是 GB 级 memset、mega 无等价物）不计时。
- GEMM2 doweight 加权两边一致，比的是同一份 routing 权重下的输出。

### 10.3 精度门禁

每 rank 把加权输出 vs f32 torch oracle 比 relL2，要求落在量化地板（fp8≈0.027 / fp4≈0.143，不是 ~1.0）且 `mega == atom-fp8`；**全 rank PASS 才 PASS**（`[FULL-E2E] ... -> PASS (all N ranks)`）。`--seed` / `--n-seeds` 多种子防 trivial 假通过。实测三网络全 bs `relL2(mega,atom) ~1e-5`（逐位级一致）。

### 10.4 常用命令

```bash
# 端到端（v4_pro a8w4，全默认，自动 compact/fixedslot）
...
torchrun --standalone --nproc_per_node=8 tests/kernels/test_mega_moe.py \
  --network r1_v3 --quant a4w4 --bs-list 128,2048 --n-seeds 3
# 编译期不支持场景 / 路由校验（pytest，无需 GPU 集群）
python3 -m pytest tests/kernels/test_mega_moe_validation.py -q
```

> 网络 keys：`v4_flash`(4096/2048/256/top6)、`v4_pro`(7168/3072/384/top6)、`r1_v3`(7168/2048/256/top8)。tune JSON（`MegaStage1` 分桶）：`(r1_v3,a4w4)`、`(v4_pro,a8w4)` 有，`v4_flash` 无（用默认 tile）。
...
**为什么 naive 融合会亏**（每 token 本地 HBM 字节，md=model_dim，k=topk）：

- 现状（独立 quant + dispatch）：quant 读 `2·md`(bf16) + 写 `md`(fp8)；dispatch 读 `k·md`(fp8)。合计 `~(3+k)·md`。
...
| bs | megav1 µs（quant 除外）| quant µs | 融合天花板 = quant/(megav1+quant) |
| ---: | ---: | ---: | ---: |
| 8 | 258.7 | 2.51 | **0.96%** |
| 64 | 340.3 | 2.63 | 0.77% |
| 128 | 356.3 | 2.75 | 0.77% |
...
大 bs 也一样：quant 与 megav1 同步增长，比值稳定在 ~0.5–1%。且这是**完美融合**（quant 完全免费消失）的上限；实际融合还要把 quant 计算塞进 dispatch，真实收益 **< 1%**。

**结论：暂不做**。收益 <1%，而 smart 融合要重构那段极其精密的 cross-PE dispatch（per-token 化 + 内联量化 + LDS staging + CUDAGraph 安全 + 逐位正确），ROI 不划算。若未来 quant 变成更大头（如 dispatch 握手被显著优化后），或为架构上去掉独立核/中间 buffer，再考虑 smart 方案。

**风险（真做时）**：dispatch 是 CUDAGraph 安全、逐位正确的跨卡代码，改并行粒度 + 内联量化风险高；LDS 预算（每 token md fp8 + md/32 e8m0，md=7168 约 7.4KB）；小 bs 并行度降 k 倍需看 occupancy。应 flag-gated、fixedslot 先行、保留独立-quant 回退、e2e a8w4/a4w4 全 rank PASS 验证。
