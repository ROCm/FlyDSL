...
`flyc.compile_aot(...).export_to_c(...)` compiles a `@flyc.jit` launcher ahead
...
compiled = flyc.compile_aot(axpy, ptr, ptr, 1024, 1.0)  # traces + lowers, runs nothing
...
```python
flyc.compile_aot(launcher, *args, **kwargs) -> AOTCompiledFunction
```
...
- The launcher is traced and lowered; nothing is executed. No execution engine
  is created, no GPU module is loaded and no kernel is launched.
...
An existing workflow that already uses `flyc.compile(...)` can export the
returned callable directly. This reuses the same lowered specialization and
...
`flyc.compile` keeps its existing behavior, including submitting the initial
call. Use `compile_aot` when compilation must not execute or initialize a GPU
module.

### CPU-only compilation

`compile_aot` and `export_to_c` do not require a visible GPU. On a build host
without one, set `ARCH` to the architecture of the deployment GPU so both the
compiler backend and architecture-sensitive kernel construction use the same
target:
...
4. `__module_unload()` unloads the module from every loaded device and releases
   the state. Idempotent; later entry calls return `-1` until init/load again.
   Kernels already submitted must have finished (synchronize first).
