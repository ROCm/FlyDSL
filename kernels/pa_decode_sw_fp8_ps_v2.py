...
def compile_pa_decode_ps(
    ...
    def pa_decode_ps_kernel(
        ...
        for _wi in range(_work_start_idx, _work_end_idx, _work_step):
            ...
            def _qk_and_intra_softmax(k_ops, partition_start, v_block_base_dw, tile_token_offset_i32):
                    ...
                    for vt in range_constexpr(VTLOOP):
                        for j in range_constexpr(2):
                            tmp_out = rocdl.mfma_f32_16x16x32_fp8_fp8(
                                T.f32x4, [v_i64s[vhe * VTLOOP * 2 + vt * 2 + j], p_i64s[vhe * VTLOOP * 2 + vt * 2 + j], tmp_out, 0, 0, 0])
                            pv_results[vhe] = tmp_out
                o0 = o0 + pv_results[0] * vector.broadcast(T.f32x4, v_scale_val)
                o1 = o1 + pv_results[1] * vector.broadcast(T.f32x4, v_scale_val)
                return o0, o1
            ...
            def _process_block(phys_block, block_idx_in_work, rmax, rsum, o0, o1,
                ...
                base_pstart = block_idx_in_work * c_tpb_step * c_cps
                ...
                # v0_ops = _load_v_ops(v_base, c_zero)
                # ═══ Tile 0: QK + softmax write ═══
                d_out_0, v0_ops = _qk_and_intra_softmax(k0_ops, pstart[0], v_base, c_zero)
                # Issue V1 loads early — overlap with barrier + softmax
                #v1_ops = _load_v_ops(v_base, c_256)
                gpu.barrier()  # BARRIER A: tile 0 softmax LDS visible
                rmax, rsum, o0, o1 = _cross_warp_softmax_and_prob_pack(d_out_0, rmax, rsum, o0, o1)

                # ═══ Tile 1: QK before tile 0 PV barrier ═══
                # k1_ops = _unflatten_k(k1_flat)
                d_out_1, v1_ops = _qk_and_intra_softmax(k1_ops, pstart[1], v_base, c_256)
                # Issue V2 loads early — overlap with barrier + PV(0) + softmax
                #v2_ops = _load_v_ops(v_base, c_512)
               ...
               # K prefetch for next block (base pre-computed by caller)
                ...
                o0, o1 = _pv_mfma(v0_ops, o0, o1)  # tile 0 PV
                rmax, rsum, o0, o1 = _cross_warp_softmax_and_prob_pack(d_out_1, rmax, rsum, o0, o1)
                # k2_flat = _load_k_flat_ps(next_k_base, c_512)

                # ═══ Tile 2 ═══
                # k2_ops = _unflatten_k(k2_flat)
                d_out_2, v2_ops = _qk_and_intra_softmax(k2_ops, pstart[2], v_base, c_512)
                # Issue V3 loads early — overlap with barrier + PV(1) + softmax
                # v3_ops = _load_v_ops(v_base, c_768)
                ...
                o0, o1 = _pv_mfma(v1_ops, o0, o1)  # tile 1 PV
                rmax, rsum, o0, o1 = _cross_warp_softmax_and_prob_pack(d_out_2, rmax, rsum, o0, o1)

                # ═══ Tile 3 ═══
                # k3_ops = _unflatten_k(k3_flat)
                d_out_3, v3_ops = _qk_and_intra_softmax(k3_ops, pstart[3], v_base, c_768)
                ...
                o0, o1 = _pv_mfma(v2_ops, o0, o1)  # tile 2 PV
                rmax, rsum, o0, o1 = _cross_warp_softmax_and_prob_pack(d_out_3, rmax, rsum, o0, o1)
                ...
                o0, o1 = _pv_mfma(v3_ops, o0, o1)  # tile 3 PV
