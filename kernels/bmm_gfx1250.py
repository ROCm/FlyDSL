"""Batched WMMA GEMM (BMM) kernel for gfx1250.

Implements: C[b,m,n] = A[b,m,k] @ B[b,k,n]
with TDM async copy and multi-stage LDS buffering.

DeepSeek-V4 use case: einsum("sgd,grd->sgr", o, wo_a)
  B=G=16, M=T (runtime), K=D=4096, N=R=1024
  A: [G, T, D]  (pass o.permute(1,0,2).contiguous())
  B: [G, D, R]  (pass wo_a.transpose(1,2).contiguous())
  C: [G, T, R]
"""
...
def compile_bmm_gfx1250(
    *,
    ...
    """Compile a batched WMMA GEMM kernel with TDM async copy.

    Returns a JitFunction: launch_fn(arg_c, arg_a, arg_b, M, stream)

    Tensors passed as flat 1-D views:
      arg_a: A[B, M, K] flattened
      arg_b: B[B, K, N] flattened (B stores [K, N] row-major per batch)
      arg_c: C[B, M, N] flattened

    Args:
        B: batch size (compile-time constant, e.g. G=16 for V4).
        N, K: matrix inner dimensions (compile-time constants).
        M: hint for the M dimension (0 = fully runtime).
        num_buffers: 2=double, 3=triple, 4=quad LDS buffering.
        use_tdm_store: TDM store epilogue via LDS (True) or buffer_store (False).
    """
    _ = M  # M is runtime; captured only to mark as used
    if B < 1:
    ...
    stage_layout = SmemAllocator(None, arch=gpu_arch, global_sym_name="bmm_tdm_layout")
    ...
    tail_plan = _base_tail_plan  # TDM_LOADS_PER_STEP=2 (no wave_specialized factor)
    ...
    arena_alloc = SmemAllocator(
        ...
        arch=gpu_arch,
        global_sym_name=(
            f"bmm_{in_dtype}_{out_dtype}_B{B}_K{K}_N{N}_{tile_m}x{tile_n}x{tile_k}_"
    ...
    gy_compile = N // tile_n  # compile-time grid Y

    @flyc.kernel
    def kernel_bmm_gfx1250(
        arg_c: fx.Tensor,
        arg_a: fx.Tensor,
        arg_b: fx.Tensor,
        i32_m: fx.Int32,
        ...
        # --- Batch offsets (folded into 2D global_offset) ---
        m_idx = arith.index_cast(T.index, i32_m.ir_value())
        a_batch_off = bz * m_idx           # row offset for A[bz, :, :] in [B*M, K]
        b_batch_off = bz * arith.index(K)  # row offset for B[bz, :, :] in [B*K, N]
        c_batch_row_off = bz * m_idx       # row offset for C[bz, :, :] in [B*M, N]
        ...
        pipeline_fence(outstanding=2 * (num_buffers - 2), use_cluster=False)
        ...
        if loop_iters > 0:
            ...
            for loop_iter, state in range(0, loop_iters, 1, init=init_args):
                ...
                for buf_idx in range_constexpr(num_buffers):
                    ...
                    pipeline_fence_signal(outstanding=_fence_outstanding, use_cluster=False)
        ...
        for _load_stage, _compute_stage, _outstanding in tail_plan:
            if _outstanding == -1:
                ...
                pipeline_fence_signal(outstanding=_outstanding, use_cluster=False)
                pipeline_fence_wait(use_cluster=False)
    ...
    cache_tag = (B, in_dtype, out_dtype, K, N, tile_m, tile_n, tile_k,
                 m_warp, n_warp, num_buffers, waves_per_eu,
                 l2_prefetch_distance, use_tdm_store, inst_prefetch, expert_sched_mode)

    @flyc.jit
    def launch_bmm_gfx1250(
        arg_c: fx.Tensor,
        arg_a: fx.Tensor,
        arg_b: fx.Tensor,
        i32_m: fx.Int32,
        ...
        launcher = kernel_bmm_gfx1250(arg_c, arg_a, arg_b, i32_m)
        ...
        if llvm_opts:
            launch_bmm_gfx1250.compile_hints["llvm_options"] = llvm_opts

    return launch_bmm_gfx1250


__all__ = ["compile_bmm_gfx1250"]
