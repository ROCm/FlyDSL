...
"""FP8 causal FMHA prefill (paged, vec_k_col_v) for gfx942 — direct K LDS load.

This experiment is derived from ``fmha_prefill_fp8_v7.py``. It keeps the
diagonal-pair scheduler and math identical, but sends K cache loads directly
from global memory to LDS with ``buffer_load_to_lds``. V still stages through
VGPRs so this isolates the lower-risk K side.
"""
...
_alloc = SmemAllocator(None, arch="gfx942", global_sym_name="fmha_prefill_fp8_v12_smem")
_K_BYTES = BN * HD
_V_BYTES = HD * BN
_K_OFF = 0
...
def attn_kernel(
    ...
    def load_kv_regs(kv0_):
        ...
        return kc_offsets, vc_words

    def store_kv_to_lds(kc_offsets, vc_words, kbuf_off, vbuf_off):
        for p in fx.range_constexpr(NPASS):
            ...
            if guard is not None:
                if guard:
                    for d in fx.range_constexpr(4):
                        ...
                        k_lds_ptr = fx.buffer_ops.get_element_ptr(
                            k_lds_ptr_base, byte_offset=fx.Index(k_dst + k_dword_off)
                        )
                        fx.rocdl.buffer_load_to_lds(rk, k_lds_ptr, kc_offsets[p] + k_dword_off, size_bytes=4)
