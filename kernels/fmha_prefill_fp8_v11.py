...
"""FP8 causal FMHA prefill (paged, vec_k_col_v) for gfx942 — K-descale hoist.

This experiment is derived from ``fmha_prefill_fp8_v7.py``. It keeps the
diagonal-pair scheduler and math identical, but issues the four K-descale
vector loads before the QK MFMA chain so their VMEM latency can overlap QK.
"""
...
_alloc = SmemAllocator(None, arch="gfx942", global_sym_name="fmha_prefill_fp8_v11_smem")
_K_BYTES = BN * HD
_V_BYTES = HD * BN
_K_OFF = 0
