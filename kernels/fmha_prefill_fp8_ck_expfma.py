...
_alloc = SmemAllocator(None, arch="gfx942", global_sym_name="fmha_prefill_fp8_ck_expfma_smem")
...
def attn_kernel(
    ...
    def process_qtile(qtile):
        ...
        def compute_kt_tile(kv0_outer, kbuf, vbuf, kdbuf, m_run, l_run, o_acc, do_mask):
            ...
            # Hoist the per-tile constant: exp arg = s*LOG2E + (log2_pscale - safe_m*LOG2E),
            # so each element is one fma (s*LOG2E + exp_bias) instead of sub+mul+add.
            exp_bias = fx.Float32(log2_pscale) - safe_m * fx.Float32(LOG2E)
            l_loc = fx.Float32(0.0)
            p_all = []
            for sub in fx.range_constexpr(NSUB):
                p_sub = []
                for i in fx.range_constexpr(16):
                    p = fx.Float32(fx.rocdl.exp2(f32t, _ar(s_all[sub][i] * fx.Float32(LOG2E) + exp_bias)))
                    p_sub.append(p)
                    l_loc = l_loc + p
                p_all.append(p_sub)
