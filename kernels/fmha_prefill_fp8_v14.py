...
"""FP8 causal FMHA prefill (paged, vec_k_col_v) for gfx942 — diagonal-pair variant.

This is an experiment derived from ``fmha_prefill_fp8_8wave.py``. It keeps the
known-good per-q-tile compute body but changes workgroup mapping so each CTA
processes a q tile and, for causal attention, its mirror tile from the opposite
side of the sequence. The intent is to port PyISA's diagonal-pair scheduling
without holding two tiles' live state at once.
"""
...
# Export the pair span so existing runners launch ceil(num_q_tiles / 2) CTAs.
BM = 2 * TILE_BM
BN = 32
NSLOT = 256
...
_alloc = SmemAllocator(None, arch="gfx942", global_sym_name="fmha_prefill_fp8_v14_smem")
_K_BYTES = BN * HD
_V_BYTES = HD * BN
_K_OFF = 0
...
def attn_kernel(
    ...
    # Idea A (v14): V-transpose via v_perm_b32 + WIDE stores. The 32kv x 128d V tile is covered by
    # 4kv x 4d BLOCKS (256 of them). Each thread owns one block: loads 4 dwords (dword k =
    # V[kv0+k, d0:d0+4], 4 contiguous-d fp8 in row-major global), transposes the 4x4 byte tile
    # in-register (12 v_perm), then writes 4 WIDE ds_write_b32 (each = 4 contiguous kv for one d)
    # into the SAME col-major vt_lds[d*BN + kv] layout GEMM2 already reads. Replaces 16 narrow
    # ds_write_b8/thread -> 4 wide b32 (the PMC-dominant LDS op). GEMM2 read path is UNCHANGED.
    VNSLOT = (BN // 4) * (HD // 4)  # 8 * 32 = 256 blocks
    VPASS = (VNSLOT + NTHREADS - 1) // NTHREADS
    vblk_valid = []
    vblk_kv0 = []  # block's first kv (0,4,..,28)
    vblk_d0 = []  # block's first d (0,4,..,124)
    for p in fx.range_constexpr(VPASS):
        vb = fx.Int32(p * NTHREADS) + tid
        vblk_valid.append(vb < fx.Int32(VNSLOT))
        vb_s = (vb < fx.Int32(VNSLOT)).select(vb, fx.Int32(0))
        vblk_kv0.append((vb_s // fx.Int32(HD // 4)) * fx.Int32(4))  # (vb // 32) * 4
        vblk_d0.append((vb_s % fx.Int32(HD // 4)) * fx.Int32(4))  # (vb % 32) * 4
    ...
    def _vload_dword(vrow, vd0):
        # one global dword = V[vrow, vd0:vd0+4] (4 contiguous-d fp8, row-major V).
        vrow_safe = (vrow < sk_i).select(vrow, fx.Int32(0))
        slot = page0 + vrow_safe // ps_i
        phys = fx.buffer_ops.buffer_load(rltd, slot, vec_width=1, dtype=fx.Int32)
        intra = vrow_safe % ps_i
        vidx = phys * v_page_stride + intra * v_tok_stride + v_head_off + vd0
        return fx.buffer_ops.buffer_load(rv, vidx // fx.Int32(4), vec_width=1, dtype=fx.Int32)
    ...
    def load_kv_regs(kv0_):
        ...
        # V: per 4kv x 4d block, load 4 dwords (one per kv row in the block).
        vc_blocks = []
        for p in fx.range_constexpr(VPASS):
            rows = []
            for k in fx.range_constexpr(4):
                rows.append(_vload_dword(kv0_ + vblk_kv0[p] + fx.Int32(k), vblk_d0[p]))
            vc_blocks.append(rows)
        return kc_words, vc_blocks

    def store_kv_to_lds(kc_words, vc_blocks, kbuf_off, vbuf_off):
        # K: wide b128 store, unchanged.
        for p in fx.range_constexpr(NPASS):
            kv_row = pass_kv[p]
            cg = pass_cg[p]
            ...
            if const_expr(NPASS > 1):
                ...
                fx.Vector(kc_words[p]).bitcast(fx.Int8).store(k_lds, [fx.Index(k_dst)])
        # V: in-register 4x4 byte transpose (perm_b32) then 4 WIDE b32 stores.
        # in[k] = [V[kv0+k,d0+0], V[kv0+k,d0+1], V[kv0+k,d0+2], V[kv0+k,d0+3]] (byte j = d0+j).
        # want out[j] = [V[kv0+0,d0+j], V[kv0+1,..], V[kv0+2,..], V[kv0+3,..]] (4 contiguous kv,
        # fixed d=d0+j) -> store at vt_lds[(d0+j)*BN + kv0] as one b32.
        # perm_b32(hi,lo,sel): out byte i = byte sel.nibble[i] of {hi[7:4]:lo[3:0]}.
        def _xpose(in0, in1, in2, in3):
            r0 = in0.ir_value() if hasattr(in0, "ir_value") else in0
            r1 = in1.ir_value() if hasattr(in1, "ir_value") else in1
            r2 = in2.ir_value() if hasattr(in2, "ir_value") else in2
            r3 = in3.ir_value() if hasattr(in3, "ir_value") else in3
            # NOTE: v_perm_b32 sel is ONE BYTE per output byte (value 0-7 = source byte index in
            # {hi[7:4]:lo[3:0]}), NOT nibbles. (Verified on-GPU: sel 0x05040100 -> out bytes [s0,s1,s4,s5].)
            outs = []
            for j in fx.range_constexpr(4):
                # ab = [in0.bj, in1.bj, x, x]: lo=in0(src0-3), hi=in1(src4-7); byte0=j, byte1=4+j.
                sel_ab = fx.Int32(((4 + j) << 8) | j).ir_value()
                ab = fx.rocdl.perm_b32(r1, r0, sel_ab)
                cd = fx.rocdl.perm_b32(r3, r2, sel_ab)  # [in2.bj, in3.bj, x, x]
                # out = [ab.b0, ab.b1, cd.b0, cd.b1]: lo=ab(src0,1), hi=cd(src4,5) -> bytes 0,1,4,5.
                sel_out = fx.Int32(0x05040100).ir_value()
                outs.append(fx.rocdl.perm_b32(cd, ab, sel_out))
            return outs

        for p in fx.range_constexpr(VPASS):
            rows = vc_blocks[p]
            outs = _xpose(rows[0], rows[1], rows[2], rows[3])
            kv0b = vblk_kv0[p]
            d0b = vblk_d0[p]

            def _emit():
                for j in fx.range_constexpr(4):
                    fx.Vector.from_elements([fx.Int32(outs[j])], fx.Int32).bitcast(fx.Int8).store(
                        vt_lds, [fx.Index(vbuf_off + (d0b + fx.Int32(j)) * fx.Int32(BN) + kv0b)]
                    )

            if const_expr(VPASS > 1):
                if vblk_valid[p]:
                    _emit()
            else:
                _emit()
    ...
    def process_qtile(qtile):
        ...
        for kt_iv, st in range(fx.Index(0), fx.Index(n_kv_rt), fx.Index(1), init=init_state):
            ...
            # Idea B (software-pipelined GEMM2, from aiter flash_attn_func_gfx1201): instead of
            # bulk-reading all 8 V packs then running 8 MFMAs, preload the FIRST V pack, then in
            # each step issue the current MFMA while the NEXT pack's ds_read is already in flight —
            # hiding LDS-read latency behind the MFMA instead of paying it up front. The 8 MFMA
            # steps run in order [dt0s0, dt0s1, dt1s0, ...]; step `st` accumulates into o_acc[st//2].
            def _load_v_pack(step):
                dt_ = step // 2
                s_ = step % 2
                d_col = fx.Int32(dt_ * 32) + (lane % fx.Int32(32))
                v_lds_elem = vbuf + d_col * fx.Int32(BN) + fx.Int32(s_ * 16) + half * fx.Int32(8)
                vv8 = fx.Vector.load(fx.typing.T.vec(8, fx.typing.T.i8), vt_lds, [fx.Index(v_lds_elem)])
                return fx.Vector(vv8).bitcast(fx.Int64)[0]

            NSTEP = DT * 2
            acc_dt = [fx.Vector(o_acc[dt]).ir_value() for dt in range(DT)]
            cur_v = _load_v_pack(0)
            for step in fx.range_constexpr(NSTEP):
                if const_expr(step + 1 < NSTEP):
                    next_v = _load_v_pack(step + 1)  # prefetch next pack while this MFMA runs
                dt = step // 2
                s = step % 2
                p_i64 = p_i64_s[s]
                a_raw = cur_v.ir_value() if hasattr(cur_v, "ir_value") else cur_v
                b_raw = p_i64.ir_value() if hasattr(p_i64, "ir_value") else p_i64
                acc_dt[dt] = fx.rocdl.mfma_f32_32x32x16_fp8_fp8(f32x16, a_raw, b_raw, acc_dt[dt], 0, 0, 0).res
                fx.rocdl.sched_mfma(1)
                if const_expr(step + 1 < NSTEP):
                    cur_v = next_v
            for dt in fx.range_constexpr(DT):
                o_acc[dt] = fx.Vector(acc_dt[dt])
