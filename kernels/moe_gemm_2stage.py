...
from flydsl.expr import arith as _arith_expr
...
class _ArithCompat:
    """Compatibility wrapper for legacy arith helper names."""

    def __getattr__(self, name):
        return getattr(_arith_expr, name)

    def i32(self, v):
        return _arith_expr.constant(v, type=T.i32)

    def f32(self, v):
        return _arith_expr.constant(v, type=T.f32)

    def cmpu(self, lhs, rhs, pred: str):
        pred_map = {
            "ult": _arith_expr.CmpIPredicate.ult,
            "ule": _arith_expr.CmpIPredicate.ule,
            "ne": _arith_expr.CmpIPredicate.ne,
        }
        p = pred_map.get(pred)
        if p is None:
            raise ValueError(f"Unsupported cmpu predicate: {pred!r}")
        return _arith_expr.cmpi(p, lhs, rhs)

    def as_value(self, v):
        return v

    def ext_f(self, ty, v):
        return _arith_expr.extf(ty, v)
    ...
    if True:
        ...
        def moe_gemm1(
            ...
            maxids_rsrc = buffer_ops.create_buffer_resource(
                arg_max_token_ids,
                max_size=False,
                num_records_bytes=arith.constant(4, index=True),
            )
            ...
            with _if_then(_if_blk):
                ...
                expert_rsrc = buffer_ops.create_buffer_resource(
                    arg_expert_ids,
                    max_size=False,
                    num_records_bytes=(size_expert_ids_in * arith.constant(4, index=True)),
                )
                ...
                for pair_i in range_constexpr(pair_iters):
                    k_iv = arith.constant(pair_i * (tile_k * 2), index=True)
    ...
    *,
    ...
    if True:
        ...
        def moe_reduction_kernel(
            ...
            m_tokens = arith.index_cast(ir.IndexType.get(), i32_m_tokens.ir_value())
            c_topk = arith.constant(topk, index=True)
            c_model_dim = arith.constant(model_dim, index=True)
            c_elem_bytes = arith.constant(4 if dtype_str == "f32" else 2, index=True)
            ...
            tok_ok = arith.cmpu(token_i32, m_tokens_i32, "ult")
            _if_tok = scf.IfOp(tok_ok)
            with _if_then(_if_tok):
                tile_cols = BLOCK_SIZE * VEC_WIDTH
                c_tile_cols = arith.constant(tile_cols, index=True)
                ...
                with _if_then(_if_col):
                    ...
                    with _if_then(_if_full):
                        ...
                        for lane in range_constexpr(VEC_WIDTH):
                            col = col_base + arith.constant(lane, index=True)
    ...
    def launch_moe_reduction(
        ...
        _ = _cache_tag
        gx = arith.as_value(arith.index_cast(ir.IndexType.get(), i32_m_tokens.ir_value()))
        stream_token = stream_ptr_to_async_token(stream_ptr)
