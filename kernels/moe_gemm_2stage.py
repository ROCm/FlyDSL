    ...
    *,
    ...
    if True:
        ...
        def moe_reduction_kernel(
            ...
            c_elem_bytes = fx.Index(ELEM_BYTES)
            ...
            with _if_then(_if_tok):
                ...
                col_base = tile_idx * fx.Index(tile_cols) + tid * c_vecw
                    ...
                    with _if_then(_if_full):
                        ...
                        if use_mask:
                            ...
                            for k in range_constexpr(topk):
                                m_idx_i32 = arith.index_cast(i32_type(), token_base + fx.Index(k))
                            ...
                            with _if_then(_if_lane):
                                ...
                                for k in range_constexpr(topk):
                                    x_idx_i32 = arith.index_cast(i32_type(),
                                        (token_base + fx.Index(k)) * c_model_dim + col)
                                    if use_mask:
                                        c0_i8 = arith.constant(0, type=i8_type())
                                        m_idx_i32 = arith.index_cast(i32_type(), token_base + fx.Index(k))
                                        mv = buffer_ops.buffer_load(mask_rsrc, m_idx_i32, vec_width=1, dtype=i8_type())
    ...
    if True:
        ...
        def moe_gemm1(
            ...
            with ir.InsertionPoint(_if_blk.then_block):
                ...
                # expert ids: [blocks] i32 -> bytes = size_expert_ids_in*4
                eid_nbytes_i32 = arith.index_cast(T.i32, size_expert_ids_in * arith.constant(4, index=True))
                expert_rsrc = buffer_ops.create_buffer_resource(
                ...
                # Expert id for this M tile (keep address math in `index`)
                expert_i32 = buffer_ops.buffer_load(expert_rsrc, bx, vec_width=1, dtype=T.i32)
                exp_valid = arith.cmpi(CmpIPredicate.ult, expert_i32, arith.constant(experts, type=T.i32)) # todo fix
                ...
                with ir.InsertionPoint(_ifexpert_of.then_block):
                    ...
                    c_k_div4 = ((k_in / c_a_pack) * arith.constant(int(elem_bytes), index=True)) / fx.Index(4)
                    ...
                    for i in range_constexpr(num_x_loads):
                        ...
                        t_idx = arith.index_cast(T.index, t_i32)
                    ...
                    def load_x_tile(base_k):
                        """Prefetch the per-thread X tile portion (gmem -> regs) for a given K base (in elements)."""
                        base_k_div4 = ((base_k / c_a_pack) * arith.constant(int(elem_bytes), index=True)) / fx.Index(4)
                        parts = []
                        for i in range_constexpr(num_x_loads):
                            idx_i32 = x_row_base_div4[i] + base_k_div4 + x_col_local_i32[i]
                            x_vec = load_x(idx_i32)
                    ...
                    def lds_load_packs_k64(curr_row_a_lds, col_base, lds_base):
                        ...
                        col_base_swz = col_base_swz_bytes if elem_bytes == 1 else (col_base_swz_bytes / fx.Index(2))
                        # Pass as list so layout_utils.crd2idx uses static arith path
                        idx_a16 = fx.crd2idx([curr_row_a_lds, col_base_swz], layout_lds)
                        idx_a16 = idx_a16 + lds_base
                        loaded_a16 = vector.load_op(vec16_x, lds_x, [idx_a16])
                    ...
                    for k_iv in range(fx.Index(0), c_k_main2, c2_tile_k):
                        # ---- stage 0: prefetch+store ping, compute pong ----
                        next_k1 = k_iv + tile_k
                        x_regs_ping = load_x_tile(next_k1)
    ...
    *,
    ...
    if True:
        ...
        def moe_gemm2(
            ...
            layout_b = b_layout.layout_b
            c_k0 = (k_in * arith.constant(int(a_elem_bytes), index=True)) / fx.Index(64)
            ...
            numids_rsrc = buffer_ops.create_buffer_resource(
                arg_num_valid_ids, max_size=False, num_records_bytes=arith.constant(4, type=T.i32)
            )
            num_valid_i32 = buffer_ops.buffer_load(
            ...
            def _moe_gemm2_then_body():
                ...
                c_k_div4 = ((k_in / c_a_pack) * arith.constant(int(a_elem_bytes), index=True)) / fx.Index(4)
                c_k_div4_i32 = arith.index_cast(T.i32, c_k_div4)
                layout_x_div4 = fx.make_layout((m_i32_v, c_k_div4_i32), stride=(c_k_div4_i32, 1))
                ...
                def load_x(idx_i32):
                    ...
                    """
                    if x_load_bytes == 16:
                        idx_elem = idx_i32 if a_elem_bytes == 1 else (idx_i32 * fx.Index(2))
                        return buffer_copy_gmem16_dwordx4(
                            buffer_ops,
                            vector,
                            elem_type=x_elem,
                ...
                def load_x_tile(base_k):
                    base_k_div4 = ((base_k / c_a_pack) * arith.constant(int(a_elem_bytes), index=True)) / fx.Index(4)
                    parts = []
                    for i in range_constexpr(num_x_loads):
                        idx_i32 = x_row_base_div4[i] + base_k_div4 + x_col_local_i32[i]
                        x_vec = load_x(idx_i32)
                ...
                if k_main2_py > 0:
                    c_k_main2 = fx.Index(k_main2_py)
                    for k_iv in range(fx.Index(0), c_k_main2, fx.Index(tile_k * 2)):
                        next_k1 = k_iv + tile_k
                        x_regs_ping = load_x_tile(next_k1)
    ...
    def launch_mixed_moe_gemm2(
        ...
        gy = (arith.index_cast(T.index, i32_size_expert_ids_in) + _c_pm_l - arith.constant(1, index=True)) / _c_pm_l
