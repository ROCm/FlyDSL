...
def compile_moe_gemm1(
    *,
    ...
    pipeline_manager = PreshufflePipelineManager(x_dtype, w_dtype, out_dtype, use_cshuffle_epilog)
    ...
    x_elem_bytes = pipeline_manager.get_a_elem_bytes()
    w_elem_bytes = pipeline_manager.get_b_elem_bytes()
    out_elem_bytes = pipeline_manager.get_out_elem_bytes()

    x_elem_pack = pipeline_manager.a_elem_pack
    w_elem_pack = pipeline_manager.b_elem_pack

    tile_k_bytes = int(tile_k) * int(x_elem_bytes)
    ...
    if (tile_k_bytes % 64) != 0:
        raise ValueError(
            ...
            f"(tile_k={tile_k}, elem_bytes={x_elem_bytes})"
        )
    ...
    def _x_vec16_type():
        return _get_mfma_dict_value("x_vec16_type", mfma_pipeline)
    def _w_vec16_type():
        return _get_mfma_dict_value("w_vec16_type", mfma_pipeline)
    ...
    is_fp8_a = mfma_pipeline == MfmaPipeline.F8F4_MXFP4_PIPELINE

    is_int_mode = is_int8 or is_int4

    no_epilogue_dequant = is_fp4 or is_f16_or_bf16

    pack_M = 2 if is_fp4 else 1
    pack_N = 2 if is_fp4 else 1
    pack_K = 2 if is_fp4 else 1

    # FP4 specific parameters for mfma_scale_f32_16x16x128_f8f6f4 (gemm1)
    # cbsz encodes A matrix type: 0 for fp8, 4 for fp4
    cbsz_g1 = 0 if mfma_pipeline == MfmaPipeline.F8F4_MXFP4_PIPELINE else 4
    blgp_g1 = 4

    is_gate_up_inter = is_fp4

    is_cast_out = out_dtype == "fp8"
    if is_cast_out and not use_cshuffle_epilog:
        raise ValueError("out_dtype='fp8' requires CShuffle epilogue (set use_cshuffle_epilog=True).")
    ...
    size_w = (experts * (2 * inter_dim) * model_dim) // w_elem_pack
    size_sorted = DYN
    size_expert_ids = DYN

    total_threads = 256
    bytes_x_per_tile = int(tile_m) * int(tile_k) * int(x_elem_bytes) // x_elem_pack
    if bytes_x_per_tile % total_threads != 0:
        raise ValueError(
            "tile_m*tile_k*elem_bytes must be divisible by "
            f"{total_threads}: tile_m={tile_m}, tile_k={tile_k}, elem_bytes={x_elem_bytes}"
        )
    ...
    module_name = (
        f"mfma_moe1_{x_dtype}_{w_dtype}_{out_dtype}_{epilog_tag}"
    ...
    class _MOE1(flir.MlirModule):
        ...
        def init_gpu_module(self):
            ...
            lds_x_bytes = 2 * int(tile_m) * int(lds_stride) * int(x_elem_bytes)
            lds_out_elem_bytes = 2 if out_dtype in ["fp16", "bf16"] else 4
            lds_out_bytes = lds_out_elem_bytes * tile_m * tile_n if _use_cshuffle_epilog else 0
            lds_total_bytes = max(lds_x_bytes, lds_out_bytes)
            lds_total_elems = lds_total_bytes if x_elem_bytes == 1 else (lds_total_bytes // 2)
            # x_lds_elem = I.f16 if is_f16 else (I.i8 if is_int8 else I.f8)
            x_lds_elem = I.f16 if is_f16_or_bf16 else (I.i8 if is_int_mode else I.f8)
        ...
        def moe_gemm1(
            ...
            vec16_x_elems = 16  // x_elem_bytes    # if x_elem_bytes == 1 else 8
            vec8_x_elems = 8  // x_elem_bytes      # if x_elem_bytes == 1 else 4
            vec4_x_elems = 4  // x_elem_bytes      # if x_elem_bytes == 1 else 2
            vec8_x = I.vec(vec8_x_elems, x_elem)
            vec16_x = I.vec(vec16_x_elems, x_elem)
            vec1_i64 = I.vec(1, i64)
            vec2_i64 = I.vec(2, i64)

            placeholder = arith.constant(0, index=True)
            ...
            acc_init = (arith.constant_vector(0, _mfma_output_pack_ty()))
            ...
            b_layout = make_preshuffle_b_layout(
                flir, arith, c_n=c_n_total, c_k=k_in // pack_K, kpack_bytes=kpack_bytes, elem_bytes=w_elem_bytes
            )
            ...
            atom_x_s16 = flir.make_copy_atom(x_elem, vector_size=vec16_x_elems)
            atom_x_s8 = flir.make_copy_atom(x_elem, vector_size=vec8_x_elems)
            atom_x_s4 = flir.make_copy_atom(x_elem, vector_size=vec4_x_elems)
            atom_x_g2r16 = flir.make_copy_atom(x_elem, vector_size=vec16_x_elems)
            atom_x_g2r8 = flir.make_copy_atom(x_elem, vector_size=vec8_x_elems)
            atom_x_g2r4 = flir.make_copy_atom(x_elem, vector_size=vec4_x_elems)
            ...
            with _if_blk.then():
                ...
                _use_cshuffle_epilog = bool(use_cshuffle_epilog)
                lds_out = (
                    SmemPtr(base_ptr, lds_x_ptr.byte_offset, I.f16 if not is_cast_out else I.f32, shape=(tile_m * tile_n,)).get()
                ...
                x_nbytes_idx = tokens_in * k_in * arith.constant(int(x_elem_bytes), index=True)
                ...
                # FP4 path uses scales in mfma_scale instruction, so we still need scale resources.
                if is_f16_or_bf16:
                    sx_rsrc = None
                    sw_rsrc = None
                elif is_fp4:
                    # FP4: scale is used in mfma_scale_f32_16x16x128_f8f6f4 instruction
                    ...
                    # fp8/int8 path: scale_x: [tokens] f32 -> bytes = tokens*4
                ...
                c_k_div4 = (k_in * arith.constant(int(x_elem_bytes), index=True)) / arith.index(4)
                layout_x_div4 = flir.make_layout((tokens_in, c_k_div4), stride=(c_k_div4, 1))
                tile_k_dwords = (int(tile_k) * int(x_elem_bytes)) // 4
                ...
                def load_x(idx_i32):
                    ...
                    """
                    if x_load_bytes == 16:
                        idx_elem = idx_i32 if x_elem_bytes == 1 else (idx_i32 * arith.index(2))
                ...
                def load_x_tile(base_k):
                    ...
                    base_k_div4 = (base_k * arith.constant(int(x_elem_bytes), index=True)) / arith.index(4)
                    parts = []
                    for i in range_constexpr(num_x_loads):
                ...
                col_offset_base_bytes = (
                    col_offset_base
                    if x_elem_bytes == 1
                    else (col_offset_base * arith.constant(int(x_elem_bytes), index=True))
                )
                ...
                n_intra_up = []
                n_blk_up = []
                # Single n_blk_list for interleaved mode (matching mixed_moe_gemm_2stage.py)
                n_blk_list = []
                n_intra_list = []
                col_g_list = []
                ...
                for ni in range_constexpr(num_acc_n):
                    ...
                    col_g = col_g // 2 + offset if is_gate_up_inter else col_g + offset
                    col_g = col_g + lane_mod_16
                    col_g_list.append(col_g)
    
                    # For both interleaved and non-interleaved layout, global_n maps to physical layout
                    ...
                    # For interleaved mode, use single list
                    n_blk_list.append(n_blk)
                    n_intra_list.append(n_intra)
                    
                    # For non-interleaved mode, keep separate gate/up lists
                ...
                m_repeat = tile_m // 16
                k_unroll = tile_k_bytes // 64  // pack_K

                # FP4 packed parameters for mfma_scale (gemm1)
                ...
                # --- FP4 Scale Load Functions for gemm1 ---
                def load_scale_inter(arg_scale, rsrc, layout, ku, mni):
                    """Load a single scale value for FP4 mfma_scale instruction (gemm1)."""
                    k_lane = lane_div_16
                    n_lane = lane_mod_16
                ...
                def load_b_scale_tile_inter(base_k):
                    """Load B scale tile for FP4 pipeline (gemm1)."""
                    b_scale_tile = []
                    for ku in range_constexpr(k_unroll_packed):
                        for ni in range_constexpr(num_acc_n_packed):
                            scale = load_scale_inter(
                                arg_scale_w,
                                sw_rsrc,
                                layout_b_scale,
                                ku + base_k,
                ...
                def prefetch_ab_scale_tile_inter(base_k):
                    """Prefetch A and B scale tiles for FP4 pipeline (gemm1)."""
                    return placeholder, load_b_scale_tile_inter(base_k)

                # --- FP4 B Load Logic for gemm1 (interleaved gate+up) ---
                def load_b_packs_k64_inter(base_k, ku: int, ni: int):
                    """Load B pack for FP4 pipeline with interleaved gate+up (gemm1)."""
                    base_k_bytes = base_k * arith.constant(int(w_elem_bytes), index=True)
                    k0_base = base_k_bytes / 64
                    k0 = k0_base + ku
                    k1 = lane_div_16
                    # Use n_blk_list directly for interleaved layout
                    ...
                    b16 = flir.copy(
                        ...
                        alignment=8,
                        return_vector=True,
                        src_buffer_resource=(w_rsrc if w_elem_bytes == 1 else None),
                        src_buffer_offset_in_bytes=(w_elem_bytes == 1),
                    )
                ...
                def load_b_tile_inter(base_k):
                    """Load B tile for FP4 pipeline with interleaved gate+up (gemm1)."""
                    b_tile = []
                    for ku in range_constexpr(k_unroll):
                        packs0 = []
                        packs1 = []
                        for ni in range_constexpr(num_acc_n):  # Match mixed_moe_gemm_2stage.py
                            b0, b1 = load_b_packs_k64_inter(base_k, ku, ni)
                            packs0.append(b0)
                            packs1.append(b1)
                        b_tile.append((packs0, packs1))
                    return b_tile
    
                acc_gate = [acc_init] * (num_acc_n * m_repeat) if not is_gate_up_inter else [acc_init] * (num_acc_n // 2 * m_repeat)
                acc_up = [acc_init] * (num_acc_n * m_repeat) if not is_gate_up_inter else [acc_init] * (num_acc_n // 2 * m_repeat)
                ...
                def lds_load_packs_k64(curr_row_a_lds, col_base_bytes, lds_base):
                    ...
                    col_base_swz = (
                        col_base_swz_bytes
                        if x_elem_bytes == 1
                        else (col_base_swz_bytes / arith.constant(int(x_elem_bytes), index=True))
                    )
                ...
                def compute_tile(
                    ...
                    *,
                    ...
                    epilogue_pf = None
                    if prefetch_epilogue and not no_epilogue_dequant:
                ...
                # --- FP4 Compute Tile using mfma_scale_f32_16x16x128_f8f6f4 (gemm1) ---
                def compute_tile_inter(
                    acc_gate_in,
                    acc_up_in,
                    b_tile_in,
                    lds_base,
                    *,
                    ...
                    """FP4 compute tile using mfma_scale for interleaved gate+up (gemm1)."""
                    gate_list = list(acc_gate_in)
                    up_list = list(acc_up_in)
                    mfma_res_ty = _mfma_output_pack_ty()
                    ...
                    # FP4 path using mfma_scale_f32_16x16x128_f8f6f4
                    for ku128 in range_constexpr(k_unroll_packed):
                        for mi in range_constexpr(m_repeat_packed):
                            ...
                            for ni in range_constexpr(num_acc_n_packed):
                                b_scale_i32 = b_scale[ku128 * num_acc_n_packed + ni] if b_scale else None
                                b_scale_val = vector.extract(b_scale_i32, static_position=[0], dynamic_position=[]) if b_scale_i32 else arith.i32(0x3F800000)
                                for ikxdl in range_constexpr(pack_K):
                                    k_idx = ku128 * pack_K + ikxdl
                                    b_packs0, b_packs1 = b_tile_in[k_idx]
                                    col_base = col_offset_base_bytes + (k_idx * 128) // x_elem_pack
                                    ...
                                    for imxdl in range_constexpr(pack_M):
                                        ...
                                        for inxdl in range_constexpr(pack_N):
                                            # Interleaved gate+up: even indices are gate, odd are up
                                            if inxdl % 2 == 0:
                                                current_list = gate_list
                                            else:
                                                current_list = up_list
                ...
                # Select pipeline functions based on is_fp4
                def _load_b_tiles(k_val):
                    if is_gate_up_inter:
                        return load_b_tile_inter(k_val // 2)
                    else:
                        return (load_b_tile(k_val, n_blk_gate, n_intra_gate), load_b_tile(k_val, n_blk_up, n_intra_up))

                def _compute_tile(acc_g, acc_u, b_cur, lds_base, a_scale, b_scale, *, prefetch_epilogue=False, a0_prefetch=None):
                    if is_gate_up_inter:
                        return compute_tile_inter(acc_g, acc_u, b_cur, lds_base, a_scale=a_scale, b_scale=b_scale, prefetch_epilogue=prefetch_epilogue, a0_prefetch=a0_prefetch)
                    else:
                        b_g, b_u = b_cur
                        return compute_tile(acc_g, acc_u, b_g, b_u, lds_base, prefetch_epilogue=prefetch_epilogue, a0_prefetch=a0_prefetch)

                def _prefetch_scale(k_val):
                    if is_gate_up_inter:
                        return prefetch_ab_scale_tile_inter(k_val // pack_K // 128)
                    return placeholder, placeholder

                # Prologue: prefetch tile0, store to LDS(cur), sync.
                k0 = arith.index(0)
                x_regs0 = load_x_tile(k0)
                b_cur = _load_b_tiles(k0)
                a_scale_pong, b_scale_pong = _prefetch_scale(k0)
                ...
                for k_iv in range(arith.index(0), c_k_main2, c2_tile_k):
                    ...
                    b_ping = _load_b_tiles(next_k1)
                    a_scale_ping, b_scale_ping = _prefetch_scale(next_k1)
    
                    acc_gate, acc_up, _ = _compute_tile(
                        acc_gate,
                        acc_up,
                        b_cur,
                        lds_base_pong,
                    ...
                    b_next = _load_b_tiles(next_k2)
                    a_scale_pong, b_scale_pong = _prefetch_scale(next_k2)
                ...
                b_ping = _load_b_tiles(k_tail1)
                a_scale_ping, b_scale_ping = _prefetch_scale(k_tail1)
                ...
                acc_gate, acc_up, epilogue_pf = _compute_tile(
                    acc_gate,
                    acc_up,
                    b_ping,
                    lds_base_ping,
                ...
                num_acc_n_final = num_acc_n // 2 if is_gate_up_inter else num_acc_n
                ...
                if _use_cshuffle_epilog:
                    ...
                    def write_row_to_lds(
                        *,
                        ...
                        sx = arith.f32(1.0) if no_epilogue_dequant else buffer_ops.buffer_load(sx_rsrc, t2, vec_width=1, dtype=f32)
                        ...
                        for ni in range_constexpr(num_acc_n_final):
                            col_local = col_base_local + (ni * 16)
    
                            acc_idx = mi * num_acc_n_final + ni
                            vg = vector.extract(
                            ...
                            if not is_cast_out:
                                ...
                                vec1_out_elem = vec1_f16
                                alignment = 2
                            else:
                                vec1_out_elem = vec1_f32
                                alignment = 1

                            lds_idx = row_base_lds + col_local
                            v1 = vector.from_elements(vec1_out_elem, [y])
                            vector.store(v1, lds_out, [lds_idx], alignment=alignment)
                    ...
                    mfma_epilog(
                        ...
                        tile_m=tile_m,
                        tile_n=tile_n // 2 if is_gate_up_inter else tile_n,
                        e_vec=4,
                        m_repeat=m_repeat,
                        num_acc_n=num_acc_n_packed if is_gate_up_inter else num_acc_n,
                        tx=tx,
                        lane_div_16=lane_div_16,
                        lane_mod_16=lane_mod_16,
                        bx_m=bx_m,
                        by_n=by_n // 2 if is_gate_up_inter else by_n,
                        n_tile_base=n_tile_base // 2 if is_gate_up_inter else n_tile_base,
                        lds_out=lds_out,
                        frag_elem_type=I.f32 if is_cast_out else I.f16,
                ...
                def _stage1_store_row(*, mi: int, ii: int, row_in_tile, row):
                    ...
                    sx0 = arith.f32(1.0) if no_epilogue_dequant else buffer_ops.buffer_load(sx_rsrc, t2, vec_width=1, dtype=f32)
                    sx = sx0
                    zero_out = arith.constant(0.0, type=_out_elem_type())
        ...
        def __call__(
            ...
            bdx = 256
            gx = (arith.index(2) * inter_in / arith.index(tile_n)) if is_gate_up_inter else (inter_in / arith.index(tile_n))
...
def compile_moe_gemm2(
    *,
    ...
    x_dtype:
      ...
      - "fp16": X/W are fp16
      - "int8": X/W are int8
      - "int4": W4A8 path: X is int8, W is packed int4 unpacked to int8 in-kernel
    w_dtype:
      - "fp8": W is fp8
      - "fp16": W is fp16
      - "int8": W is int8
      - "int4": W4A8 path: W is packed int4 unpacked to int8 in-kernel
    ...
    if (tile_k_bytes % 64) != 0:
        raise ValueError(
            f"tile_k_bytes must be divisible by 64, got tile_k_bytes={tile_k_bytes} "
            f"(tile_k={tile_k}, x_elem_bytes={x_elem_bytes})"
        )
    ...
    # FP4 specific parameters for mfma_scale_f32_16x16x128_f8f6f4
    pack_M = 2 if is_fp4 else 1
    pack_N = 2 if is_fp4 else 1
    pack_K = 2 if is_fp4 else 1
    is_fp8_a = mfma_pipeline == MfmaPipeline.F8F4_MXFP4_PIPELINE
    cbsz = 0 if is_fp8_a else 4  # fp8 a: cbsz=0, fp4 a: cbsz=4
    blgp = 4

    out_s = pipeline_manager.out_dtype
    ...
    size_scale_x = DYN
    # W is packed int4 for W4A8: 2 values per byte.
    size_w = (experts * model_dim * inter_dim) // w_elem_pack

    total_threads = 256
    tile_k_bytes = int(tile_k) * int(x_elem_bytes) // x_elem_pack
    if (tile_k_bytes % 64) != 0:
        raise ValueError(
            f"tile_k_bytes must be divisible by 64, got tile_k_bytes={tile_k_bytes} "
            f"(tile_k={tile_k}, elem_bytes={x_elem_bytes})"
    ...
    if bytes_x_per_tile % total_threads != 0:
        raise ValueError(
            "tile_m*tile_k*x_elem_bytes must be divisible by "
            f"{total_threads}: tile_m={tile_m}, tile_k={tile_k}, x_elem_bytes={x_elem_bytes}"
        )
    bytes_per_thread_x = bytes_x_per_tile // total_threads
    ...
    module_name = (
        f"mfma_moe2_{x_dtype}_{w_dtype}_{out_s}_{epilog_tag}"
        f"_t{tile_m}x{tile_n}x{tile_k}"
    ...
    class _MOE2(flir.MlirModule):
        ...
        def init_gpu_module(self):
            ...
            lds_total_bytes = max(lds_x_bytes, lds_out_bytes)
            lds_total_elems = lds_total_bytes // x_elem_bytes
            _state["lds_x_decl"] = allocator.allocate_array(_x_elem_type(), lds_total_elems)
            allocator.finalize()

        @flir.kernel
        def moe_gemm2(
            self: flir.T.i64,
            arg_out: lambda: T.memref(size_out, _out_elem_type()),
            ...
            vec4_f16 = I.vec(4, f16)
            vec16_x_elems = 16 if x_elem_bytes == 1 else 8
            vec8_x_elems = 8 if x_elem_bytes == 1 else 4
            vec4_x_elems = 4 if x_elem_bytes == 1 else 2
            vec8_x = I.vec(vec8_x_elems, x_elem)
            vec16_x = I.vec(vec16_x_elems, x_elem)
            vec1_i64 = I.vec(1, i64)
            vec2_i64 = I.vec(2, i64)

            placeholder = arith.constant(0, type=I.i32)

            acc_init = (
                arith.constant_vector(0, _mfma_output_pack_ty())
            )
            ...
            x_nbytes_idx = (tokens_in * c_topk) * k_in * arith.constant(int(x_elem_bytes), index=True)
            x_nbytes_i32 = arith.index_cast(i32, x_nbytes_idx)
            x_rsrc = buffer_ops.create_buffer_resource(
                arg_x, max_size=False, num_records_bytes=x_nbytes_i32
            )
            ...
            # bias resource for optional bias addition
            bias_rsrc = buffer_ops.create_buffer_resource(arg_bias, max_size=False) if enable_bias else None
            ...
            def _moe_gemm2_then_body():
                ...
                m_repeat = tile_m // 16
                k_unroll = tile_k_bytes // 64 if not is_fp4 else tile_k_bytes // 128 # K64-byte micro-step (2x MFMA)

                # FP4 packed parameters for mfma_scale_f32_16x16x128_f8f6f4
                k_unroll_packed = k_unroll // pack_K
                m_repeat_packed = m_repeat // pack_M
                num_acc_n_packed = num_acc_n // pack_N
                ...
                # --- FP4 Scale Load Functions for gemm2 ---
                def load_scale(arg_scale, rsrc, layout, ku, mni):
                    """Load a single scale value for FP4 mfma_scale instruction (gemm2)."""
                    k_lane = lane_div_16
                    n_lane = lane_mod_16
                    coord_pack = flir.make_coord(mni, ku, k_lane, n_lane)
                    idx_pack = flir.crd2idx(coord_pack, layout)
                ...
                def load_b_scale_tile(base_k):
                    """Load B scale tile for FP4 pipeline (gemm2)."""
                    b_scale_tile = []
                    for ku in range_constexpr(k_unroll_packed):
                        for ni in range_constexpr(num_acc_n_packed):
                            scale = load_scale(
                ...
                def load_a_scale_tile(base_k):
                    """Load A scale tile for FP4 pipeline (gemm2)."""
                    a_scale_tile = []
                    for ku in range_constexpr(k_unroll_packed):
                        for mi in range_constexpr(m_repeat_packed):
                            scale = load_scale(
                ...
                def prefetch_ab_scale_tile(base_k):
                    """Prefetch A and B scale tiles for FP4 pipeline (gemm2)."""
                    ...
                    if prefetch_epilogue:
                        ...
                        if not no_epilogue_dequant:
                            ...
                            for ni in range_constexpr(num_acc_n):
                                ...
                                sw_pf.append(buffer_ops.buffer_load(sw_rsrc, row_w_idx, vec_width=1, dtype=f32))
                            ...
                            for mi in range_constexpr(m_repeat_packed):
                                for ni in range_constexpr(num_acc_n_packed):
                                    ...
                                    for ikxdl in range_constexpr(pack_K):
                                        k_idx = ku128 * pack_K + ikxdl
                                        b_packs0, b_packs1 = b_tile_in[k_idx]
                                        col_base = col_offset_base + (k_idx * 128) // x_elem_pack
                ...
                def _prefetch_scale(k_val):
                    if is_fp4:
                        return prefetch_ab_scale_tile(k_val // pack_K // 128)
                    return placeholder, placeholder
                ...
                a_scale_ping, a_scale_pong = placeholder, placeholder
                _, b_scale_pong = _prefetch_scale(k0)
                store_x_tile_to_lds(x_regs0, lds_base_cur)
                gpu.barrier()
                ...
                for k_iv in range(arith.index(0), c_k_main2, arith.index(tile_k * 2)):
                    next_k1 = k_iv + tile_k
                    x_regs_ping = load_x_tile(next_k1)
                    b_ping = load_b_tile(next_k1 // pack_K)
                    _, b_scale_ping = _prefetch_scale(next_k1)

                    acc, _ = compute_tile(acc, b_cur, lds_base_pong, a_scale=a_scale_pong, b_scale=b_scale_pong, a0_prefetch=a0_prefetch_pong)
                    store_x_tile_to_lds(x_regs_ping, lds_base_ping)
                    hot_loop_scheduler()
                    gpu.barrier()
                    ...
                    next_k2 = k_iv + c2_tile_k
                    x_regs_pong = load_x_tile(next_k2)
                    b_next = load_b_tile(next_k2 // pack_K)
                    _, b_scale_pong = _prefetch_scale(next_k2)

                    acc, _ = compute_tile(acc, b_ping, lds_base_ping, a_scale=a_scale_ping, b_scale=b_scale_ping, a0_prefetch=a0_prefetch_ping)
                    store_x_tile_to_lds(x_regs_pong, lds_base_pong)
                    hot_loop_scheduler()
                    gpu.barrier()
                ...
                if odd_k_tiles:
                    ...
                    k_tail1 = (k_in + tile_k - 1) // tile_k * tile_k - tile_k if is_fp4 else k_in - tile_k
                    x_regs_ping = load_x_tile(k_tail1)
                    b_ping = load_b_tile(k_tail1 // pack_K)
                    _, b_scale_ping = _prefetch_scale(k_tail1)
                    ...
                    a0_prefetch_ping = lds_load_packs_k64(row_a_lds, col_offset_base_bytes, lds_base_ping)
                    acc, epilogue_pf = compute_tile(
                        acc, b_ping, lds_base_ping, a_scale=a_scale_ping, b_scale=b_scale_ping, prefetch_epilogue=True, a0_prefetch=a0_prefetch_ping
                    )
                ...
                if out_is_f32:
                    ...
                    def _stage2_row_atomic(*, mi: int, ii: int, row_in_tile, row):
                        ...
                        sx = arith.f32(1.0) if no_epilogue_dequant else buffer_ops.buffer_load(sx_rsrc, ts2, vec_width=1, dtype=f32)
                    ...
                    def write_row_to_lds(
                        *,
                        ...
                        for ni in range_constexpr(num_acc_n):
                            ...
                            lds_idx = row_base_lds + col_local
                            vec1_out = I.vec(1, _out_elem_type())
