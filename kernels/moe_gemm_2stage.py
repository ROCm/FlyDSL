...
def compile_moe_gemm1(
    *,
    ...
    in_dtype:
      ...
      - "int4_fp8": W4A_FP8 path: X is fp8, W is packed int4 unpacked to fp8 in-kernel
    """
    ...
    _valid_dtypes = ("fp8", "fp16", "bf16", "int8", "int8smooth", "int4", "int4_bf16", "int4_fp8")
    ...
    is_int4_fp8 = in_dtype == "int4_fp8"    # W4A_FP8: fp8 activations, packed int4 weights
    ...
    # Weight element bytes for addressing: 1 for packed int4, otherwise same as activation.
    ...
    # - fp8/int8/int4/int4_fp8: both scales needed
    needs_scale_x = not is_f16_or_bf16  # True for fp8/int8/int4/int4_fp8
    needs_scale_w = (not is_f16_or_bf16) or is_int4_bf16
    # Group-wise scale support for W4A16 and W4A8
    ...
    # For groupwise scale, weight scale is applied per-group (in load_b for W4A16,
    # in the K loop for W4A8/W4A_FP8), so epilogue can skip weight scale multiplication.
    ...
    class _MOE1(flir.MlirModule):
        ...
        def moe_gemm1(
            ...
            arg_w: lambda: T.memref(size_w, I.i8) if w_is_int4 else T.memref(DYN, I.bf16 if is_bf16 else (I.f16 if is_f16 else (I.i8 if is_int8 else I.f8))),
            ...
            w_elem = I.i8 if w_is_int4 else (I.bf16 if is_bf16 else (I.f16 if is_f16 else (I.i8 if is_int8 else I.f8)))
            f16 = I.f16
            bf16 = I.bf16
            f32 = I.f32
            i16 = I.i16
            ...
            zero_i32_acc = arith.constant_vector(0, vec4_i32) if is_int4_groupwise else None
            zero_f32_acc = arith.constant_vector(0.0, vec4_f32) if is_int4_fp8_groupwise else None
            ...
            # Match GEMM naming/pattern: row in LDS is lane_mod_16, and col base is lane_div_16*16.
                ...
                def load_b_raw_tile(base_k, blk_list, intra_list):
                    """Phase 1: Issue VMEM loads only (no dequant VALU). Returns raw data."""
                    if is_int4_bf16:
                        raw_data = []
                        ...
                        return raw_data
                    elif is_int4_groupwise or is_int4_fp8_groupwise:
                        raw_data = []
                        for ku in range_constexpr(k_unroll):
                            raw_ku = []
                            for ni in range_constexpr(num_acc_n):
                                ...
                                raw_ku.append((h0, h1, sc0, sc1))
                            raw_data.append(raw_ku)
                        return raw_data
                    elif is_int4_fp8 or is_int4:
                        raw_data = []
                        for ku in range_constexpr(k_unroll):
                            raw_ku = []
                            for ni in range_constexpr(num_acc_n):
                ...
                def dequant_b_tile(raw_data):
                    """Phase 2: VALU dequant only. Returns final packed tile."""
                    if is_int4_bf16:
                        b_tile = []
                        for ku in range_constexpr(k_unroll):
                            packs0, packs1 = [], []
                        ...
                        b_tile = []
                        for ku in range_constexpr(k_unroll):
                            packs0, packs1, scales0, scales1 = [], [], [], []
                            for ni in range_constexpr(num_acc_n):
                                h0, h1, sc0, sc1 = raw_data[ku][ni]
                        ...
                        return b_tile
                    elif is_int4_fp8_groupwise:
                        b_tile = []
                        for ku in range_constexpr(k_unroll):
                            packs0, packs1, scales0, scales1 = [], [], [], []
                            for ni in range_constexpr(num_acc_n):
                                h0, h1, sc0, sc1 = raw_data[ku][ni]
                                packs0.append(unpack_b_w4a_fp8(h0, arith, vector, rocdl))
                                packs1.append(unpack_b_w4a_fp8(h1, arith, vector, rocdl))
                                scales0.append(sc0)
                                scales1.append(sc1)
                            b_tile.append((packs0, packs1, scales0, scales1))
                        return b_tile
                ...
                def compute_tile(
                    ...
                    *,
                    ...
                    if is_int4_fp8_groupwise:
                        ...
                        for ku in range_constexpr(k_unroll):
                            b_gate_packs0, b_gate_packs1, b_gate_scales0, b_gate_scales1 = b_gate_tile_in[ku]
                            b_up_packs0, b_up_packs1, b_up_scales0, b_up_scales1 = b_up_tile_in[ku]
                            ...
                            for mi in range_constexpr(m_repeat):
                                ...
                                for ni in range_constexpr(num_acc_n):
                                    ...
                                    f32_g0 = mfma_fn(vec4_f32, [a0, b_gate_packs0[ni], zero_f32_acc, 0, 0, 0])
                                    gate_list[acc_idx] = _acc_scaled_f32(gate_list[acc_idx], f32_g0, b_gate_scales0[ni])
                                    f32_u0 = mfma_fn(vec4_f32, [a0, b_up_packs0[ni], zero_f32_acc, 0, 0, 0])
                                    up_list[acc_idx] = _acc_scaled_f32(up_list[acc_idx], f32_u0, b_up_scales0[ni])
                                    f32_g1 = mfma_fn(vec4_f32, [a1, b_gate_packs1[ni], zero_f32_acc, 0, 0, 0])
                                    gate_list[acc_idx] = _acc_scaled_f32(gate_list[acc_idx], f32_g1, b_gate_scales1[ni])
                                    f32_u1 = mfma_fn(vec4_f32, [a1, b_up_packs1[ni], zero_f32_acc, 0, 0, 0])
                                    up_list[acc_idx] = _acc_scaled_f32(up_list[acc_idx], f32_u1, b_up_scales1[ni])
                    elif is_int4_groupwise:
                        # W4A8 groupwise: per-K32 MFMA with fresh i32 acc → f32 scale → f32 running acc.
                        for ku in range_constexpr(k_unroll):
                            b_gate_packs0, b_gate_packs1, b_gate_scales0, b_gate_scales1 = b_gate_tile_in[ku]
                            b_up_packs0, b_up_packs1, b_up_scales0, b_up_scales1 = b_up_tile_in[ku]
                            ki64 = arith.constant(ku * 64, index=True)
                            ...
                            for mi in range_constexpr(m_repeat):
                                ...
                                for ni in range_constexpr(num_acc_n):
                                    acc_idx = mi * num_acc_n + ni
                                    # K32 half 0: fresh i32 MFMA, then scale and accumulate into f32
                                    i32_g0 = mfma_i32_k32(vec4_i32, [a0, b_gate_packs0[ni], zero_i32_acc, 0, 0, 0])
                                    gate_list[acc_idx] = _acc_scaled_i32_to_f32(gate_list[acc_idx], i32_g0, b_gate_scales0[ni])
                                    i32_u0 = mfma_i32_k32(vec4_i32, [a0, b_up_packs0[ni], zero_i32_acc, 0, 0, 0])
                                    up_list[acc_idx] = _acc_scaled_i32_to_f32(up_list[acc_idx], i32_u0, b_up_scales0[ni])
                                    # K32 half 1
                                    i32_g1 = mfma_i32_k32(vec4_i32, [a1, b_gate_packs1[ni], zero_i32_acc, 0, 0, 0])
                                    gate_list[acc_idx] = _acc_scaled_i32_to_f32(gate_list[acc_idx], i32_g1, b_gate_scales1[ni])
                                    i32_u1 = mfma_i32_k32(vec4_i32, [a1, b_up_packs1[ni], zero_i32_acc, 0, 0, 0])
                                    up_list[acc_idx] = _acc_scaled_i32_to_f32(up_list[acc_idx], i32_u1, b_up_scales1[ni])
                    else:
                        for ku in range_constexpr(k_unroll):
                ...
                k0 = arith.index(0)
                b_raw_gate0 = load_b_raw_tile(k0, n_blk_gate, n_intra_gate)
                b_raw_up0 = load_b_raw_tile(k0, n_blk_up, n_intra_up)
                x_regs0 = load_x_tile(k0)
                b_gate_cur = dequant_b_tile(b_raw_gate0)
                b_up_cur = dequant_b_tile(b_raw_up0)
                ...
                for k_iv in range(arith.index(0), c_k_main2, c2_tile_k):
                    ...
                    next_k1 = k_iv + tile_k
                    b_raw_gate_ping = load_b_raw_tile(next_k1, n_blk_gate, n_intra_gate)
                    b_raw_up_ping = load_b_raw_tile(next_k1, n_blk_up, n_intra_up)
                    x_regs_ping = load_x_tile(next_k1)
                    b_gate_ping = dequant_b_tile(b_raw_gate_ping)
                    b_up_ping = dequant_b_tile(b_raw_up_ping)
                    ...
                    b_raw_gate_next = load_b_raw_tile(next_k2, n_blk_gate, n_intra_gate)
                    b_raw_up_next = load_b_raw_tile(next_k2, n_blk_up, n_intra_up)
                    x_regs_pong = load_x_tile(next_k2)
                    b_gate_next = dequant_b_tile(b_raw_gate_next)
                    b_up_next = dequant_b_tile(b_raw_up_next)
                ...
                k_tail1 = k_in - tile_k
                b_raw_gate_ping = load_b_raw_tile(k_tail1, n_blk_gate, n_intra_gate)
                b_raw_up_ping = load_b_raw_tile(k_tail1, n_blk_up, n_intra_up)
                    ...
                    def write_row_to_lds(
                        *,
                        ...
                        for ni in range_constexpr(num_acc_n):
                            ...
                            # For groupwise scale, weight scale is already applied per-group.
                            # W4A16: skip_epilogue_scale=True (no scale_x needed either).
                            # W4A8: skip_epilogue_scale=False, but sw_gate/sw_up are 1.0.
...
def compile_moe_gemm2(
    *,
    ...
    needs_scale_w = (not is_f16_or_bf16) or is_int4_bf16
    # Group-wise scale support for W4A16, W4A8, and W4A_FP8
    # NOTE: Only group_size=32 is supported due to int4 preshuffle layout constraints.
    use_groupwise_scale = (is_int4_bf16 or is_int4 or is_int4_fp8) and group_size > 0
    if use_groupwise_scale and group_size != 32:
        raise ValueError(
    ...
    # For groupwise scale, weight scale is applied per-group.
    skip_epilogue_scale = use_groupwise_scale and (not needs_scale_x)
    ...
    class _MOE2(flir.MlirModule):
        ...
        def moe_gemm2(
            ...
            arg_w: lambda: T.memref(size_w, I.i8) if w_is_int4 else T.memref(size_w, I.bf16 if is_bf16 else (I.f16 if is_f16 else (I.i8 if is_int8 else I.f8))),
            ...
            def _moe_gemm2_then_body():
                ...
                def compute_tile(acc_in, b_tile_in, lds_base, *, prefetch_epilogue: bool = False, a0_prefetch=None):
                    ...
                    if is_int4_fp8_groupwise:
                        for ku in range_constexpr(k_unroll):
                            b_packs0, b_packs1, b_scales0, b_scales1 = b_tile_in[ku]
                            ki64 = arith.constant(ku * 64, index=True)
                            col_base = col_offset_base_bytes + ki64
                            ...
                            for mi in range_constexpr(m_repeat):
                                ...
                                for ni in range_constexpr(num_acc_n):
                                    acc_idx = mi * num_acc_n + ni
                                    f32_p0 = mfma_fn(vec4_f32, [a0, b_packs0[ni], zero_f32_acc, 0, 0, 0])
                                    acc_list[acc_idx] = _acc_scaled_f32(acc_list[acc_idx], f32_p0, b_scales0[ni])
                                    f32_p1 = mfma_fn(vec4_f32, [a1, b_packs1[ni], zero_f32_acc, 0, 0, 0])
                                    acc_list[acc_idx] = _acc_scaled_f32(acc_list[acc_idx], f32_p1, b_scales1[ni])
                    elif is_int4_groupwise:
                        for ku in range_constexpr(k_unroll):
                            b_packs0, b_packs1, b_scales0, b_scales1 = b_tile_in[ku]
                            ki64 = arith.constant(ku * 64, index=True)
                            ...
                            for mi in range_constexpr(m_repeat):
                                ...
                                for ni in range_constexpr(num_acc_n):
                                    acc_idx = mi * num_acc_n + ni
                                    i32_p0 = mfma_i32_k32(vec4_i32, [a0, b_packs0[ni], zero_i32_acc, 0, 0, 0])
                                    acc_list[acc_idx] = _acc_scaled_i32_to_f32(acc_list[acc_idx], i32_p0, b_scales0[ni])
                                    i32_p1 = mfma_i32_k32(vec4_i32, [a1, b_packs1[ni], zero_i32_acc, 0, 0, 0])
                                    acc_list[acc_idx] = _acc_scaled_i32_to_f32(acc_list[acc_idx], i32_p1, b_scales1[ni])
                    else:
                        for ku in range_constexpr(k_unroll):
                ...
                # Prologue.
                k0 = arith.index(0)
                b_raw0 = load_b_raw_tile(k0)
                x_regs0 = load_x_tile(k0)
                b_cur = dequant_b_tile(b_raw0)
                store_x_tile_to_lds(x_regs0, lds_base_cur)
                gpu.barrier()
                ...
                for k_iv in range(arith.index(0), c_k_main2, arith.index(tile_k * 2)):
                    ...
                    x_regs_ping = load_x_tile(next_k1)
                    b_ping = dequant_b_tile(b_raw_ping)
                    ...
                    x_regs_pong = load_x_tile(next_k2)
                    b_next = dequant_b_tile(b_raw_next)
