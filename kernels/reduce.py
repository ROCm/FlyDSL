...
def _shuffle_offsets_for_warp(warp_size: int) -> list[int]:
    """Return the xor-shuffle offsets for a tree reduction within a wavefront.

    For wave64: [32, 16, 8, 4, 2, 1]
    For wave32: [16, 8, 4, 2, 1]
    """
    offsets = []
    s = warp_size // 2
    while s >= 1:
...
def make_block_reduce(
    *,
    ...
    def block_reduce(val, reduce_op_name):
        # Wavefront size: 64 for CDNA (gfx9xx), 32 for RDNA (gfx10xx/11xx/12xx).
        ...
        for sh in _shuffle_offsets_for_warp(WARP_SIZE):
