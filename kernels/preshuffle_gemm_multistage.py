    ...
    *,
    ...
    if use_async_copy and gpu_arch != "gfx950":
        raise NotImplementedError(
            f"Async_copy is only supported on gfx950, but got '{gpu_arch}'"
        )

    lds_stage = int(lds_stage)
    if lds_stage not in (1, 2, 3):
        raise ValueError(f"lds_stage must be 1, 2, or 3, got {lds_stage!r}")


    # Shared-memory allocators (one per LDS stage).
    allocators = [
        SmemAllocator(None, arch=gpu_arch, global_sym_name=f"smem_stage{stage}")
        for stage in range(lds_stage)
    ]
    _state = {}
    ...
    class _GEMM(flir.MlirModule):
        ...
        def init_gpu_module(self):
            ...
            # A-only multistage: allocate one LDS A-tile buffer per stage (1/2/3).
            for s in range(int(lds_stage)):
                _state[f"lds_a_stage{s}"] = allocators[s].allocate_array(
                    _elem_type(), buffer_size_elems
                )
        ...
        def kernel_gemm(
            ...
            bx = gpu.block_id("x")
            by = gpu.block_id("y")
            base_ptrs = [a.get_base() for a in allocators]

            # Build LDS stage buffers (A-only multistage).
            # Each stage has its own allocator / base pointer, so they don't alias.
            lds_a_stages = []
            # IMPORTANT: use `range_constexpr` so this unrolls at build-time.
            # A plain `range(...)` is lowered to `scf.for` and would produce a Value IV,
            # which cannot index Python lists/dicts during IR construction.
            for s in range_constexpr(int(lds_stage)):
                base_ptr_s = base_ptrs[s]
                lds_a_ptr_s = _state[f"lds_a_stage{s}"](base_ptr_s)
                lds_a_s = SmemPtr(
                    base_ptr_s,
                    lds_a_ptr_s.byte_offset,
                    _elem_type(),
                    shape=(tile_m * tile_k,),
                ).get()
                lds_a_stages.append(lds_a_s)

            # CShuffle output buffer: REUSE stage-0 buffer (A tiles are dead after mainloop).
            if use_cshuffle_epilog:
                lds_a0_ptr = _state["lds_a_stage0"](base_ptrs[0])
                lds_out = SmemPtr(
                    base_ptrs[0],
                    lds_a0_ptr.byte_offset,
                    T.f16,
                    shape=(tile_m * tile_n,),
                ).get()
            else:
            ...
            b_tile_ping = None
            b_tile_pong = None
            # ---------------- Pipeline (A-only multistage ring) ----------------
            # Semantics:
            # - stages=1: single LDS A buffer
            # - stages=2: prefetch A 1 tile ahead
            # - stages=3: prefetch A 2 tiles ahead
            # B stays register-prefetch (current behavior).

            num_tiles = K // tile_k
            if num_tiles <= 0:
                raise ValueError(f"Invalid num_tiles={num_tiles} (K={K}, tile_k={tile_k})")

            stage_count = int(lds_stage)
            lookahead = stage_count - 1  # stage3 => prefetch A two tiles ahead

            accs = [acc_init] * (num_acc_n * m_repeat)

            def prefetch_a_into_stage(tile_idx: int, stage_idx: int):
                k_base = arith.constant(tile_idx * tile_k, index=True)
                lds_buf = lds_a_stages[stage_idx]
                if use_async_copy:
                    prefetch_a_to_lds(k_base, lds_buf)
                else:
                    store_a_tile_to_lds(prefetch_a_tile(k_base), lds_buf)

            # Prologue:
            # - Always prefetch tile0 into stage0.
            # - For stage>=2, prefetch tiles 1..lookahead-1 into stages 1..lookahead-1.
            #   For stage3 (lookahead=2): this prefetches tile1 only; tile2 is prefetched
            #   during iteration t=0 into stage2.
            prologue_tiles = 1 if stage_count == 1 else min(num_tiles, lookahead)
            for ti in range_constexpr(prologue_tiles):
                prefetch_a_into_stage(ti, ti)
            b_tile_cur = prefetch_b_tile(arith.constant(0, index=True))
            gpu.barrier()
            ...
            for t in range_constexpr(num_tiles):
                stage_cur = t % stage_count
                is_last = (t == (num_tiles - 1))

                # B prefetch stays 1-tile ahead (registers).
                if not is_last:
                    b_tile_next = prefetch_b_tile(arith.constant((t + 1) * tile_k, index=True))

                # A prefetch is (stage_count-1) tiles ahead for stages>=2.
                # For stage3, this means the last two iterations do not prefetch A.
                if (lookahead > 0) and ((t + lookahead) < num_tiles):
                    stage_pf = (t + lookahead) % stage_count
                    prefetch_a_into_stage(t + lookahead, stage_pf)

                if is_last:
                    final_accs, scales = compute_tile(
                        accs,
                        b_tile_cur,
                        lds_a_stages[stage_cur],
                        is_last_tile=True,
                    )
                    break

                accs, _ = compute_tile(accs, b_tile_cur, lds_a_stages[stage_cur])

                hot_loop_scheduler()
                gpu.barrier()
                b_tile_cur = b_tile_next

            if final_accs is None or scales is None:
                raise RuntimeError("Pipeline did not produce final_accs/scales")
