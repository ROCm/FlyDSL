...
def attn_kernel(
    ...
    # cooperative K-load decode: 512 threads each move one i64 (8 fp8) of the 32x128 K tile.
    kc_kv = tid // fx.Int32(16)  # kv row 0..31 within the tile
    kc_rem = tid % fx.Int32(16)
    kc_cg = kc_rem // fx.Int32(2)  # feature group 0..7 (16 feats each)
    kc_hg = kc_rem % fx.Int32(2)  # low/high 8 of the group
    kc_lds_elem = kc_kv * fx.Int32(HD) + kc_cg * fx.Int32(16) + kc_hg * fx.Int32(8)

    vt_lds = SmemPtr(_alloc.get_base(), _V_OFF, fx.typing.T.i8, shape=(2 * _V_BYTES,)).get()
    # cooperative V-load decode: 512 threads each move 8 contiguous d (one i64) of the
    # 32-kv x 128-hd V slab. Global V row-major [ps, nk, hd] → d is the fast dim (coalesced).
    vc_kv = tid // fx.Int32(16)  # kv row 0..31
    vc_dg = tid % fx.Int32(16)  # which 8-d group 0..15 (HD/8)
    vc_d0 = vc_dg * fx.Int32(8)  # first d of this thread's 8
    ...
    def load_kv_regs(kv0_):
        ...
        kc_off = (
            kc_phys * k_page_stride + k_head_off + kc_cg * (ps_i * fx.Int32(16)) + kc_intra * fx.Int32(16) + kc_hg * fx.Int32(8)
        )
        kc_w_ = fx.buffer_ops.buffer_load(rk, kc_off // fx.Int32(4), vec_width=2, dtype=fx.Int32)
        vc_kvrow = kv0_ + vc_kv
        ...
        vc_w_ = fx.buffer_ops.buffer_load(rv, vc_vidx // fx.Int32(4), vec_width=2, dtype=fx.Int32)
        return kc_w_, vc_w_
    ...
    for kt_iv, st in range(fx.Index(0), fx.Index(n_kv_rt), fx.Index(1), init=init_state):
        ...
        # Prefetch + store NEXT tile into the OTHER buffer (overlaps this tile's GEMMs).
        ...
        # GEMM1: S[kv,q] = K @ Q^T (K operand read from LDS: kv=kv_local, k=ks*16+half*8)
        fx.rocdl.s_setprio(1)  # prioritize MFMA over other waves' memory ops
        ...
        # GEMM2: O[d,q] += V^T @ P. A=V (transposed LDS), B=P (register-resident).
        for dt in fx.range_constexpr(DT):
        ...
        fx.gpu.barrier()  # V/K LDS consumed; safe for next iter's cooperative writes
