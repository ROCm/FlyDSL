...
# ASYNC copy: K streams global->LDS via buffer_load_to_lds (no VGPR staging), which removes the
# VGPR-occupancy penalty that made KT>32 regress in the VGPR-staged baseline. Default KT=128 (CK kN0).
KT = int(os.environ.get("FMHA_KT", "128"))
...
# Corrected async global->LDS for K via buffer_load_to_lds (default ON in this async kernel).
# ROOT CAUSE of the old BUFK breakage (v12 + ck FMHA_BUFK): they passed a PER-LANE LDS pointer.
# The hw instruction (buffer_load_dword ... lds) forces M0 = readfirstlane(lds_ptr) (lane-0's value)
# and writes each lane's dword to LDS[M0 + lane*4] in lane order -- so any per-lane LDS term is
# dropped and the tile is scrambled. CORRECT usage: lds_ptr must be PER-WAVE-UNIFORM and the
# per-lane GLOBAL voffset must be arranged so lane L fetches the element destined for LDS slot L.
# (Proven bit-exact in tests/kernels/lds_dma_probe.py.)
BUFK = int(os.environ.get("FMHA_BUFK", "1")) != 0
...
# K async-DMA tiling: the K LDS tile is a contiguous [KT, HD] i8 block = KT*HD/4 dwords. Each
# buffer_load_to_lds moves 1 dword/lane => NTHREADS dwords/instruction. Lane L (within its wave)
# writes to LDS[wave_base + L*4], so we issue NDMA_K instructions, instruction i filling the
# contiguous dword window [i*NTHREADS, (i+1)*NTHREADS). KT is a multiple of 32 => KT*HD/4 (=KT*32)
# is a multiple of NTHREADS (=256), so the window tiling is exact (no remainder guard).
NK_DW = (KT * HD) // 4
NDMA_K = NK_DW // NTHREADS
LOG2E = 1.4426950408889634

_alloc = SmemAllocator(None, arch="gfx942", global_sym_name="fmha_prefill_fp8_ck_async_smem")
_K_BYTES = KT * HD  # K tile [KT kv x HD]
_V_BYTES = HD * KT  # V tile transposed [HD d x KT kv]
_K_OFF = 0
...
def attn_kernel(
    ...
    # Corrected async global->LDS DMA for the whole K tile (replaces the broken per-lane-pointer
    # BUFK path). The K LDS tile is a contiguous [KT, HD] i8 block (== KT*32 dwords, linear order
    # [kv, hd]); filling it linearly keeps GEMM1's K reads unchanged. For DMA instruction i, the
    # workgroup fills LDS dwords [i*NTHREADS, +NTHREADS); thread tid handles linear dword
    # g = i*NTHREADS + tid at LDS byte g*4 = kv_row*HD + (g%32)*4. The matching GLOBAL byte addr is
    # the vec_k layout kc_off + (byte-within-16B-group). lds_ptr is PER-WAVE-UNIFORM (base +
    # wave*64*4); the hw adds the per-lane L*4 to reach g. (See lds_dma_probe.py for the proof.)
    def dma_k_to_lds(kv0_, kbuf_off):
        wave = tid // fx.Int32(64)
        for i in fx.range_constexpr(NDMA_K):
            g = fx.Int32(i * NTHREADS) + tid
            kv_row = g // fx.Int32(32)
            b = (g % fx.Int32(32)) * fx.Int32(4)  # byte within the 128-byte HD row (0..124)
            cg = b // fx.Int32(16)  # feature-group 0..7
            bin_ = b % fx.Int32(16)  # dword byte within the 16B group (0,4,8,12)
            kvrow_g = kv0_ + kv_row
            kvrow_safe = (kvrow_g < sk_i).select(kvrow_g, fx.Int32(0))
            kslot = page0 + kvrow_safe // ps_i
            kphys = fx.buffer_ops.buffer_load(rltd, kslot, vec_width=1, dtype=fx.Int32)
            kintra = kvrow_safe % ps_i
            voff = kphys * k_page_stride + k_head_off + cg * (ps_i * fx.Int32(16)) + kintra * fx.Int32(16) + bin_
            lds_byte = fx.Int32(kbuf_off) + fx.Int32(i * NTHREADS * 4) + wave * fx.Int32(64 * 4)
            lds_ptr = fx.buffer_ops.get_element_ptr(k_lds_ptr_base, byte_offset=fx.Index(lds_byte))
            fx.rocdl.buffer_load_to_lds(rk, lds_ptr, voff, size_bytes=4)

    # Issue the cooperative global loads for a KT-kv tile (V only when BUFK; V always stages through
    # VGPRs because the row-major path needs a transpose and col-V a contiguous repack).
    def load_kv_regs(kv0_):
        kc = []
        vc_words = []
        for p in fx.range_constexpr(NPASS):
            # --- K (kv-major, vectorized) -- skipped under async DMA (handled by dma_k_to_lds) ---
            if const_expr(not BUFK):
        ...
        for p in fx.range_constexpr(NPASS):
            ...
            def _do_store():
                ...
                if const_expr(VCOL):
                    ...
                    v_dst = vbuf_off + pass_dv[p] * fx.Int32(KT) + pass_kvg[p] * fx.Int32(16)
                    ...
                    for e in fx.range_constexpr(16):
                        fx.Vector.from_elements([fx.Vector(vc_i8)[e]], fx.Int8).store(
                            vt_lds, [fx.Index(vbuf_off + (v_d0 + fx.Int32(e)) * fx.Int32(KT) + kv_row)]
                        )
    ...
    def process_qtile(qtile):
        ...
        def compute_kt_tile(kv0_outer, kbuf, vbuf, m_run, l_run, o_acc, do_mask):
            ...
            for sub in fx.range_constexpr(NSUB):
                k_packs = []
                for ks in fx.range_constexpr(KSTEPS):
                    k_lds_elem = kbuf + (fx.Int32(sub * BN) + kv_local) * fx.Int32(HD) + fx.Int32(ks * 16) + half * fx.Int32(8)
            ...
            for sub in fx.range_constexpr(NSUB):
                ...
                for dt in fx.range_constexpr(DT):
                    ...
                    for s in fx.range_constexpr(2):
                        v_lds_elem = vbuf + d_col * fx.Int32(KT) + fx.Int32(sub * BN) + fx.Int32(s * 16) + half * fx.Int32(8)
        ...
        def loop_body(kt_iv, m_run, l_run, o_acc, do_mask):
            ...
            if const_expr(BUFK):
                # Issue async K DMA for the NEXT tile into the next buffer; it streams to LDS while
                # this tile's MFMAs run (the actual CK overlap). K never touches VGPRs.
                dma_k_to_lds(kv0_outer + fx.Int32(KT), kbuf_n)
            kc_w_next, vc_w_next = load_kv_regs(kv0_outer + fx.Int32(KT))  # OPT3 prefetch (V; K if !BUFK)
