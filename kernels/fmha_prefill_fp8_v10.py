...
"""FP8 causal FMHA prefill (paged, vec_k_col_v) for gfx942 — inline barrier variant.

Derived from ``fmha_prefill_fp8_v7.py``. This keeps the diagonal-pair q-tile
mapping and replaces the two hot-path workgroup barriers with explicit inline
assembly:

    s_waitcnt vmcnt(0) lgkmcnt(0)
    s_barrier

The goal is to avoid any extra conservative waits around ``gpu.barrier`` while
still waiting for the global-load-to-LDS store chain before other waves consume
the newly staged K/V tile.
"""
...
_alloc = SmemAllocator(None, arch="gfx942", global_sym_name="fmha_prefill_fp8_v10_smem")
_K_BYTES = BN * HD
_V_BYTES = HD * BN
_K_OFF = 0
...
def _inline_barrier(vmcnt=63, lgkmcnt=63):
    """Emit an exact waitcnt + s_barrier sequence through LLVM inline asm."""
    parts = []
    wait_fields = []
    if vmcnt < 63:
        wait_fields.append(f"vmcnt({vmcnt})")
    if lgkmcnt < 63:
        wait_fields.append(f"lgkmcnt({lgkmcnt})")
    if wait_fields:
        parts.append("s_waitcnt " + " ".join(wait_fields))
    parts.append("s_barrier")
    llvm.inline_asm(None, [], "\n".join(parts), "", has_side_effects=True)
...
def attn_kernel(
    ...
    def process_qtile(qtile):
        ...
        _inline_barrier(vmcnt=0, lgkmcnt=0)
