...
(D_INTER>512). The BM32 GEMM core is identical to ``mxfp4_moe_gemm1`` (same
...
Kept raw (shared via ``mxfp4_moe_common``): pointer helpers, the A-side
...
tile sizes are module constants (mirrors mxfp4_moe_gemm1). The D_INTER_REAL pad-tail
path is not supported (KIMI never pads).
"""
...
def _issue_a_load_lds_dt(aq_rsrc, saq, slot, kt, m_row, wave, lane, is_f8, KH_TILE_A, K_BYTES):
    ...
    Mirrors mxfp4_moe_gemm1.issue_a_load_lds; BM32 -> kSubBlocks=1."""
...
def compile_gemm2_a4w4_port(
    ...
    if (BM, epilog) != (32, "atomic"):
        raise AssertionError(
            f"mxfp4_moe_gemm2 supports only (BM=32, epilog='atomic'); "
