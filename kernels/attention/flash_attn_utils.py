...
def _factor_fp8_pv_head_dim(value_head_dim, segment_capacity, alignment):
    """Split a vector width into at most two independently staged segments."""
    if value_head_dim <= 0 or value_head_dim % alignment != 0:
        raise ValueError(f"FP8 P*V head dim must be a positive multiple of {alignment}, got {value_head_dim}")
    h1 = min(1 << (value_head_dim.bit_length() - 1), segment_capacity)
    h2 = value_head_dim - h1
    if h2 > segment_capacity:
        raise ValueError(f"FP8 P*V head dim {value_head_dim} exceeds two segment capacities of {segment_capacity}")
    return h1, h2
...
class DualwaveFp8KernelContext:
    ...
    def init_runtime_indices(self):
        ...
        if const_expr(traits.PAGED):
            self.stride_v_n_v = fx.Index(traits.NUM_HEADS_KV * traits.V_HEAD_DIM)
    ...
    def init_descriptors(self):
        ...
        if const_expr(traits.PAGED):
            ...
            k_page_elems = fx.Index(traits.PAGE_SIZE * traits.NUM_HEADS_KV * traits.HEAD_DIM)
            v_page_elems = fx.Index(traits.PAGE_SIZE * traits.NUM_HEADS_KV * traits.V_HEAD_DIM)
            ...
            self.k_page_layout = fx.make_layout(fx.Int32(k_page_elems), fx.Int32(1))
            self.v_page_layout = fx.make_layout(fx.Int32(v_page_elems), fx.Int32(1))
            ...
            self.page_base = self.batch_idx * fx.Index(self.block_table_stride)
            self.k_div = None
            self.v_div = None
        else:
            ...
            self.page_i32x2_atom = None
            self.page_v2i32 = None
            self.page_base = None
    ...
    def global_idx_o(self, token_idx, col):
        return (self.q_tok_base + token_idx) * self.stride_o_n_v + self.q_head_idx * self.traits.V_HEAD_DIM + col
    ...
    def load_page_id_pair(self, tile_start):
        ...
        page1 = rocdl.readfirstlane(T.i32, as_mlir_value(fx.Int32(pages[1])))
        return fx.Index(fx.Int32(page0)), fx.Index(fx.Int32(page1))
        ...
        if const_expr(self.traits.PAGED):
            ...
            for w in range_constexpr(8):
                p = buffer_ops.get_element_ptr(
                    base_ptr,
                    byte_offset=fx.Int32(byte_row + w * 4),
                    elem_type=T.i8,
                )
...
class DualwaveFp8QLoader(DualwaveFp8KernelContext):
    ...
    def load_all_wide(self, q_row_in_block):
        ...
        for ws in range_constexpr(traits.HEAD_DIM // 64):
            q_col = ws * 64 + d_base
            if const_expr(self.lds_q_base_ptr is None):
                ...
                byte_row = q_row_in_block * fx.Index(traits.HEAD_DIM) + q_col
...
class DualwaveFp8GemmHelper(DualwaveFp8KernelContext):
    ...
    def _load_v_fp8_segmented_chunk(self, buf_id, dc):
        traits = self.traits
        row_stride = traits.FP8_V_ROW_STRIDE
        tile_bytes = traits.V_HEAD_DIM * row_stride
        aligned_base = ((self.lds_vt_base_idx + fx.Index(127)) // fx.Index(128)) * fx.Index(128)
        tile_base = aligned_base + fx.Index(buf_id * tile_bytes)
        ...
        token_base = self.lane_div_32 * fx.Index(32)
        d_row = fx.Index(d_local) + self.lane_mod_32
        row_base = tile_base + fx.Index(segment_base) + d_row * fx.Index(row_stride) + token_base
        halves = []
    ...
    def _pv_step_fp8_segmented(self, step, v_p, buf_id, v_o):
        traits = self.traits
        if const_expr(step == 0 or (traits.PAGED and step == 1)):
            self._pv_segmented_p_cache = self._p_to_fp8_i32x8(v_p)

        chunks_per_step = traits.D_CHUNKS // 4
        extra_chunks = traits.D_CHUNKS % 4
        step_extra = 1 if step < extra_chunks else 0
        dc_start = step * chunks_per_step + min(step, extra_chunks)
        dc_end = dc_start + chunks_per_step + step_extra
        for dc in range_constexpr(dc_start, dc_end):
            v_op = self._load_v_fp8_segmented_chunk(buf_id, dc)
...
class DualwaveFp8KvGmemToLdsLoader(DualwaveFp8KernelContext):
    ...
    def load_k(self, tile_start, buf_id, page_id=None):
        ...
        if const_expr(traits.KV_VECTORIZED):
            ...
            if const_expr(traits.BN128 and traits.V_HEAD_DIM == 192):
                ...
                def _load_compact():
                    ...
                    if self.wave_id_uni < fx.Index(copy_waves):
                        _load_vectorized()
    ...
    def _stage_v_fp8_vectorized_segmented(self, tile_start, buf_id, page_id=None):
        if page_id is None:
            page_id = self.load_page_id(tile_start)
        prefix = self._load_v_fp8_vectorized_bankpad_source(tile_start, page_id=page_id)
        self._store_v_fp8_vectorized_bankpad(prefix, buf_id)
        if const_expr(self.traits.FP8_V_H2 > 0):
            self._stage_v_fp8_vectorized_segment(
                tile_start,
                buf_id,
                self.traits.FP8_V_H1,
                self.traits.FP8_V_H2,
                page_id,
            )
    ...
    def _stage_v_fp8_vectorized_segment(self, tile_start, buf_id, d_offset, segment_dim, page_id):
        ...
        def _stage_active():
            ...
            d_local = self.wave_id * fx.Index(16) + self.lane_in_warp % fx.Index(16)
            d_col = fx.Index(d_offset) + d_local
            src_elem = (
                self.kv_head_idx * token_groups16 * traits.V_HEAD_DIM * traits.KV_VEC_SIZE
                + n_group16 * traits.V_HEAD_DIM * traits.KV_VEC_SIZE
                + d_col * traits.KV_VEC_SIZE
            )
            reordered = self._permute_v_fp8_vectorized(self.buffer_load_fp8x16(src_div, src_elem))
            dest_group16 = (n_group16 % fx.Index(2)) * fx.Index(2) + n_group16 // fx.Index(2)
            aligned_base = ((self.lds_vt_base_idx + fx.Index(127)) // fx.Index(128)) * fx.Index(128)
            tile_bytes = traits.V_HEAD_DIM * traits.FP8_V_ROW_STRIDE
            dst_byte = (
                aligned_base
                + fx.Index(buf_id * tile_bytes)
                + fx.Index(d_offset * traits.FP8_V_ROW_STRIDE)
                + d_local * fx.Index(traits.FP8_V_ROW_STRIDE)
                + dest_group16 * fx.Index(16)
            )
            ptr = buffer_ops.get_element_ptr(
                self.lds_vt_base_ptr,
        ...
        scf_if_dispatch(flat < fx.Index(segment_dim * token_groups16), _stage_active)
    ...
    def _load_v_fp8_vectorized_bankpad_source(self, tile_start, page_id=None):
        ...
        prefix_dim = traits.FP8_V_H1 if traits.FP8_PV_SEGMENTED else traits.V_HEAD_DIM
        ...
        if const_expr(traits.BN128 and traits.FP8_PV_SEGMENTED):
            tail = self.buffer_load_fp8x16(src_div, src_elem + fx.Index(traits.FP8_V_H1 * traits.KV_VEC_SIZE))
    ...
    def _store_v_fp8_vectorized_bankpad(self, src_i32x4, buf_id):
        ...
        if const_expr(self.traits.BN128 and self.traits.FP8_PV_SEGMENTED):
            ...
            @flyc.jit
            def _store_tail():
                if self.wave_id_uni < fx.Index(self.traits.FP8_V_H2 // self.traits.KV_VEC_SIZE):
    ...
    def _store_v_fp8_vectorized_bankpad_segment(self, src_i32x4, buf_id, d_offset):
        ...
        n_group16 = self.lane_in_warp // fx.Index(16)
        d_col = fx.Index(d_offset) + self.wave_id * fx.Index(16) + self.lane_in_warp % fx.Index(16)
        reordered = self._permute_v_fp8_vectorized(src_i32x4)
        dest_group16 = (n_group16 % fx.Index(2)) * fx.Index(2) + n_group16 // fx.Index(2)
        if const_expr(traits.BN128 and traits.V_HEAD_DIM == 192):
            # XOR 16-byte groups instead of padding each row in the six-slot ring.
            dest_group16 = dest_group16 ^ ((d_col // fx.Index(4)) % fx.Index(4))
...
class DualwaveFp8KvLdsToVgprLoader(DualwaveFp8KernelContext):
        ...
        if const_expr(traits.KV_VECTORIZED):
            ...
            def _read_vec_key(key):
                ...
                for ws in range_constexpr(traits.HEAD_DIM // 64):
                    ...
                    off1 = (k_base + ((dg0 + 1) * traits.BLOCK_N + key_sigma) * traits.KV_VEC_SIZE) * traits.ELEM_BYTES
    ...
    def _load_v_fp8_vectorized_bankpad(self, buf_id):
        ...
        v_tile_bytes = traits.V_HEAD_DIM * row_stride
        aligned_base = ((self.lds_vt_base_idx + fx.Index(127)) // fx.Index(128)) * fx.Index(128)
        tile_base = aligned_base + fx.Index(buf_id * v_tile_bytes)
        token_base = self.lane_div_32 * fx.Index(32)
        packs = [[None] * traits.D_CHUNKS for _ in range(4)]
        for dc in range_constexpr(traits.D_CHUNKS):
            d_row = fx.Index(dc * traits.D_CHUNK) + self.lane_mod_32
            row_base = tile_base + d_row * fx.Index(row_stride)
            for half in range_constexpr(2):
                token_offset = token_base + fx.Index(half * 16)
                if const_expr(traits.BN128 and traits.V_HEAD_DIM == 192):
                    token_offset = token_offset ^ (((d_row // fx.Index(4)) % fx.Index(4)) * fx.Index(16))
                ptr = buffer_ops.get_element_ptr(
                    self.lds_vt_base_ptr,
                    byte_offset=fx.Int32(row_base + token_offset - self.lds_vt_base_idx),
                    elem_type=T.i8,
                )
...
class DualwaveFp8SoftmaxHelper(DualwaveFp8KernelContext):
    ...
    def lazy_rescale_o(self, v_o, m_row, l_row, m_tile_max, v_p):
        if const_expr(self.traits.PAGED):
            return self._lazy_rescale_o_paged(v_o, m_row, l_row, m_tile_max, v_p)
    ...
    def _lazy_rescale_o_paged(self, v_o, m_row, l_row, m_tile_max, v_p):
        traits = self.traits
...
class DualwaveFp8StoreHelper(DualwaveFp8KernelContext):
    ...
    @flyc.jit
    def store_final_o_if_valid(self, v_o, q_row):
        if q_row < self.seqlen_q_v:
            self.store_final_o(v_o, q_row)
