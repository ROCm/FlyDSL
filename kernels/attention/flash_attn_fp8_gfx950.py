...
def build_flash_attn_dualwave_swp_fp8_module(
    ...
    def flash_attn_dualwave_swp_fp8_gfx950_kernel(
        ...
        with _split_guard:
            # Prologue: load K tile split_t0 -> LDS buf0, stage Q to LDS, wait, and sync.
            ...
            # init_q_row sets q_row/q_row_i32/q_start_pos_i32 on ctx for the causal-mask helpers.
            ctx.init_q_row()
            q_row = ctx.q_row
    ...
    # 2-phase kernel (TWO_PHASE): matrix phase || vector phase with wave-group stagger.
    @flyc.kernel(known_block_size=[BLOCK_SIZE, 1, 1])
    def flash_attn_dualwave_swp_fp8_2phase_kernel(
        ...
        for j, loop_args in range(t0 + fx.Index(1), t_end, fx.Index(1), init=init_args):
            ...
            v_k = kv_lds_to_regs.load_k(buf_cur)
            v_v = kv_lds_to_regs.load_v(buf_oth)
            kv_gmem_to_lds.load_k((j + fx.Index(1)) * BN, buf_oth)
            kv_gmem_to_lds.load_v(j * BN, buf_cur)
