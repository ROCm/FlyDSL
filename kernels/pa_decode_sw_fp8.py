...
Supports:
  ...
  - Shared K/V block number: single BT lookup per block for large block sizes
"""
...
def compile_pa_decode_sw(
    ...
    # For large blocks: all partitions within one block share the same phys_block
    # so we carry phys_block as loop state and do BT lookup once per block
    _share_bt = _use_large_block and _cpb > 1
    ...
    def pa_decode_sw_kernel(
        ...
        # ════════════════════════════════════════════════════════════
        # Helper: BT lookup — single block table entry for a partition
        # For large blocks (bs=1024), all tokens in a partition share
        # the same physical block, so we look up once.
        # ════════════════════════════════════════════════════════════
        def _bt_lookup(part_idx_i32):
            """Look up physical block number for the first token of partition."""
            token0 = part_idx_i32 * c_cps
            kbi = token0 // arith.constant(_bs, type=T.i32)
            return buffer_ops.buffer_load(bt_rsrc, seq * c_bt + kbi,
                                          vec_width=1, dtype=T.i32)

        # ════════════════════════════════════════════════════════════
        # Helper: load K data for a partition → flat list of _N_K i64
        # phys_block: if provided, use this instead of per-token BT lookup
        # ════════════════════════════════════════════════════════════
        def _load_k_flat(part_idx_i32, phys_block=None):
        ...
        # phys_block: if provided, use for V BT lookup (shared with K)
        ...
        # For large blocks (_share_bt): also carry phys_block (i32)
        # ════════════════════════════════════════════════════════════
        def _unwrap(v):
            return v.ir_value() if hasattr(v, 'ir_value') else v

        if _share_bt:
            # ── Large block mode: shared BT lookup ──
            # One BT lookup per block, shared between K and V
            def _pack_state_bt(rmax, rsum, o0, o1, k_flat, pb):
                return [_unwrap(v) for v in [rmax, rsum, o0, o1] + k_flat + [pb]]

            def _unpack_state_bt(state):
                return (state[0], state[1], state[2], state[3],
                        list(state[4:4 + _N_K]), state[4 + _N_K])

            # Prologue: BT lookup + K load for first partition
            pb0 = _bt_lookup(part_start)
            k0_flat = _load_k_flat(part_start, phys_block=pb0)

            init_state = _pack_state_bt(
                NEG_INF, ZERO_F,
            ...
            for iv, state in range(_loop_start, _loop_stop, _loop_step,
                                   init=init_state):
                running_max, running_sum, out0, out1, k_flat, phys_block = _unpack_state_bt(state)
                ...
                # Prefetch next block: BT lookup + K load
                next_part = part_idx + c_cpb
                pb_next = _bt_lookup(next_part)
                k_next_flat = _load_k_flat(next_part, phys_block=pb_next)

                results = yield _pack_state_bt(running_max, running_sum, out0, out1,
                                               k_next_flat, pb_next)

            # Epilogue
            running_max, running_sum, out0, out1, k_last_flat, pb_last = _unpack_state_bt(results)
            ...
            # ── Small block mode: per-token BT lookup ──
