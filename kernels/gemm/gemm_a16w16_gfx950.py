...
def async_load_to_lds(
    ...
    for i in range_constexpr(load_iters):
        ...
        safe_global_outer_idx = fx.Int32(fx.arith.select(global_outer_idx < tile.outer_bound, global_outer_idx, 0))
        if const_expr(is_k_major):
...
def gemm_a16w16_gfx950_kernel(
    ...
    if const_expr(is_split_k):
        ...
        for i in range_constexpr(fx.size(frag_C.shape).unpack()):
            ...
            safe_global_n_idx = fx.Int32(fx.arith.select(global_n_idx < n, global_n_idx, 0))
            bias_val = bias_buf[safe_global_n_idx].to(fx.Float32)
            if const_expr(is_slice_k):
                is_first_k_slice = k_wave_idx == 0
                bias_val = fx.Float32(fx.arith.select(is_first_k_slice, bias_val, fx.Float32(0.0)))
            frag_C[i] = bias_val
    else:
        frag_C.fill(0.0)
...
def gemm_a16w16_hti_gfx950_kernel(
    ...
    if const_expr(is_split_k):
        ...
        for i in range_constexpr(fx.size(c00.shape).unpack()):
            ...
            safe_global_n0_idx = fx.Int32(fx.arith.select(global_n0_idx < n, global_n0_idx, 0))
            safe_global_n1_idx = fx.Int32(fx.arith.select(global_n1_idx < n, global_n1_idx, 0))
