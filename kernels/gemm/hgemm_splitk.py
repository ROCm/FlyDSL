...
def compile_hgemm_kernel(
    ...
    if B_TO_LDS:
        assert ASYNC_COPY
    C_ELEMS = BLOCK_K_WARPS * BLOCK_M * BLOCK_N
    SMEM_TOTAL_ELEMS = max(A_ELEMS + B_ELEMS, C_ELEMS)
    ...
    @fx.struct
    class SharedStorage:
        smem: fx.Array[elem_cls, SMEM_TOTAL_ELEMS, 16]
    ...
    def hgemm_kernel(
        ...
        def as_off(stage, row, col):
            return (fx.Int64(stage) * BLOCK_M + row) * BLOCK_K + col

        def bs_off(stage, row, col):
            return A_ELEMS + (fx.Int64(stage) * BLOCK_N + row) * BLOCK_K + col

        def cs_off(kw, row, col):
            return (fx.Int64(kw) * BLOCK_M + row) * BLOCK_N + col
        ...
        def sts_a(vecs, lds_stage):
            for i in range_constexpr(LDG_REG_A_COUNT):
                ...
                vecs_off = smem_ptr + as_off(lds_stage, m_local_idx, col_in_bytes // DTYPE_BYTES)
                fx.ptr_store(vecs[i], vecs_off)
        ...
        def ldg_sts_a_async(k_offset, lds_stage):
            for i in range_constexpr(LDG_REG_A_COUNT_AS):
                ...
                if const_expr(i == 0):
                    ...
                    lds_base = smem_i64 + fx.Int64(lds_byte_offset)
            ...
            for kk in range_constexpr(WARP_K_STEPS):
                ...
                if const_expr(initial_b_frags is None):
                    ...
                    for ii in range_constexpr(WARP_N_STEPS):
                        ...
                        vec = fx.ptr_load(
                            smem_ptr + bs_off(s, row, col_in_bytes // DTYPE_BYTES),
                            result_type=fx.Vector.make_type(WMMA_B_FRAG_VALUES * MFMA_PER_WARP_K, elem_cls),
                        ).ir_value()
                        b_frags[ii] = vec
                else:
                ...
                for ii in range_constexpr(WARP_M_STEPS):
                    ...
                    col_in_bytes = swizzle_xor16(row, col_in_bytes, k_blocks16)
                    vec = fx.ptr_load(
                        smem_ptr + as_off(s, row, col_in_bytes // DTYPE_BYTES),
                        result_type=fx.Vector.make_type(WMMA_A_FRAG_VALUES * MFMA_PER_WARP_K, elem_cls),
                    ).ir_value()
                    a_frags[ii] = vec
                rocdl.sched_barrier(0)
                for ii in range_constexpr(WARP_M_STEPS):
                    ...
                    for jj in range_constexpr(WARP_N_STEPS):
                        ...
                        if const_expr(MFMA_PER_WARP_K == 2):
                            ...
                            a0_i64 = vector.extract(as_ir_value(a_i64x2), static_position=[0], dynamic_position=[])
                            a1_i64 = vector.extract(as_ir_value(a_i64x2), static_position=[1], dynamic_position=[])
                            a_v0 = vector.bitcast(
                            ...
                            b0_i64 = vector.extract(as_ir_value(b_i64x2), static_position=[0], dynamic_position=[])
                            b1_i64 = vector.extract(as_ir_value(b_i64x2), static_position=[1], dynamic_position=[])
                            b_v0 = vector.bitcast(
        ...
        for ii in range_constexpr(WARP_M_STEPS):
            ...
            for jj in range_constexpr(WARP_N_STEPS):
                ...
                for kk in range_constexpr(WMMA_C_FRAG_VALUES):
                    ...
                    val = vector.extract(
                        as_ir_value(c_frags[ii * WARP_N_STEPS + jj]), static_position=[kk], dynamic_position=[]
                    )
                    val = val.truncf(dtype_)
                    if const_expr(IS_SLICE_K):
                        fx.memref_store(val, smem_view, fx.Int32(cs_off(wid_k, lds_m_idx, lds_n_idx)))
                    else:
                        fx.memref_store(val, smem_view, fx.Int32(cs_off(0, lds_m_idx, lds_n_idx)))
            ...
            for i in range_constexpr(LDG_REG_C_COUNT):
                ...
                with ir.InsertionPoint(cond_boundary_if.then_block):
                    ...
                    for vec_idx in range_constexpr(LDG_VEC_SIZE // 2):
                        e0 = vector.extract(as_ir_value(pk_val), static_position=[vec_idx * 2], dynamic_position=[])
                        e1 = vector.extract(as_ir_value(pk_val), static_position=[vec_idx * 2 + 1], dynamic_position=[])
