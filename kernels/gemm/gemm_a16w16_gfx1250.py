...
def _disable_unroll_on_enclosing_loop():
    ...
    if op.name != "scf.for":
        return
    anno = ir.Attribute.parse("#llvm.loop_annotation<unroll = <disable = true>, disableNonforced = true>")
...
def compile_gemm_a16w16(
    *,
    ...
    if variant not in ("bandwidth_bound", "compute_bound"):
        raise ValueError("variant must be 'bandwidth_bound' or 'compute_bound', " f"got {variant!r}")
    ...
    if sched_strategy is not None and sched_strategy not in (
        ...
        raise ValueError("sched_strategy must be None, 'max-ilp', or 'max-memory-clause', " f"got {sched_strategy!r}")
    ...
    if physical_kn:
        ...
        if (tile_n & (tile_n - 1)) != 0:
            raise ValueError(
                f"tile_n must be a power of 2 when physical_kn=True " f"(TDM pad_interval requirement), got {tile_n}"
            )
    if not physical_mk:
        if M == 0:
            raise ValueError(
        ...
        if (tile_m & (tile_m - 1)) != 0:
            raise ValueError(
                f"tile_m must be a power of 2 when physical_mk=False " f"(TDM pad_interval requirement), got {tile_m}"
            )
    ...
    stage_a_data_off = [unified_a_off + i * lds_a_data_bytes for i in range(num_buffers)]
    stage_b_data_off = [unified_b_off + i * lds_b_data_bytes for i in range(num_buffers)]
    ...
    def kernel_gemm_a16w16(
        ...
        addr_hi_a = vector.extract(_desc_a_init.dgroup0, static_position=[3], dynamic_position=[])
        addr_hi_w = vector.extract(_desc_b_init.dgroup0, static_position=[3], dynamic_position=[])
        addr_lo_init_a = vector.extract(_desc_a_init.dgroup0, static_position=[2], dynamic_position=[])
        addr_lo_init_w = vector.extract(_desc_b_init.dgroup0, static_position=[2], dynamic_position=[])
        a_lds_base_i32 = vector.extract(_desc_a_init.dgroup0, static_position=[1], dynamic_position=[])
        b_lds_base_i32 = vector.extract(_desc_b_init.dgroup0, static_position=[1], dynamic_position=[])
        ...
        adv_a_i32 = arith.constant((tile_k if physical_mk else tile_k * M) * elem_bytes, type=T.i32)
        adv_w_i32 = arith.constant((tile_k if not physical_kn else tile_k * N) * elem_bytes, type=T.i32)
        ...
        def issue_tdm_loads(buf_idx, lo_a, lo_w):
            ...
            dg0_a = vector.from_elements(T.vec(4, T.i32), [pred_const, a_addr, lo_a, addr_hi_a])
            tdm_ops.tensor_load_2d(tdm_ops.TDMDescriptor2D(dg0_a, dgroup1_a))
            dg0_w = vector.from_elements(T.vec(4, T.i32), [pred_const, b_addr, lo_w, addr_hi_w])
        ...
        def _precompute_a_lane_bases():
            ...
            k_lane_off = (lane_kgrp * arith.index(8) + lane8) * arith.index(lds_a_stride)
        ...
        def load_a_frags(buf_idx):
            ...
            if const_expr(isinstance(buf_idx, int)):
                ...
                slot_off_a = arith.index_cast(T.index, arith.muli(buf_idx, slot_stride_a_elems_i32))
            a_frags = []
            for ks in range_constexpr(k_wmma_steps):
                for wm in range_constexpr(wmma_m_rep):
                    a_frags.append(_load_a_frag(big_a_mem, a_lane_bases[wm] + slot_off_a, ks))
            return a_frags
        ...
        def load_b_frags(buf_idx):
            ...
            if const_expr(isinstance(buf_idx, int)):
                ...
                slot_off_b = arith.index_cast(T.index, arith.muli(buf_idx, slot_stride_b_elems_i32))
            b_frags = []
            for ks in range_constexpr(k_wmma_steps):
                for wn in range_constexpr(wmma_n_rep):
                    b_frags.append(_load_b_frag(big_b_mem, b_lane_bases[wn] + slot_off_b, ks))
            return b_frags
        ...
        def wmma_tile(accs_in, a_frags, b_frags):
            ...
            for ks in range_constexpr(k_wmma_steps):
                a_f = [a_frags[ks * wmma_m_rep + wm] for wm in range_constexpr(wmma_m_rep)]
                b_f = [b_frags[ks * wmma_n_rep + wn] for wn in range_constexpr(wmma_n_rep)]
                for wm in range_constexpr(wmma_m_rep):
                    for wn in range_constexpr(wmma_n_rep):
        ...
        def _load_one_a_frag(buf_i32, ks, wm):
            ...
            slot_off_a = arith.index_cast(T.index, arith.muli(buf_i32, slot_stride_a_elems_i32))
        ...
        def _load_one_b_frag(buf_i32, ks, wn):
            ...
            slot_off_b = arith.index_cast(T.index, arith.muli(buf_i32, slot_stride_b_elems_i32))
        ...
        def _wmma_rotate(accs_in, a_frags, b_frags, next_buf_i32):
            ...
            for ks in range_constexpr(k_wmma_steps):
                for wm in range_constexpr(wmma_m_rep):
                    ...
                    next_a[ks * wmma_m_rep + wm] = _load_one_a_frag(next_buf_i32, ks, wm)
                rocdl.sched_barrier(_SCHED_ALLOW_SALU)
                for wn in range_constexpr(wmma_n_rep):
                    next_b[ks * wmma_n_rep + wn] = _load_one_b_frag(next_buf_i32, ks, wn)
            ...
            for wm in range_constexpr(wmma_m_rep):
                for wn in range_constexpr(wmma_n_rep):
                    ...
                    if const_expr(add_bias):
                        ...
                        bv_lo = buffer_ops.buffer_load(bias_rsrc, col_base_i32, vec_width=4, dtype=_bias_elem)
                        bv_hi = buffer_ops.buffer_load(bias_rsrc, col_base_hi, vec_width=4, dtype=_bias_elem)

                        bias_elems = []
                        for i in range_constexpr(4):
                            b_val = vector.extract(bv_lo, static_position=[i], dynamic_position=[])
                            bias_elems.append(_widen_to_f32(b_val))
                        for i in range_constexpr(4):
                            b_val = vector.extract(bv_hi, static_position=[i], dynamic_position=[])
                            bias_elems.append(_widen_to_f32(b_val))
                    ...
                    if const_expr(_half_out):
                        if const_expr(split_k > 1):
                            ...
                            for pair in range_constexpr(4):
                                ...
                                byte_off = arith.index_cast(T.i32, c_off_bytes + arith.index(pair * 4))
                                rocdl.raw_ptr_buffer_atomic_fadd(pair_vec, y_rsrc, byte_off, zero_i32, zero_i32)
                        else:
                            h_vec = arith.trunc_f(T.vec(8, _out_elem), final_accs[idx])
                            i32_vec = vector.bitcast(T.vec(4, T.i32), h_vec)
                            c_off_bytes = (row * n_stride + col_base) * arith.index(elem_bytes_d)
                            buffer_ops.buffer_store(i32_vec, y_rsrc, c_off_bytes, offset_is_bytes=True)
                    else:
                        if const_expr(split_k > 1):
                            for half in range_constexpr(2):
                                ...
                                for vi in range_constexpr(4):
                                    ...
                                    rocdl.raw_ptr_buffer_atomic_fadd(val, y_rsrc, byte_off, zero_i32, zero_i32)
                        else:
                            for half in range_constexpr(2):
                                vals = [
                                    vector.extract(
            ...
            if const_expr(main_loop_unroll):
                ...
                if const_expr(num_pairs > 0):
                    ...
                    for pair_step, state in range(0, num_pairs, 1, init=init_state):
                        ...
                        p_accs, bank1, lx, lw = _run_tile(p_accs, b0a, b0b, lx, lw, cidx, lidx, do_prefetch=True)
                        b1a, b1b = bank1
                        cidx1 = arith.addi(cidx, one_i32)
                        lidx1 = arith.addi(lidx, one_i32)

                        # sub-1: tile T+1 from bank1, prefetch T+2 -> bank0 (yielded).
                        p_accs, bank0, lx, lw = _run_tile(p_accs, b1a, b1b, lx, lw, cidx1, lidx1, do_prefetch=True)
                        nb0a, nb0b = bank0
                ...
                for step, state in range(0, main_loop_iters, 1, init=init_state):
                    ...
                    p_accs, bank, lx, lw = _run_tile(p_accs, ca, cb, lx, lw, cidx, lidx, do_prefetch=True)
                    next_a, next_b = bank
                    cidx1 = arith.addi(cidx, one_i32)
                    lidx1 = arith.addi(lidx, one_i32)
        ...
        drain_count_d = num_buffers - 1 if main_loop_iters > 0 else min(num_buffers - 1, num_k_tiles)
        ...
        if kernarg_preload:
            ...
            for op in ctx.gpu_module_body.operations:
                if hasattr(op, "attributes") and op.OPERATION_NAME == "gpu.func":
                    ...
                    per_arg = [ir.DictAttr.get({"llvm.inreg": inreg_attr}) for _ in range(num_args)]
...
def gemm_a16w16(
    ...
    if y is not None and not _splitk_f32_accum:
        y_buf = y if N_stride == N else _alloc((M, N_stride), device=x.device, dtype=buf_dtype)
