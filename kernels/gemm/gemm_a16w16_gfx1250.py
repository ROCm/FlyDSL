...
def compile_gemm_a16w16(
    *,
    ...
    _req(tile_k & (tile_k - 1) == 0, f"tile_k must be a power of 2 for TDM, got {tile_k}")
    ...
    def kernel_gemm_a16w16(
        ...
        y_rsrc = fx.rocdl.get_buffer_rsrc(fx.rocdl.make_buffer_ptr(fx.get_iter(arg_y), y_nrec))
        if const_expr(add_bias):
            bias_rsrc = fx.rocdl.get_buffer_rsrc(fx.rocdl.make_buffer_ptr(fx.get_iter(arg_bias)))
        ...
        gA, atom_a = _mk_side(arg_x, a_base_off, a_tile_shape, a_outer_stride, a_pad_interval, LDS_PAD_A)
        if const_expr(physical_kn):
            b_base_off = split_k_base64 * fx.Int64(N) + blk_n64
        else:
            b_base_off = blk_n64 * fx.Int64(K) + split_k_base64
        gB, atom_b = _mk_side(arg_w, b_base_off, b_tile_shape, b_outer_stride, b_pad_interval, LDS_PAD_B)

        def _imm64(k_tile, mul):
            if const_expr(isinstance(k_tile, int)):
                return fx.Int64(k_tile * mul)
            as_index = arith.index_cast(T.index, arith.muli(k_tile, arith.constant(mul, type=T.i32)))
        ...
        def _lane_bases(warp_base, reps, lds_stride, transpose):
            if const_expr(not transpose):
                ...
                return [row_off + arith.index(rep * WMMA_M * lds_stride) + k_off for rep in range_constexpr(reps)]
            k_off = (lane_kgrp * arith.index(8) + lane16 % arith.index(8)) * arith.index(lds_stride)
            grp_off = (lane16 / arith.index(8)) * arith.index(8)
            return [k_off + warp_base + arith.index(rep * WMMA_M) + grp_off for rep in range_constexpr(reps)]

        # One spec per operand side; A and B differ only in these fields.
        MEM, BASE, BASES, STRIDE, TR, ELEMS, SSTRIDE, REPS, ATOM, GT, SHAPE, IMM = range(12)
        A_SIDE = (
            big_a_mem,
            big_a_base_idx,
        ...
        def _frag(sd, slot_off, ks, rep):
            ...
            for k_half in range_constexpr(2):
                if const_expr(sd[TR]):
                    off = sd[BASES][rep] + slot_off + arith.index((ks * WMMA_K + k_half * 16) * sd[STRIDE])
                    addr = arith.index_cast(T.i32, sd[BASE] + off * arith.index(elem_bytes))
                    ptr = llvm_dialect.inttoptr(ir.Type.parse("!llvm.ptr<3>"), arith.unwrap(addr))
                    halves.append(raw_rocdl.ds_load_tr16_b128(vec8_ty, ptr))
                else:
                    off = sd[BASES][rep] + slot_off + arith.index(ks * WMMA_K + k_half * 16)
                    halves.append(arith.unwrap(fx.ptr_load(fx.add_offset(sd[MEM], off), result_type=vec8_ty)))
            return fx.Vector(halves[0]).shuffle(fx.Vector(halves[1]), list(range(16))).ir_value()

        def load_frags(sd, buf_idx):
            off = _slot_off(buf_idx, sd[ELEMS], sd[SSTRIDE])
            return [
                _frag(sd, off, ks, rep) for ks in range_constexpr(k_wmma_steps) for rep in range_constexpr(sd[REPS])
            ]

        wmma_atom = fx.make_mma_atom(fx.rocdl.WMMA(WMMA_M, WMMA_N, WMMA_K, _fx_elem, fx.Float32))
        ...
        _out_num = fx.Float16 if out_dtype == "f16" else fx.BFloat16 if _half_out else None
        _bias_elem = elem_ty
            ...
            if const_expr(add_bias):
                ...
                for wn in range_constexpr(wmma_n_rep):
                    col_i32 = arith.index_cast(
                        T.i32,
                        blk_n + warp_n_base + arith.index(wn * WMMA_N) + lane_kgrp * arith.index(8),
                    )
                    elems = []
                    ...
                    bias_vecs.append(fx.Vector.from_elements(elems, fx.Float32).ir_value())
                ...
                for wn in range_constexpr(wmma_n_rep):
                    ...
                    if const_expr(activation is not None):
                        acc = fx.Vector.from_elements(
                            [
                                apply_activation_scalar(fx.Vector(acc)[i].ir_value(), activation)
                    ...
                    if const_expr(_half_out):
                        ...
                        if const_expr(split_k > 1):
                            for pair in range_constexpr(4):
                                ...
                                rocdl.raw_ptr_buffer_atomic_fadd(
                                    pair_vec,
                                    y_rsrc,
                                    arith.index_cast(T.i32, c_off_bytes + arith.index(pair * 4)),
                                    zero_i32,
                                    zero_i32,
                                )
                        else:
                        ...
                        for e in range_constexpr(8):
                            rocdl.raw_ptr_buffer_atomic_fadd(
                                ...
                                arith.index_cast(
                                    T.i32,
                                    (row * n_stride + col_base + arith.index(e)) * arith.index(4),
                                ),
                                zero_i32,
                                zero_i32,
                            )
                        ...
                        for half in range_constexpr(2):
                            vec4 = fx.Vector.from_elements(
                                [fx.Vector(acc)[half * 4 + vi].ir_value() for vi in range_constexpr(4)],
                                fx.Float32,
                            ).ir_value()
        ...
        def _run_tile(accs_, ca, cb, cidx, lidx):
            ...
            tdm_ops.tensor_wait(0 if num_buffers == 2 else (num_buffers - 2) * _TDMS_PER_TILE)
            gpu.barrier()
        ...
        if const_expr(main_loop_iters % TILES_PER_TRIP == 1):
            accs, cur_a, cur_b = _run_tile(accs, cur_a, cur_b, compute_idx_s, load_idx_s)
        ...
        for j in range_constexpr(drain_count_d):
            ...
            if const_expr(j < drain_count_d - 1):
                ...
                tdm_ops.tensor_wait(max(0, num_k_tiles - 1 - next_tile) * _TDMS_PER_TILE)
                gpu.barrier()
    ...
    def launch_gemm_a16w16(
        ...
        for op in ctx.gpu_module_body.operations:
            if hasattr(op, "attributes") and op.OPERATION_NAME == "gpu.func":
                ...
                if wpe >= 1:
                    op.attributes["rocdl.waves_per_eu"] = ir.IntegerAttr.get(ir.IntegerType.get_signless(32), wpe)
                if kernarg_preload:
        ...
        launcher.launch(grid=(gx, gy, split_k), block=(block_threads, 1, 1), stream=stream)
