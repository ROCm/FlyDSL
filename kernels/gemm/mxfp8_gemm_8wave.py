...
class MxMfma:
    ...
    def call(self, a, b, c, sa, sb, *, k_pack, set_prio=True):
        ...
        b_frags = [self._operand(b[j], self.b_words) for j in range_constexpr(self.n_tiles_b)]
...
def compile_mxfp8_gemm_8w(
    *,
    ...
    assert (BLOCK_M, BLOCK_N) in ((256, 256), (128, 512)), "supported tiles: 256x256 and 128x512"
    ...
    def kernel_gemm(
        ...
        if active:
            if const_expr(xcd_swizzle > 0):
                block_m, block_n = xcd_remap_pid(m_blocks, n_blocks, group_m=xcd_swizzle, allow_ragged=grouped)
            else:
                block_m, block_n = divmod(fx.block_idx.x, n_blocks)
            b_row = block_n * BLOCK_N
            if const_expr(grouped):
                expert = rocdl.readfirstlane(fx.Int32.ir_type, expert_ids[block_m // (expert_block_m // BLOCK_M)])
            ...
            gl_off_a = compute_global_swizzle(lane_id, wave_id, K, N_LDS_STEPS_A, preshuffled=False)
            if const_expr(b_dtype == "fp4"):
                gl_off_b = [
                    (wave_id + step * 8) * (b_k // 2 * 16) + lane_id % 16 * 16 + lane_id // 16 * 256
                    for step in range_constexpr(N_LDS_STEPS_B)
                ]
            else:
                gl_off_b = compute_global_swizzle(lane_id, wave_id, b_k, N_LDS_STEPS_B, preshuffled=b_preshuffled)
