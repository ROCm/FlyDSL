...
def compile_gemm_a8w8_splitk_reduce(*, split_k: int, out_dtype_str: str = "bf16", unroll: int = 0):
    unroll = unroll or max(1, 4 // split_k)
    if unroll & (unroll - 1):
        raise ValueError(f"unroll must be a power of two, got {unroll}")
    is_f16 = out_dtype_str == "f16"
    span = TILE * unroll
    name = format_kernel_name(f"gemm_a8w8_splitk_reduce_{out_dtype_str}_sk{split_k}_u{unroll}")
    ...
    def reduce_kernel(
        ...
        nbytes = is_tail.select(fx.Int32(rest) * fx.Int32(ELEM_BYTES), fx.Int32(span * ELEM_BYTES))
        row = fx.Int64(run) * fx.Int64(i32_ld) + off

        def _view(ptr_i64):
            pt = fx.PointerType.get(elem.ir_type, address_space=fx.AddressSpace.Global, alignment=ELEM_BYTES)
            view = fx.make_view(
        ...
        srcs = [[thr.partition_S(_part(pbuf, u)) for pbuf in pbufs] for u in range_constexpr(unroll)]
