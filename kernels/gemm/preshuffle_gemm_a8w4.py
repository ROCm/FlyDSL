...
def compile_preshuffle_gemm_a8(
    *,
    ...
    dsrd_preload: int = -1,
    dvmem_preload: int = -1,
    epilogue: str = "none",  # "none","bias","bias_relu","bias_silu","bias_gelu","svd","svd_bias"
    rank: int = 0,
    svd_use_mfma: bool = True,  # svd epilogue: MFMA-accumulate d@L2^T (fast) vs scalar loop
    xcd_swizzle: int = 0,
):
    ...
    Signature:  launch_fn(arg_c, arg_a, arg_b, arg_scale_a, arg_scale_b, arg_bias,
                          arg_d, arg_l2, M, N, stream)

    ``epilogue="svd"`` / ``"svd_bias"`` fuse the SVDQuant low-rank up-projection
    ``y += d @ L2^T`` into the store: ``arg_d`` is the pre-computed down-projection
    ``d = x @ L1^T`` (M x rank, out_dtype, row-major) and ``arg_l2`` is the low-rank
    factor L2 (N x rank, out_dtype, row-major). ``rank`` is the SVD rank (constexpr).
    "svd_bias" additionally fuses the bias add. Both are unsupported with
    use_cshuffle_epilog (fusion lives in body_row).
    ...
    if in_dtype not in ("fp8", "int8", "int4", "fp16", "bf16", "fp4", "fp6", "a8w4"):
        raise ValueError(
            "in_dtype must be one of " "('fp8','int8','int4','fp16','bf16','fp4','fp6','a8w4'), " f"got {in_dtype!r}"
        )
    ...
    is_mxscale_w4 = is_fp4_or_fp6 or is_a8w4
    is_int4 = in_dtype == "int4"
    is_int8 = (in_dtype == "int8") or is_int4
    is_f16 = in_dtype == "fp16"
    is_bf16 = in_dtype == "bf16"
    is_f16_or_bf16 = is_f16 or is_bf16
    elem_bytes = 1 if (in_dtype in ("fp8", "int8", "int4", "fp4", "fp6", "a8w4")) else 2
    a_elem_vec_pack = 2 if is_fp4 else 1
    b_elem_vec_pack = 2 if is_mxscale_w4 else 1
    ...
    _lane_group_bytes = 128 if is_a8w4 else 4 * a_per_lane_kpack_bytes  # 64 for fp4/fp8, 128 for fp6/a8w4
    ...
    # ---- Kernel function ----
    if epilogue not in ("none", "bias", "bias_relu", "bias_silu", "bias_gelu", "svd", "svd_bias"):
        raise ValueError(
            "epilogue must be one of ('none','bias','bias_relu','bias_silu',"
            f"'bias_gelu','svd','svd_bias'), got {epilogue!r}"
        )
    _has_epilogue = epilogue != "none"
    _has_svd = epilogue in ("svd", "svd_bias")
    # "svd_bias" fuses the low-rank up-proj AND a bias add.
    ...
    if _has_svd and int(rank) <= 0:
        raise ValueError(f"epilogue={epilogue!r} requires rank > 0, got {rank!r}")
    # MFMA-accumulate the low-rank up-proj d@L2^T into the same 16x16 f32 accumulators
    # as the main GEMM (tensor cores), instead of the per-output-element scalar dot loop.
    # Requires rank % 16 == 0 (one bf16 16x16x16 MFMA per 16-wide rank block); otherwise
    # fall back to the scalar loop. Only meaningful for bf16 out (the MFMA bf16 path).
    _svd_mfma = bool(_has_svd and svd_use_mfma and (int(rank) % 16 == 0))
    ...
    def kernel_gemm(
        ...
        # The kernel launches ceil(M/tile_m) M-tiles and processes each full tile;
        # when M is not a multiple of tile_m the last tile covers rows >= M. Those
        # ragged rows are NOT masked on the A read / C write, so with an unaligned
        # M they access past the M*K / M*N buffers and fault (read-only / unmapped
        # page). The tensors arrive as flat 1-D memrefs with dynamic size, so
        # max_size=False cannot recover the true byte extent from the memref type.
        # Instead pass the EXACT runtime byte size (M*K, M*N) as num_records so the
        # hardware buffer descriptor performs precise OOB checking: ragged-row reads
        # return 0 and ragged-row writes are dropped. This makes the kernel correct
        # for any M without caller-side padding.
        _out_elem_bytes = 2  # bf16 / fp16 output
        a_bytes = c_m * fx.Index(K * elem_bytes)
        c_bytes = c_m * c_n * fx.Index(_out_elem_bytes)
        ...
        _needs_per_token_scale = not is_f16_or_bf16 and not is_mxscale_w4
        ...
        # ---- SVDQuant low-rank up-proj buffers (for the "svd"/"svd_bias" epilogue) ----
        # arg_d: down-proj d = x @ L1^T, shape (M, rank), out_dtype row-major.
        # arg_l2: low-rank factor L2, shape (N, rank), out_dtype row-major.
        d_rsrc = None
        l2_rsrc = None
        if const_expr(_has_svd):
            _d_bytes = c_m * fx.Index(int(rank) * _out_elem_bytes)
            d_rsrc = buffer_ops.create_buffer_resource(arg_d, num_records_bytes=_d_bytes)
            l2_rsrc = buffer_ops.create_buffer_resource(arg_l2, max_size=True)
        ...
        if const_expr(is_mxscale_w4):
            ...
            for mi in range_constexpr(_m_repeat_packed_outer):
                mni_a = fx.Index(mi) + bx_m // fx.Index(_scale_mn_pack * 16)
            ...
            for ni in range_constexpr(_num_acc_n_packed_outer):
                mni_b = fx.Index(ni) + (by_n + n_tile_base) // fx.Index(_scale_mn_pack * 16)
        ...
        def compute_tile(
            ...
            *,
            ...
            if const_expr(use_mfma_scale_128):
                ...
                # fp4: cbsz=4 (E2M1); fp6: cbsz=2 (E2M3); a8w4/fp8: cbsz=0 (E4M3).
                # B is MXFP4 for the MX-scale W4 formats -> blgp=4.
                _fp4_cbsz = 2 if is_fp6 else (4 if is_fp4 else 0)
                _fp4_blgp = 4 if is_mxscale_w4 else 0
                _fp4_pack_M = 2 if is_mxscale_w4 else 1
                _fp4_pack_N = 2 if is_mxscale_w4 else 1
                _fp4_pack_K = 2 if is_mxscale_w4 else 1
                _quant_block_size = 32
                _K1 = K // (_quant_block_size * 4 * _fp4_pack_K) if is_mxscale_w4 else 1
                                ...
                                for ikxdl in range_constexpr(ikxdl_iters):
                                    ...
                                    for imxdl in range_constexpr(_fp4_pack_M):
                                        ...
                                        if const_expr(is_a8w4):
                                            a2, a3 = lds_load_packs_k64(curr_row_a_lds, col_base + 64, lds_buffer)
                                            a128 = pack_i64x4_to_i32x8(a0, a1, a2, a3)
                                        elif const_expr(is_fp6):
        ...
        def store_output(final_accs, scales):
            s_b_vals = []
            s_a_vecs = []
            if const_expr(not (is_f16_or_bf16 or is_mxscale_w4)):
                ...
                def write_row_to_lds(
                    *,
                    ...
                    for ni in range_constexpr(num_acc_n):
                        ...
                        if const_expr(is_int8):
                            val = fx.Float32(val)
                        if const_expr(is_f16_or_bf16 or is_mxscale_w4):
                            val_s = val
            ...
            # ---- Fused SVD up-proj via MFMA (fast path) --------------------------
            # y += d @ L2^T is a small bf16 GEMM (contraction = rank) into the SAME
            # 16x16 f32 accumulators as the main loop. d (M x rank) plays the A role,
            # L2 (N x rank) the B role. One mfma_f32_16x16x16_bf16 per 16-wide rank
            # block accumulates directly into final_accs, replacing the per-element
            # scalar dot loop in body_row. Fragment layout mirrors the main bf16 MFMA:
            #   A(d):  lane l -> d[m=bx_m+mi*16+(l%16),  r=kb*16+(l//16)*4+{0..3}]
            #   B(L2): lane l -> L2[n=by_n+ntb+ni*16+(l%16), r=kb*16+(l//16)*4+{0..3}]
            # both are 4 contiguous bf16 (row-major, stride=rank) -> one vec4 load.
            if const_expr(_svd_mfma):
                _svd_mfma_ty = Vec.make_type(4, fx.Float32)
                _rank_blocks = int(rank) // 16
                _r_lane = lane_div_16 * fx.Index(4)  # (l//16)*4 : this lane's 4 rank elems
                # A(d) fragments: one per (mi, kb). Row = bx_m + mi*16 + lane_mod_16.
                a_d = []
                for mi in range_constexpr(m_repeat):
                    _m_row = bx_m + (mi * 16) + lane_mod_16
                    _d_row_base = _m_row * fx.Index(int(rank))
                    kb_fr = []
                    for kb in range_constexpr(_rank_blocks):
                        _off = _d_row_base + (kb * 16) + _r_lane
                        v4 = buffer_ops.buffer_load(d_rsrc, _off, vec_width=4, dtype=_out_dtype())
                        kb_fr.append(Vec(v4).bitcast(fx.Int16))
                    a_d.append(kb_fr)
                # B(L2) fragments: one per (ni, kb). Col = by_n + n_tile_base + ni*16 + lane_mod_16.
                b_l2 = []
                for ni in range_constexpr(num_acc_n):
                    _n_col = by_n + n_tile_base + (ni * 16) + lane_mod_16
                    _l2_row_base = _n_col * fx.Index(int(rank))
                    kb_fr = []
                    for kb in range_constexpr(_rank_blocks):
                        _off = _l2_row_base + (kb * 16) + _r_lane
                        v4 = buffer_ops.buffer_load(l2_rsrc, _off, vec_width=4, dtype=_out_dtype())
                        kb_fr.append(Vec(v4).bitcast(fx.Int16))
                    b_l2.append(kb_fr)
                for mi in range_constexpr(m_repeat):
                    for ni in range_constexpr(num_acc_n):
                        ...
                        for kb in range_constexpr(_rank_blocks):
                            acc = rocdl.mfma_f32_16x16x16bf16_1k(
                                _svd_mfma_ty, [a_d[mi][kb], b_l2[ni][kb], acc, 0, 0, 0]
                            )
                        final_accs[acc_idx] = acc

            # SVD up-proj: preload L2[col, :] once per tile. col = col_base + ni*16
            # is row-independent, so loading it here (instead of per output row)
            # removes an m_repeat*4 factor of redundant global loads. l2_pre[ni][r].
            # (Scalar fallback: only when the MFMA path is disabled/ineligible.)
            l2_pre = None
            if const_expr(_has_svd and (not _svd_mfma) and l2_rsrc is not None):
                _col_base_pre = by_n + n_tile_base + lane_mod_16
                l2_pre = []
                for ni in range_constexpr(num_acc_n):
                    _l2_base = (_col_base_pre + (ni * 16)) * fx.Index(int(rank))
                    row_vals = []
                    for r in range_constexpr(int(rank)):
                        row_vals.append(
                            fx.Float32(
                                buffer_ops.buffer_load(l2_rsrc, _l2_base + fx.Index(r), vec_width=1, dtype=_out_dtype())
                            )
                        )
                    l2_pre.append(row_vals)
            ...
            def body_row(*, mi, ii, row_in_tile, row):
                ...
                # SVD up-proj: load d[row, :] once per row (row-constant across ni).
                # (Scalar fallback only; the MFMA path already folded d@L2^T into final_accs.)
                d_pre = None
                if const_expr(_has_svd and (not _svd_mfma) and d_rsrc is not None):
                    d_row_base = row * fx.Index(int(rank))
                    d_pre = []
                    for r in range_constexpr(int(rank)):
                        d_pre.append(
                            fx.Float32(
                                buffer_ops.buffer_load(
                                    d_rsrc, d_row_base + fx.Index(r), vec_width=1, dtype=_out_dtype()
                                )
                            )
                        )
                for ni in range_constexpr(num_acc_n):
                    ...
                    # ---- Fused epilogue: bias + SVD low-rank up-proj + activation ----
                    ...
                    # SVDQuant fusion: add the low-rank up-projection
                    #   y[row, col] += sum_r d[row, r] * L2[col, r]
                    # d is (M, rank) row-major, L2 is (N, rank) row-major; both in
                    # out_dtype. Accumulate the rank-`rank` dot in f32 before the store.
                    # Scalar fallback only; the MFMA path already added d@L2^T to final_accs.
                    if const_expr(_has_svd and (not _svd_mfma) and d_rsrc is not None):
                        svd_dot = fx.Float32(0.0)
                        _l2_row = l2_pre[ni]
                        for r in range_constexpr(int(rank)):
                            svd_dot = svd_dot + d_pre[r] * _l2_row[r]
                        val_s = val_s + svd_dot
        ...
        def hot_loop_scheduler():
            ...
            if const_expr(_is_gfx942):
                ...
                if const_expr(is_mxscale_w4 and tile_k != 128):
        ...
        def _build_pingpong_body(
            ...
            *,
            ...
            if const_expr(_fp4_tilek128):
                ...
                _sc_ping = load_fp4_scale_chunk(next_k2) if is_mxscale_w4 else None
                rocdl.sched_barrier(0)
                if const_expr(use_async_copy):
                    prefetch_a_to_lds(
                        next_k2,
            ...
            _sc_ping = load_fp4_scale_chunk(k_iv + fx.Index(tile_k)) if is_mxscale_w4 else None
            b_tile_ping = prefetch_b_tile(next_k1)
            accs_in, _ = compute_tile(
                accs_in,
                b_tile_pong_in,
            ...
            _sc_pong = load_fp4_scale_chunk(k_iv + (tile_k * 2)) if is_mxscale_w4 else None
            b_tile_pong_new = prefetch_b_tile(next_k2)
            accs_in, _ = compute_tile(
                accs_in,
                b_tile_ping,
        ...
        if const_expr(lds_stage == 2):
            ...
            fp4_scales0 = load_fp4_scale_chunk(fx.Index(0)) if is_mxscale_w4 else None
            ...
            if const_expr(_fp4_tilek128):
                if const_expr((num_tiles % 2) == 1):
                    ...
                    final_accs, scales = compute_tile(
                        ...
                        b_tile_pong_final,
                        lds_a_pong,
                        is_last_tile=not is_mxscale_w4,
                        a0_prefetch=a0pf,
                ...
                if const_expr(use_async_copy):
                    ...
                    a_regs_ping = prefetch_a_tile(last_k)
                _sc_last = load_fp4_scale_chunk(last_k) if is_mxscale_w4 else None
                accs, _ = compute_tile(
                    accs,
                    b_tile_pong_ep,
                    lds_a_pong,
            ...
            for iv, state in range(0, K - tile_k, tile_k, init=init_state):
                ...
                _fp4_sc = (
                    load_fp4_scales(iv // fx.Index(tile_k) * fx.Index(_fp4_scale_k_stride)) if is_mxscale_w4 else None
                )
            ...
            _last_fp4_sc = (
                load_fp4_scales(fx.Index((K - tile_k) // tile_k * _fp4_scale_k_stride)) if is_mxscale_w4 else None
            )
            final_accs, scales = compute_tile(
                accs_final,
                bt_final,
    ...
    def launch_gemm(
        ...
        launcher = kernel_gemm(arg_c, arg_a, arg_b, arg_scale_a, arg_scale_b, arg_bias, arg_d, arg_l2, i32_m, i32_n)
...
def compile_preshuffle_gemm_w4(
    *,
    ...
    epilogue: str = "none",
    rank: int = 0,
    svd_use_mfma: bool = True,
):
    """MXFP4 preshuffle GEMM - delegates to compile_preshuffle_gemm_a8 with fp4 config.

    ``epilogue``/``rank`` support the fused bias and SVDQuant low-rank up-proj
    epilogues (a4w4 = fp4 activation x fp4 weight); see compile_preshuffle_gemm_a8.
    ``svd_use_mfma`` accumulates the SVD up-proj via MFMA (tensor cores) vs the
    legacy scalar loop -- forwarded to both the a8w4 and fp4 delegates.
    """
    if a_dtype == "fp8":
        return compile_preshuffle_gemm_a8w4(
            M=M,
            N=N,
            K=K,
            tile_m=tile_m,
...
def compile_preshuffle_gemm_a8w4(
    *,
    M: int = 0,
    N: int = 0,
    K: int,
    ...
    or "svd"/"svd_bias" (SVDQuant low-rank up-proj, needs ``rank`` > 0). For the
    SVD epilogue, ``svd_use_mfma`` accumulates d@L2^T via MFMA (tensor cores);
    set False to use the legacy per-output-element scalar dot loop.
    """
    if str(get_hip_arch()) != "gfx950":
