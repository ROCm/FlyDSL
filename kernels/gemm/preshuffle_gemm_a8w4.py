...
def compile_preshuffle_gemm_a8(
    *,
    ...
    if in_dtype not in ("fp8", "int8", "int4", "fp16", "bf16", "fp4", "fp6", "a8w4"):
        raise ValueError(
            "in_dtype must be one of "
            "('fp8','int8','int4','fp16','bf16','fp4','fp6','a8w4'), "
            f"got {in_dtype!r}"
        )
    ...
    def kernel_gemm(
        ...
        # Only remap when XCD swizzle is enabled. Calling xcd_remap_bx_by with
        # xcd_swizzle=0 exercises a ternary whose branches disagree on i32 vs
        # index, which the >=0.2 compiler rejects (dynamic ifexp type mismatch).
