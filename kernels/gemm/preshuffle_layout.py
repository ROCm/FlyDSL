...
def make_preshuffle_dma_layouts(tile_m, tile_k_bytes, row_stride, num_threads, *, swizzle=True):
    ...
    # Keep row and K-block coordinates separate.  Flattening first and then
    # composing with a pitched row layout lowers to a div/mul pair for every
    # unrolled DMA round.  Basis strides expose (row, col) directly, so the
    # lowering only needs the lane decomposition once.
    ...
    source = fx.make_composed_layout(fx.make_layout((tile_m, tile_k_bytes), (row_stride, 1)), dma_to_coord)
    destination = fx.make_layout((16, waves, rounds), (1, 1024, num_threads * 16))
    return source, destination


def preshuffle_dma_lane_coord(lane, tile_k_bytes):
    """Coordinate for the lane mode returned by :func:`make_preshuffle_dma_layouts`."""
    blocks_per_row = tile_k_bytes // 16
