...
def compile_fp8fp4_gemm(
    *,
    ...
    warp_k_chunk = tile_k // k_warp
    warp_k_wmma_steps = warp_k_chunk // WMMA_K
    ...
    if use_tdm_store:
        ...
        _tdm_d_num_warps = m_warp * n_warp
    ...
    if k_warp > 1:
        ...
        kpar_f32_per_thread = _kpar_n_accs * ACC_VEC_SIZE
        kpar_reduce_f32_total = block_threads * kpar_f32_per_thread
        kpar_reduce_total_bytes = kpar_reduce_f32_total * 4
        if use_tdm_store:
    ...
    def kernel_mxscale_gemm(
        ...
        def compute_tile(
            ...
            # k_warp > 1: shift LDS bases to this warp's K-slice and use
            # warp_k_wmma_steps instead of k_wmma_steps for the loop bound.
            if const_expr(k_warp > 1):
                ...
                a_bases = [base + kpar_a_off for base in a_bases]
                b_bases = [base + kpar_b_off for base in b_bases]
                as_bases = [as_bases[0] + kpar_as_off]
                bs_bases = [bs_bases[0] + kpar_bs_off]
            _eff_k_steps = warp_k_wmma_steps if k_warp > 1 else k_wmma_steps
            ...
            if const_expr(_has_next):
                ...
                nxt_bs_buf, nxt_bs_bases = _precompute_scale_lane_bases(
            ...
            if const_expr(_has_pf):
                # Pre-fetched WMMAs, no next-tile prefetch (tail tiles)
                for ks in range_constexpr(_eff_k_steps):
                    pf_a_ks, pf_b_ks, pf_bs_ks, pf_as_ks = pf_all_ks[ks]
                    _mid_cb = mid_compute_callback if ks == 0 else None
                    _filler = emit_filler if ks == _eff_k_steps - 1 else None
            ...
            if const_expr(_eff_k_steps == 1):
                ...
                for ks in range_constexpr(_eff_k_steps - 1):
                    _mid_cb = mid_compute_callback if ks == 0 else None
                    current_accs, (prev_b, prev_bs, prev_as) = _a_streaming_compute(
                        current_accs, a_buf, a_bases, prev_b, prev_bs, prev_as, ks,
                ...
                current_accs = _a_streaming_compute(
                    ...
                    _eff_k_steps - 1, emit_filler=emit_filler,
                )
            return current_accs
        ...
        if const_expr(_use_lds_pf):
            ...
            def _issue_pf_all_ks(lds_a, lds_b, lds_bs, lds_as):
                """Issue all k-step ds_loads from a given LDS tile. Returns pf_all_ks list."""
                ...
                _bs_buf, _bs_bases = _precompute_scale_lane_bases(
                    lds_bs, warp_n_base, b_scale_load_rep, interleaved_scale_cols_b
                )
                pf = []
                for ks in range_constexpr(k_wmma_steps):
                    _b_ks, _bs_ks, _as_ks = _load_b_and_scales(
                        _b_buf, _b_bases, _bs_buf, _bs_bases, _as_buf, _as_bases, ks
                    )
                    _a_ks = [load_a_frag(_a_buf, _a_bases[wm], ks) for wm in range_constexpr(wmma_m_rep)]
