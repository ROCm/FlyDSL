...
def compile_mixed_moe_gemm2(
    *,
    ...
    # Decode & validate fused_p2p_scatter cfg (Stage 1 inline P2P path).
    # k = experts_per_token sets the f32 weight count per (pe, lid) P2P slot;
    # it must match combine-side shmem_comb_inp_wts=[npes][max_tok][k].
    ...
    if _fp2p_enabled:
        ...
        # epilogue and writes 1 byte/elem to remote shmem_comb_inp_tok
        # (matches the peer combine_no_stage1 data_type=float8_e4m3fn).
    ...
    if _tfs_enabled:
        # local counter: one i32 per (target_token_id in [0, mr), j in [0, topk))
        # device-global slot; cross-CTA atomic_add accumulates N-tile completions.
        # remote flag: one i32 per target_token_id, system-scope atomic_add,
        # spin-waited by combine kernel stage 3.
        _tfs_max_recv = int(_fp2p_npes) * int(_fp2p_max_tok)
        _tfs_topk = int(_fp2p_k)
        # num_n_tiles = ceil(model_dim / tile_n). fused path forces
        # use_cshuffle_epilog=True with model_dim_pad=0 (PR1 MVP), so this is
