"""FlyDSL fused recurrent gated delta rule update-forward kernel.

MVP scope:
- fixed-length sequence
- scalar beta only: beta[B*T, HV]
- reads initial recurrent states from a shared state pool via state indices
- always computes output and always writes back updated final states
"""
...
def build_fused_recurrent_gated_delta_rule_update_fwd_module(
    B,
    T_seq,
    H,
    HV,
    ...
    """Build fused recurrent gated delta rule update-forward kernel module.

    Args:
        B, T_seq, H, HV, K, V: compile-time dimensions
        N_STATE: number of rows in initial_state_source state pool
    ...
    class _FusedRecurrentGatedDeltaRuleUpdateFwd(flir.MlirModule):
        GPU_MODULE_NAME = f"fused_recurrent_gated_delta_rule_update_fwd_{dtype_str}"
        ...
        def fused_recurrent_gated_delta_rule_update_fwd_kernel(
            ...
            # Initialize recurrent state h from state pool.
            ...
            # Time loop: apply update-forward logic each step.
            for t in range_constexpr(T_seq):
                ...
                # Load g and beta (precomputed).
                g_val = memref.load(g, [arith.as_value(row_gb), arith.as_value(i_hv)])
                beta_val = memref.load(
                    beta_scalar, [arith.as_value(row_gb), arith.as_value(i_hv)]
                )
                if dtype_str != "f32":
                ...
                # v *= beta (scalar beta branch in MVP)
                for iv in range_constexpr(V):
                ...
                for ik in range_constexpr(K):
                    for iv in range_constexpr(V):
                        ...
                        old_h = memref.load(s_h, [arith.as_value(idx)])
                        prod = flir.arith.MulFOp(
                            arith.as_value(k_vals[ik]), arith.as_value(v_vals[iv]), fastmath=fm_fast
                        ).result
                ...
                for iv in range_constexpr(V):
                    ...
                    memref.store(
                        arith.as_value(out_val), o, [arith.as_value(row_vt), arith.index(iv)]
                    )

            # Write final recurrent state back to shared state pool.
            for ik in range_constexpr(K):
                for iv in range_constexpr(V):
                    crd = flir.make_coord(arith.index(ik), arith.index(iv))
                    idx = flir.crd2idx(crd, layout_h)
        ...
        def __call__(
            ...
            flir.gpu_ext.LaunchFuncOp(
                [self.GPU_MODULE_NAME, "fused_recurrent_gated_delta_rule_update_fwd_kernel"],
                grid_size=(gx, c1, c1),
                block_size=(bx, c1, c1),
                kernel_operands=[q, k, v, g, beta_scalar, initial_state_source, initial_state_indices, o],
            )

    return _FusedRecurrentGatedDeltaRuleUpdateFwd()
