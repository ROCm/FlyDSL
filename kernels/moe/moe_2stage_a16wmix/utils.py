...
def a16wmix_resolve_arch(arch=None):
    """Arch string for the rocm_arch argument of compile_gemm{1,2}_a16w4_port.

    The kernels derive every gfx942 (CDNA3) fallback from this string: the K=16 MFMA
    split, the lshr-16 bf16 pack, the fp4 byte-LUT decode, the LDS staging path and the
    global (non-buffer) atomic epilog. FLYDSL_A16WMIX_FORCE_K16=1 reports gfx942
    whatever the device is, so the CDNA3 path -- a strict ISA subset -- can be validated
    on a gfx950 box.
    """
    forced = os.environ.get("FLYDSL_A16WMIX_FORCE_K16", "0")
    if forced not in ("0", "", "false", "False"):
        return "gfx942"
    return str(arch if arch is not None else (get_rocm_arch() or ""))
...
def _int4_nibble_to_bf16x8(raw_i32, scale_f32, *, is_gfx942=False, old_pack=True):
    ...
    ``old_pack=True`` (contiguous {2j,2j+1} packing, mxfp4 fp4 upconvert order):
...
def make_a_loader(
    ...
    *,
    ...
    s_x_i32_flat = fx.make_view(fx.recast_iter(fx.Int32, lds_raw_ptr), fx.make_layout(num_i32, 1))
    ...
    def store_a_tile(base_k, slot=0):
        ...
        for i in range_constexpr(num_x_loads):
            ...
            col_sw = _a16w4_swizzle_xor16(x_row_local[i], col_bytes, fx.Int32(k_blocks16), enable=swizzle)
    ...
    def lds_load_a(mi, ku, slot=0):
        ...
        r = fx.make_rmem_tensor(fx.make_layout(4, 1), fx.Int32)
        fx.copy_atom_call(lds_atom, fx.slice(tiles, (None, byte_off // fx.Int32(16))), r)
    ...
    def load_b_raw_int4(base_k, n_blk, n_intra):
        ...
        for k0i in range_constexpr(_k0_count):
            ...
            for slot in range_constexpr(2):
                ...
                fx.copy(w_copy_atom64, fx.slice(w_tiles8, (None, idx // fx.Int32(8))), r)
