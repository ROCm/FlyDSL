...
def _atomic_bf16_epilog(
    ...
    if const_expr(not is_gfx942):
        atomic_bf16x2 = fx.make_copy_atom(fx.rocdl.BufferAtomicPkAdd(fx.BFloat16), fx.BFloat16)
    else:
...
def _reduce_bf16_epilog(
    ...
    for mr in range_constexpr(M_REPS):
        ...
        if token_id < i32_M:
            row_base_addr = (
                (token_id * fx.Int32(TOPK) + slot) * fx.Int32(N_OUT) + n_block_idx * fx.Int32(BN) + col_start
            )
            for s in range_constexpr(_s_count):
                idx0 = row_in_block * fx.Int32(BN) + col_start + fx.Int32(s * 64)
                pk = Vec(
...
def _gemm2_body_a16w4(
    ...
    *,
    ...
    cols = [b_loader.col(by_n, n_tile_base, fx.Int32(ni * 16)) for ni in range_constexpr(num_acc_n)]
...
def compile_gemm2_a16w4_port(
    ...
    *,
    ...
    assert (TILE_N // 4) >= 16, f"TILE_N//4 must be >= 16 (num_acc_n>=1), got TILE_N={TILE_N}"
    # Whole 16-wide groups per N-wave, else num_acc_n truncates and drops columns.
    assert TILE_N % 64 == 0, (
        f"TILE_N must be a multiple of 64 (else num_acc_n truncates and drops " f"columns), got TILE_N={TILE_N}"
    )
