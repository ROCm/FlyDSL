...
def moe_reduction_kernel(
    ...
    if const_expr(is_fp8):
        ...
        in_elem = fx.Float32 if dtype_str == "f32" else (fx.Float16 if dtype_str == "f16" else fx.BFloat16)
    ...
    def _view(elem, ptr_i64, ncols, nbytes):  # 2D [1, ncols] V# buffer descriptor
        ...
        view = fx.make_view(fx.inttoptr(pt, ptr_i64), fx.make_layout((1, ncols), (ncols, 1)))
    ...
    if const_expr(use_mask):
        ...
        tk_ptr = fx.inttoptr(i32pt, fx.Int64(ptrtoint(topk_ids)) + tok64 * fx.Int64(topk * 4))
    ...
    tile_mn, tv_layout = fx.make_layout_tv(fx.make_layout((1, BLOCK), (1, 1)), fx.make_layout((1, V), (1, 1)))
    ...
    if const_expr(is_fp8):
        ...
        sc_tile, sc_tv = fx.make_layout_tv(fx.make_layout((1, BLOCK), (1, 1)), fx.make_layout((1, 1), (1, 1)))
    ...
    def _reduce_tile():
        ...
        if const_expr(is_fp8):
            p_sc = thr_sc.partition_S(fx.slice(fx.zipped_divide(scbuf, sc_tile), (None, (0, tile))))
    ...
    if const_expr(model_dim % TILE != 0):
        if fx.Int32(tile) * fx.Int32(TILE) + fx.Int32(tid) * fx.Int32(V) < fx.Int32(model_dim):
            _reduce_tile()
    else:
        _reduce_tile()
