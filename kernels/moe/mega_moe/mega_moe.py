...
stage-1 (dispatch + GEMM1) -> ``MegaMoeStage1``; stage-2 (GEMM2 + EP combine) ->
``MegaMoeStage2`` + ``compile_fused_moe_gemm2_combine``. The expert-major dispatch
symmetric buffers are owned by ``FlyDSLDispatchCombineIntraNodeOp`` (``comb_op._gm``,
enabled by ``enable_group_major``); ``total_recv`` is shared with combine (no bridge).
Main entry ``MegaMoE.forward(x_bf16, wts, topk_ids)``: quantize -> stage1 -> stage2 -> bf16.
"""
...
__all__ = ["MegaMoE", "MegaMoeStage1", "MegaMoeStage2", "Stage1Output", "compile_fused_moe_gemm2_combine"]
...
class Stage1Output:
    """Stage-1 -> stage-2 handoff (field names match ``MegaMoeStage2.run`` params).

    All tensors are stable buffers allocated once in ``MegaMoeStage1.__init__``;
    each ``forward`` overwrites them in place.
    """
...
# mega_moe.py lives at kernels/moe/mega_moe/mega_moe.py; the tuning JSONs are at
# kernels/comm/mega_moe_tuning_config -> three parents up to the kernels/ root.
MEGA_TUNING_DIR = Path(__file__).resolve().parent.parent.parent / "comm" / "mega_moe_tuning_config"
# megastage1 weights are always w4(fp4); the JSON ``dtype`` is the ACTIVATION quant.
MEGA_QUANT_TO_DTYPE = {"a4w4": "fp4", "a8w4": "fp8_ocp"}


def detect_gpu_model_name(device_index=0):
...
def load_mega_tuning_rows(ep_size: int, gpu_model):
    ...
    if not MEGA_TUNING_DIR.is_dir():
        return ()
    suffix = f"_MegaStage1_ep{ep_size}.json"
    cands = [p for p in MEGA_TUNING_DIR.glob(f"flydsl_*{suffix}") if p.is_file()]
    if not cands:
        return ()
    cands.sort(key=lambda p: (1 if (gpu_model and gpu_model in p.name) else 0, p.name), reverse=True)
    try:
...
def mega_tuned_tile(model_dim, inter_dim, experts, topk, quant, mtpr, ep_size, gpu_model):
    ...
    if dtype is None:
        return None
    rows = load_mega_tuning_rows(int(ep_size), gpu_model)
    if not rows:
        return None
    ...
    buckets = {int(r["num_tokens"]): r for r in rows if match(r)}
    if not buckets:
        return None
...
@functools.lru_cache(maxsize=8)
def load_mega_gemm2_rows(ep_size: int, gpu_model):
...
def mega_gemm2_tuned_table(model_dim, inter_dim, experts, topk, ep_size, gpu_model, quant=None):
    ...
    rows = load_mega_gemm2_rows(int(ep_size), gpu_model)
    if not rows:
        return None
...
def compile_fused_moe_gemm2_combine(
    *,
    ...
    """
    # Plan B slot = dest_lid*k+s; the combine op sizes shmem_comb_inp_tok to
    # max(npes, topk)*mtpr so mt*k stays in-bounds for any topk.
    ...
    # fused_cfg[3]=enable_weights pinned False (in-kernel weight P2P is unreliable under fabric
    # saturation; combine_no_stage1 does it). Under fp8_cast GEMM2 stays bf16, store_pair casts.
    fused_cfg = (
        int(npes),
        int(rank),
...
class MegaMoeStage2:
    ...
    def __init__(
        ...
        *,
        ...
        if force_mode not in (None, "stage1_only"):
            raise ValueError(
                f"MegaMoeStage2: unsupported force_mode={force_mode!r} " "(only 'stage1_only' / None supported)"
            )
        self.comb_cfg = comb_cfg
        self.comb_op = comb_op
        # MegaMoE is the audited consumer, so opt into the default-off combine_no_stage1 gate.
        ...
        # Per-config JIT caches: (tile_m,tile_n,tile_k,persist_m,b_nt,xcd_swizzle) are baked
        # compile-time constants, so cache one kernel per config and pick by run_tokens at run().
        self._launch_by_tile = {}
        self._compiled_by_tile = {}
        # Optional {pow2_token: {tile_m,tile_n,tile_k,[xcd_swizzle],[persist_m],[b_nt]}}; legacy
        # 3-tuple values are normalised to dicts. None -> ctor defaults.
        if gemm2_tile_table:
            self._tile_table = {}
            for tok, v in dict(gemm2_tile_table).items():
                if isinstance(v, dict):
                    self._tile_table[int(tok)] = dict(v)
                else:  # legacy (tile_m, tile_n, tile_k)
                    tm, tn, tk = v
                    self._tile_table[int(tok)] = {"tile_m": int(tm), "tile_n": int(tn), "tile_k": int(tk)}
        else:
            self._tile_table = None
        # With a per-M table active, stage1 lays sorted_expert_ids at a FIXED block (32); pin
        # sort_block_m=32 so gemm2 reads at that block (tile_m only sub-tiles within it).
    ...
    def _select_tile(self, run_tokens):
        ...
        if not self._tile_table or run_tokens is None:
            return (
                self._clamp_tile_m(self.tile_m),
                self.tile_n,
                self.tile_k,
                self.persist_m,
                self.b_nt,
    ...
    def run(
        ...
        *,
        ...
        """Run the fused GEMM2+combine -> ``(out_tok, out_wts)``.

        ``wts_buf`` is the f32[max_recv*k] combine weight buffer (falls back to
        ``comb_op.shmem_disp_out_wts``); ``cur_tok`` is this rank's local token count.
        """
    ...
    def _ensure_launch_fn(self, cfg=None):
        """Build+cache the launch_fn for ``cfg`` (ctor config default), keyed by the
        (tile_m, tile_n, tile_k, persist_m, b_nt, xcd_swizzle) compile-time constants."""
        if cfg is None:
            cfg = (self.tile_m, self.tile_n, self.tile_k, self.persist_m, self.b_nt, self.xcd_swizzle)
    ...
    def _run_stage1_only(
        ...
        *,
        ...
        # Per-M config keys on the un-expanded forward token count (host-static; baked per graph).
        ...
        # tokens_in MUST be max_recv (drives the epilogue row_valid early-exit; a2.shape[0]
        # would let padding rows OOB-read addr_tis and corrupt combine state).
        ...
        # No upfront zero_() on shmem_comb_inp_tok: a local zero races peers' P2P writes to the
        # same HBM; safe because every (tok_id,j) is fully overwritten. FLYDSL_FUSED_ZERO_MODE=normal
        # force-enables the zero for debugging.
        ...
        # Weight source: shmem_disp_out_wts (f32[max_recv*k], wts[t*k+s]) unless wts_buf overrides.
        ...
        # Weights still go through combine_no_stage1's weight-only P2P scatter + Stage-3b accum
        # (negligible traffic), so the output matches baseline; fused only removes the token P2P.
...
def resolve_stage1_config(
    *,
    ...
    # Layout switched by buffer size: fixedslot below the 4GB wrap, compact above it. Also force
    # compact for epr>64: fixedslot P-static decode is a lane-per-expert readlane scan (<=64 lanes),
    # so epr>64 must use the compact path (loop-over-tiles, no lane limit).
    ...
    if int(unit_size) <= 0:
        tuned = mega_tuned_tile(
            model_dim, inter_dim, experts, topk, quant, tune_tokens, world_size, detect_gpu_model_name(rank)
        )
    ...
    # INTERLEAVE (g1u1) halves tile_n in the c_shuffle epilog; bump to 256 when inter_dim allows
    # (a tuned tn128 leaves the interleave epilog only 64 wide -> ~15% slower). SEPARATED keeps tuned.
    if gate_mode is GateMode.INTERLEAVE and int(tile_n) < 256 and int(inter_dim) % 256 == 0:
        tile_n = 256

    # XCD swizzle correctness guard: disable when the WGM group straddles XCD boundaries (would
    # corrupt output tiles) -- valid only when gx cleanly relates to the 8 XCDs and cu % gx == 0.
    xcd = 4
    if xcd > 0:
        cu_g = torch.cuda.get_device_properties(torch.cuda.current_device()).multi_processor_count
        # gx N-tile grid over the 2*inter output (SEPARATED = 2 B-tile streams, INTERLEAVE = 1).
...
class MegaMoeStage1:
    ...
    def __init__(
        ...
        *,
        ...
        self.slice_k = int(cfg.get("slice_k", 1))
        self._xcd = cfg["xcd"]
        self.b_nt = int(cfg.get("b_nt", 0))
        ...
        for pe in range(world_size):
            if rank == pe:
                self.mega = compile_fused_moe_gemm1(
                    ...
                    xcd_swizzle=self._xcd,
                    fuse_dispatch=scheme,
        ...
        if self.out_is_quant:
            sbm = max(32, self.unit_size)
            prows = ((self.max_blocks * sbm + 255) // 256) * 256
            pcols = (((inter_dim // 32) + 7) // 8) * 8
            self._osd = torch.zeros(prows * pcols + inter_dim, dtype=torch.uint8, device=self.dev)
        else:
        ...
        self.output = Stage1Output(
            a2=self._out,
            a2_scale=self._osd,
            sorted_token_ids=self._sti,
            sorted_expert_ids=self._se_atom,
            sorted_weights=self._wts_sorted,
            num_valid_ids=self._nv,
        ...
        if self.scheme == "fixedslot":
            ...
            if self.compact:
                ...
                tbl += [
                    ...
                    self._sti.data_ptr(),
                    self._se_atom.data_ptr(),  # 40 _sti | 41 _se_atom
                    self._wts_sorted.data_ptr(),
    ...
    def forward(self, x, wts, scales, topk_ids, stream=None) -> "Stage1Output":
        ...
        er_ptr = self.op.ll_count.data_ptr() if (self.scheme == "fixedslot" and not self.compact) else 0
        if self.compact:
            self.op.local_hist.zero_()
            self.op.local_cursor.zero_()
        a_mat = self._agv(self._rx)
        if self.compact:
            sorted_arg = self._trb
            se_arg = self._se
        else:
            sorted_arg = self._sti
            se_arg = self._se_atom
        wt_arg = self.op.wts_em
        self.mega(
            self._out,
            a_mat,
            self.w1,
    ...
    def __init__(
        ...
        *,
        ...
        if self.enable_fused_stage2:
            # fused stage-1 + fused stage-2: pin the GEMM2 ghost-gate to constant max_recv (identity
            # routing). Non-fused stage-1 keeps the op's real total_recv.
            if self.enable_fused_stage1:
            ...
            if gemm2_tile_table is None:
                try:
                    gemm2_tile_table = mega_gemm2_tuned_table(
                        ...
                        int(topk),
                        int(self.world_size),
                        detect_gpu_model_name(int(self.rank)),
                        quant=self.quant,
                    )
                    if int(self.rank) == 0:
                        print(
            ...
            # persist_m / xcd_swizzle are per-launch scalars orthogonal to the tile table; forward
            # them unconditionally. tile_m/n/k stay conditional (only an explicit gemm2_tile_m>0
            # overrides the table). gemm2_persist_m<=0 -> persistent mode (grid_y=cu_num).
            g2_kwargs = dict(
                comb_cfg=self.comb_cfg,
                comb_op=self.comb_op,
