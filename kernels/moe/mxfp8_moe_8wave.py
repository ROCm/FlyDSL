...
def _pack_fp8x8(values, scale):
    ...
    for word in range_constexpr(2):
        ...
        for half in range_constexpr(2):
            ...
            packed = Vec(
                rocdl.cvt_scalef32_pk_fp8_bf16(T.i16x2, packed.ir_value(), pair.ir_value(), scale.ir_value(), half)
            )
...
def _store_factory(
    *,
    ...
    def factory(C, rows, cols, idx, n_tiles_a, n_tiles_b, scratch):
        ...
        kp = (cols + 255) // 256 * 256
        records = fx.Int64(rows) * (kp + kp // 32) if fuse_quant else fx.Int64(rows) * cols * 2
        ...
        atom = fx.make_copy_atom(fx.rocdl.BufferCopy128b(), fx.Int8 if fuse_quant else fx.BFloat16)
        ...
        def store_scale(row, kg, exponent, valid):
            scale_index = ((row // 32 * (kp // 256) + kg // 8) * 64 + kg % 4 * 16 + row % 16) * 4
            scale_index += kg // 4 % 2 * 2 + row // 16 % 2
            scales[valid.select(rows * kp + scale_index, fx.Int32(-1))] = exponent.to(fx.Int8)
        ...
        def quant_store(base_row, base_col):
            ...
            values = [scratch_at(row, col + chunk * 8, 8).load() for chunk in range_constexpr(2)]
            ...
            for ti in range_constexpr(n_tiles_a):
                ...
                for tj in range_constexpr(n_tiles_b // 2 if activation else n_tiles_b):
                    ...
                    if const_expr(transpose):
                        ...
                        fx.make_view(ptr + offset, fx.make_layout(4, 1)).store(value.to(fx.BFloat16))
                    else:
                        ...
                        for i in range_constexpr(4):
                            ...
                            if const_expr(activation):
                                gate, linear = v, up[i]
                                if const_expr(activation_type == "swiglu" or swiglu_limit):
                            ...
                            scratch_at(row + i, scratch_col, 1).store(Vec.filled(1, v.to(fx.BFloat16), fx.BFloat16))
...
def compile_mxfp8_moe_sort_input_scale(*, K: int, topk: int):
    ...
    def kernel(
        ...
        kg_base = wave * 8
        if row < valid_rows[0]:
            src = fx.logical_divide(fx.rocdl.make_buffer_tensor(source, max_size=False), fx.make_layout(4, 1))
            ...
            for step in range_constexpr((groups + 31) // 32):
                kg = kg_base + step * 32
                if kg < groups:
                    index = valid.select((ta * groups + kg + lane // 32 * 4) // 4, fx.Int32(-1))
...
def compile_mxfp8_moe_quant(*, K: int, gather: bool, scatter_scale_topk: int = 0, dynamic_rows=False):
    ...
    def kernel(x: fx.Tensor, y: fx.Tensor, scale: fx.Tensor, row_map: fx.Tensor, rows: fx.Int32, valid_rows: fx.Tensor):
        inp = fx.logical_divide(fx.rocdl.make_buffer_tensor(x, max_size=False), fx.make_layout(8, 1))
        out = fx.logical_divide(fx.rocdl.make_buffer_tensor(y, max_size=False), fx.make_layout(16, 1))
        ...
        if row < limit:
            ...
            for slot in range_constexpr(max(1, scatter_scale_topk)):
                ...
                scales[(sr >= 0).select(scale_index, fx.Int32(-1))] = exponent.to(fx.Uint8)
            for half in range_constexpr(2):
                words = []
...
def compile_mxfp8_moe_reduce(*, N: int, topk: int, sorted_weights=False):
    ...
    def kernel(x: fx.Tensor, y: fx.Tensor, inverse: fx.Tensor, weights: fx.Tensor, tokens: fx.Int32):
        inp = fx.logical_divide(fx.rocdl.make_buffer_tensor(x, max_size=False), fx.make_layout(8, 1))
        out = fx.logical_divide(fx.rocdl.make_buffer_tensor(y, max_size=False), fx.make_layout(8, 1))
