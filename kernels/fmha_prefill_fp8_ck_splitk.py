...
NEGATIVE RESULT (measured 2026-06-14): split-K is a WASH — sq1024 6.0->6.2 (SPLITK2, +3%), sq2048 flat,
sq16384 REGRESSES (107->99). Per-split Q-reload+prologue+kv-fragmentation cancels the grid gain.
Kept as a documented negative result. Best kernel remains fmha_prefill_fp8_ck_hk5.
...
# SPLIT-K: fight CU-starvation at small seqlen by splitting the kv loop across SPLITK workgroups,
# multiplying the launch grid by SPLITK. Each split computes a PARTIAL flash-attention state
# (m_run, l_run, unnormalized o_acc) over its slice of KT-tiles, written to scratch; the torch
# harness combines them. SPLITK==1 is the exact hk5 fast path (normalized O written directly).
SPLITK = int(os.environ.get("FMHA_SPLITK", "4"))
DIAG = int(os.environ.get("FMHA_DIAG", "1")) != 0
# Diagonal-pair tiling complicates the per-split kv-range math; disable it whenever we split.
if SPLITK > 1:
    DIAG = False
...
_alloc = SmemAllocator(None, arch="gfx942", global_sym_name="fmha_prefill_fp8_ck_splitk_smem")
...
def attn_kernel(
    ...
    O: fx.Tensor,
    Ms: fx.Tensor,  # f32 [batch, nq, sq, SPLITK] partial per-row running max (split-K only)
    Ls: fx.Tensor,  # f32 [batch, nq, sq, SPLITK] partial per-row running sum (split-K only)
    Os: fx.Tensor,  # f32 [batch, nq, sq, SPLITK, HD] partial UNNORMALIZED o_acc (split-K only)
    sq: fx.Int32,
    sk: fx.Int32,
    nq: fx.Constexpr[int],
    nk: fx.Constexpr[int],
    ...
    # Grid mapping. Split-K: the low SPLITK factor of blk selects the kv-slice; the rest decodes the
    # (q-tile, head, batch) exactly as hk5. total grid = (nq * num_q_tiles_first) * SPLITK.
    # Diagonal-pair: ceil(num_q_tiles/2) CTAs per (batch,qhead); each does a tile and its causal
    # mirror (forced off when SPLITK>1). Non-diag: one CTA per q-tile (== 8wave baseline).
    if const_expr(SPLITK > 1):
        split = blk % fx.Int32(SPLITK)
        rest = blk // fx.Int32(SPLITK)
    else:
        split = fx.Int32(0)
        rest = blk
    ...
    def process_qtile(qtile):
        ...
        # SPLIT-K kv range: this split owns KT-tiles [kt_lo, kt_hi) of the runtime range [0, n_kt_rt).
        # Even ceil split of n_kt_rt across SPLITK workgroups, clamped to n_kt_rt. The phase-split
        # (unmasked interior then masked diagonal) is intersected with [kt_lo, kt_hi).
        if const_expr(SPLITK > 1):
            per = (n_kt_rt + fx.Int32(SPLITK - 1)) // fx.Int32(SPLITK)
            kt_lo = split * per
        ...
        # (kt_lo % 2 -- matches the loop_body buffer index). For SPLITK==1 kt_lo==0 (buffer 0),
        # exactly the hk5 behaviour.
        ...
        store_kv_to_lds(kc_w0, vc_w0, pf0_k, pf0_v)
        if const_expr(BUFK):
            _wait_vmem()
        fx.gpu.barrier()
        ...
        in_b = qrow < sq_i
        if const_expr(SPLITK > 1):
            # SPLIT-K epilogue: write PARTIAL state to scratch (do NOT normalize). m_run/l_run are
            # per-q-row scalars replicated across both lane-halves (shuffle_xor(32) in compute), so
            # write them once from half==0. o_acc is the UNNORMALIZED V^T@P accumulation.
            # Scratch layout: Ms/Ls [batch, nq, sq, SPLITK]; Os [batch, nq, sq, SPLITK, HD].
            ml_idx = ((batch * fx.Int32(nq) + qhead) * sq_i + qrow_safe) * fx.Int32(SPLITK) + split
            if in_b:
                if is_h0:
                ...
                o_row_base = (
                    ((batch * fx.Int32(nq) + qhead) * sq_i + qrow_safe) * fx.Int32(SPLITK) + split
                ) * fx.Int32(HD)
                for dt in fx.range_constexpr(DT):
...
def run_attn(
    ...
    Ms: fx.Tensor,  # partial scratch (always passed; for SPLITK==1 the caller passes small dummies)
    Ls: fx.Tensor,
    Os: fx.Tensor,
    stream: fx.Stream = fx.Stream(None),
):
    # NOTE: scratch tensors are REQUIRED positional args (no =None defaults — @flyc.jit can't derive
    # a cache key from None). The caller always passes real tensors; for SPLITK==1 they can be tiny.
...
# ---------------------------------------------------------------------------
# Self-contained correctness + perf harness for split-K (run ONE shape per process;
# the module-global SmemAllocator finalizes once). Use env FMHA_SPLITK to pick the split.
#   HIP_VISIBLE_DEVICES=2 FMHA_SPLITK=4 python3 kernels/fmha_prefill_fp8_ck_splitk.py 1024
# ---------------------------------------------------------------------------
def _run_one(nq, sq, sk, causal, b=1, nk=1, page_size=16, pscale=1.0, iters=30, warmup=5):
    ...
    grid = b * nq * ((sq + BM - 1) // BM) * SPLITK

    if SPLITK > 1:
        Ms = torch.full((b, nq, sq, SPLITK), -3.0e38, device="cuda", dtype=torch.float32)
        Ls = torch.zeros((b, nq, sq, SPLITK), device="cuda", dtype=torch.float32)
        Os = torch.zeros((b, nq, sq, SPLITK, HD), device="cuda", dtype=torch.float32)
        run_attn(*args, Og, sq, sk, nq, nk, page_size, c.k_page_stride, c.v_page_stride, sm, causal, grid, Ms, Ls, Os)
        torch.cuda.synchronize()
        # Flash-attention cross-split combine (per b, nq, sq row):
        #   global_m = max_s m_s ; scale_s = exp(m_s - global_m)
        #   l = sum_s scale_s * l_s ; O = sum_s scale_s * O_s_unnorm / l ; then * v_descale
        gm = Ms.amax(dim=-1, keepdim=True)  # [b,nq,sq,1]
        scale = torch.exp(Ms - gm)  # [b,nq,sq,SPLITK]
        l = (scale * Ls).sum(dim=-1)  # [b,nq,sq]
        O = (scale.unsqueeze(-1) * Os).sum(dim=-2)  # [b,nq,sq,HD]
        inv_l = torch.where(l < 1e-30, torch.zeros_like(l), 1.0 / l)
        vdesc = vd[:, 0].view(b, 1, 1, 1).to(O.device)  # nk==1: per-(b,head) v descale
        out = (O * inv_l.unsqueeze(-1) * vdesc).permute(0, 2, 1, 3).contiguous()  # [b,sq,nq,HD]
    else:
        # SPLITK==1: scratch unused, but the signature requires the args -> pass tiny dummies.
        dM = torch.zeros(1, device="cuda", dtype=torch.float32)
        run_attn(*args, Og, sq, sk, nq, nk, page_size, c.k_page_stride, c.v_page_stride, sm, causal, grid, dM, dM, dM)
        torch.cuda.synchronize()
        out = Og.float().cpu()
    ...
    # kernel-only timing (perf_counter; flydsl do_bench mis-times these)
    _dM = torch.zeros(1, device="cuda", dtype=torch.float32)

    def _launch():
        if SPLITK > 1:
            run_attn(*args, Og, sq, sk, nq, nk, page_size, c.k_page_stride, c.v_page_stride, sm, causal, grid, Ms, Ls, Os)
        else:
            run_attn(*args, Og, sq, sk, nq, nk, page_size, c.k_page_stride, c.v_page_stride, sm, causal, grid, _dM, _dM, _dM)
    ...
    print(
        f"SPLITK={SPLITK} sq{sq} sk{sk} nq{nq} c{causal} -> ERR {err:.4f} {'PASS' if ok else 'FAIL'} "
        f"| {ms:.3f} ms  {tflops:.1f} TF  (BM={BM} grid={grid})"
    )
