...
"""Fused intranode MoE stage1 (A8W4): **peer gather + MFMA GEMM + activation** in one ``@flyc.kernel``.

This file **vendors** ``compile_mixed_moe_gemm1`` / ``moe_gemm1`` from ``mixed_moe_gemm_2stage.py``
(lines 102–2694) without modifying the original module.  When ``intranode_peer_gather`` is enabled,
a **device-wide barrier** plus **MX-FP8 peer row gather** (``recv_meta`` + ``base_x_pe[]``) is
prepended to the vendored kernel body so ``arg_x`` staging is filled on-device before the existing
MFMA + epilogue path runs.  When disabled (single-PE microbench), that prologue is omitted at
compile time while keeping the same host ABI.

**Metadata-only dispatch** (separate grid / mori topology) remains in
``kernels.moe_metadata_dispatch_recv_meta.py``; call it before this fused kernel when routing
metadata is produced on device.
"""
...
def compile_fused_dispatch_gather_gemm1_a8w4(
    *,
    ...
    xcd_swizzle: int = 0,
    intranode_peer_gather: bool = True,
):
    """Compile stage1 kernel (gate+up with silu/swiglu).

    When ``intranode_peer_gather`` is True (default), the kernel prepends MX-FP8 **recv_meta**
    peer gather into ``addr_peer_x_staging`` (same bytes as ``arg_x``) on the GEMM1 CTA grid,
    then a device-wide i32 arrival counter at ``addr_peer_gather_grid_bar`` before MFMA
    (each GEMM CTA increments once after its recv-slot loop; spin waits for ``num_ctas``).

    Set ``intranode_peer_gather=False`` only for **single-PE / no cross-rank X exchange** runs
    (e.g. ``torchrun --nproc_per_node=1`` microbench): the launcher ABI is unchanged, but the
    gather+grid barrier prologue is compiled out for lower latency.

    The host launcher always passes the six trailing recv_meta / P2P / barrier arguments.
    ...
    if intranode_peer_gather and (int(model_dim) % 4 != 0):
        raise ValueError(
            "intranode_peer_gather requires model_dim % 4 == 0 (4-byte row chunks), "
    ...
    if True:
        ...
        def moe_fused_stage1_a8w4(
            ...
            if const_expr(intranode_peer_gather):
                # MX-FP8 peer row gather into X staging (vec4, grid-stride recv_slot, one dst rsrc).
                # Host must zero the i32 at addr_peer_gather_grid_bar before each launch.
                # All threads redundantly decode recv_meta (same addresses → L1 broadcast per warp).
                _tid_i32 = arith.index_cast(T.i32, gpu.thread_id("x"))
                ...
                _ff_i32 = arith.constant(0xFF, type=T.i32)
                _md_i32 = arith.constant(int(model_dim), type=T.i32)

                total_recv_i32 = arith.index_cast(
                    T.i32, i32_peer_recv_total.ir_value()
                )
                npes_i32 = arith.index_cast(T.i32, i32_peer_npes.ir_value())

                gdx_i32 = gpu.grid_dim.x.ir_value()
                gdy_i32 = gpu.grid_dim.y.ir_value()
                gdz_i32 = gpu.grid_dim.z.ir_value()
                bxi_i32 = arith.index_cast(T.i32, gpu.block_id("x"))
                byi_i32 = arith.index_cast(T.i32, gpu.block_id("y"))
                bzi_i32 = arith.index_cast(T.i32, gpu.block_id("z"))
                gxy = arith.muli(gdx_i32, gdy_i32)
                num_ctas_i32 = arith.muli(gxy, gdz_i32)
                cta_linear_i32 = arith.addi(
                    arith.addi(
                        arith.muli(bzi_i32, gxy),
                        arith.muli(byi_i32, gdx_i32),
                    ),
                    bxi_i32,
                )

                r_meta_pg = create_buffer_resource_from_addr(
                    _lv_unwrap(addr_recv_meta_flat)
                )
                r_p2p_pg = create_buffer_resource_from_addr(
                    _lv_unwrap(addr_p2p_base_x)
                )
                out_base_pg = _lv_unwrap(addr_peer_x_staging)
                r_out_staging_pg = create_buffer_resource_from_addr(out_base_pg)

                _zero_i8 = arith.constant(0, type=T.i8)
                _chunks_i32_pg = arith.constant(int(_chunks_pg), type=T.i32)

                recv_init = cta_linear_i32
                w_recv = scf.WhileOp([T.i32], [recv_init])
                w_recv_b = ir.Block.create_at_start(w_recv.before, [T.i32])
                w_recv_a = ir.Block.create_at_start(w_recv.after, [T.i32])
                with ir.InsertionPoint(w_recv_b):
                    recv_slot_w = w_recv_b.arguments[0]
                    cont_recv = arith.cmpi(
                        CmpIPredicate.ult, recv_slot_w, total_recv_i32
                    )
                    scf.ConditionOp(_lv_unwrap(cont_recv), [recv_slot_w])
                with ir.InsertionPoint(w_recv_a):
                    recv_slot_w = w_recv_a.arguments[0]
                    meta_off_w = arith.muli(recv_slot_w, _c8_i32)
                    hdr_w = _dc_buffer_load(
                        r_meta_pg, meta_off_w + _c0_i32, vec_width=1, dtype=T.i32
                    )
                    src_tok_w = _dc_buffer_load(
                        r_meta_pg, meta_off_w + _c1_i32, vec_width=1, dtype=T.i32
                    )
                    dst_flat_w = _dc_buffer_load(
                        r_meta_pg, meta_off_w + _c5_i32, vec_width=1, dtype=T.i32
                    )
                    src_pe_w = arith.andi(hdr_w, _ff_i32)
                    pe_ok_w = arith.cmpi(
                        CmpIPredicate.ult, src_pe_w, npes_i32
                    )
                    p2p_base_w = _dc_buffer_load(
                        r_p2p_pg, src_pe_w, vec_width=1, dtype=T.i64
                    )
                    row_off_w = arith.muli(
                        arith.zext_i64(src_tok_w),
                        arith.zext_i64(_md_i32),
                    )
                    src_row_base_w = arith.select(
                        pe_ok_w,
                        arith.addi(p2p_base_w, row_off_w),
                        out_base_pg,
                    )
                    dst_row_b0_i32 = arith.muli(dst_flat_w, _md_i32)
                    r_src_row_pg = create_buffer_resource_from_addr(src_row_base_w)

                    for _ii_pg in range_constexpr(int(_iters_pg)):
                        _step_pg = arith.constant(
                            int(_ii_pg * int(total_threads)), type=T.i32
                        )
                        chunk_id_pg = arith.addi(_tid_i32, _step_pg)
                        _in_chunk_pg = arith.cmpi(
                            CmpIPredicate.ult, chunk_id_pg, _chunks_i32_pg
                        )
                        _if_chunk_pg = scf.IfOp(
                            _lv_unwrap(_in_chunk_pg),
                            results_=[],
                            has_else=True,
                        )
                        with ir.InsertionPoint(_if_chunk_pg.then_block):
                            byte_off_row_pg = arith.muli(
                                chunk_id_pg, _c4_i32
                            )
                            dst_abs_byte_pg = arith.addi(
                                dst_row_b0_i32, byte_off_row_pg
                            )
                            # Scalar i8 loads/stores (unrolled): vec4 buffer loads fail LLVM legalize.
                            _boff0 = arith.addi(byte_off_row_pg, arith.constant(0, type=T.i32))
                            _boff1 = arith.addi(byte_off_row_pg, arith.constant(1, type=T.i32))
                            _boff2 = arith.addi(byte_off_row_pg, arith.constant(2, type=T.i32))
                            _boff3 = arith.addi(byte_off_row_pg, arith.constant(3, type=T.i32))
                            _doff0 = arith.addi(dst_abs_byte_pg, arith.constant(0, type=T.i32))
                            _doff1 = arith.addi(dst_abs_byte_pg, arith.constant(1, type=T.i32))
                            _doff2 = arith.addi(dst_abs_byte_pg, arith.constant(2, type=T.i32))
                            _doff3 = arith.addi(dst_abs_byte_pg, arith.constant(3, type=T.i32))
                            _b0 = _dc_buffer_load(
                                r_src_row_pg, _boff0, vec_width=1, dtype=T.i8
                            )
                            _b1 = _dc_buffer_load(
                                r_src_row_pg, _boff1, vec_width=1, dtype=T.i8
                            )
                            _b2 = _dc_buffer_load(
                                r_src_row_pg, _boff2, vec_width=1, dtype=T.i8
                            )
                            _b3 = _dc_buffer_load(
                                r_src_row_pg, _boff3, vec_width=1, dtype=T.i8
                            )
                            buffer_ops.buffer_store(
                                arith.select(pe_ok_w, _b0, _zero_i8),
                                r_out_staging_pg,
                                _doff0,
                                offset_is_bytes=True,
                            )
                            buffer_ops.buffer_store(
                                arith.select(pe_ok_w, _b1, _zero_i8),
                                r_out_staging_pg,
                                _doff1,
                                offset_is_bytes=True,
                            )
                            buffer_ops.buffer_store(
                                arith.select(pe_ok_w, _b2, _zero_i8),
                                r_out_staging_pg,
                                _doff2,
                                offset_is_bytes=True,
                            )
                            buffer_ops.buffer_store(
                                arith.select(pe_ok_w, _b3, _zero_i8),
                                r_out_staging_pg,
                                _doff3,
                                offset_is_bytes=True,
                            )
                            scf.YieldOp([])
                        with ir.InsertionPoint(_if_chunk_pg.else_block):
                            scf.YieldOp([])
                    gpu.barrier()
                    next_recv_w = arith.addi(recv_slot_w, num_ctas_i32)
                    scf.YieldOp([next_recv_w])

                rocdl.s_waitcnt(0)
                _if_t0_at = scf.IfOp(
                    _lv_unwrap(
                        arith.cmpi(CmpIPredicate.eq, _tid_i32, _c0_i32)
                    ),
                    results_=[],
                    has_else=True,
                )
                with ir.InsertionPoint(_if_t0_at.then_block):
                    atomic_add_global_acq_rel(
                        addr_peer_gather_grid_bar,
                        arith.constant(1, type=T.i32),
                    )
                    scf.YieldOp([])
                with ir.InsertionPoint(_if_t0_at.else_block):
                    scf.YieldOp([])
                gpu.barrier()

                spin_init = _c0_i32
                w_spin = scf.WhileOp([T.i32], [spin_init])
                sb = ir.Block.create_at_start(w_spin.before, [T.i32])
                sa = ir.Block.create_at_start(w_spin.after, [T.i32])
                with ir.InsertionPoint(sb):
                    _spin_carry = sb.arguments[0]
                    cur_ar = load_i32_global_acquire(addr_peer_gather_grid_bar)
                    # One atomic per CTA after the recv-slot while (thread 0 only); total arrivals
                    # == num_ctas (full GEMM grid), not total_recv.
                    need_m = arith.cmpi(
                        CmpIPredicate.slt, cur_ar, num_ctas_i32
                    )
                    scf.ConditionOp(_lv_unwrap(need_m), [_spin_carry])
                with ir.InsertionPoint(sa):
                    rocdl.s_waitcnt(0)
                    scf.YieldOp([_c0_i32])
                gpu.barrier()
