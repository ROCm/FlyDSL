    ...
    def __init__(self, *, m_repeat, num_acc_n, a_dtype="fp8"):
        ...
        self._atoms = {
            (osa, osb): fx.make_mma_atom(
                fx.rocdl.cdna4.MFMA_Scale(16, 16, 128, elem_a, fx.Float4E2M1FN, opsel_a=osa, opsel_b=osb)
            )
            for osa in range(4)
            for osb in range(4)
        }
    ...
    def store(self, acc, tile_i32, tile_row_base_i32, n_tile_base_i32):
        ...
        if self._out_base_addr is not None:
            out_rsrc = _buffer_ops.create_buffer_resource_from_addr(
                self._out_base_addr
                + fx.Int64(tile_i32)
                * fx.Int64(self._sort_block_m * self._out_row_bytes),
        ...
        for mr in range_constexpr(m_reps):
            ...
            for nr in range_constexpr(n_reps):
                ...
                _buffer_ops.buffer_store(payload, out_rsrc, out_byte, offset_is_bytes=True)
