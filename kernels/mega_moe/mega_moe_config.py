...
def _fixed_dispatch_cu(bucket: int) -> int:
    ...
    return min(
        MAX_DISPATCH_CU,
        _FIXED_DISPATCH_CU_SCALE * (bucket.bit_length() + _FIXED_DISPATCH_CU_BIT_OFFSET),
    )
...
def _select_fixed_stage1(bucket: int, experts_per_rank: int) -> Stage1Config:
    ...
    return Stage1Config(
        ...
        waves_per_eu_hint=WAVES_PER_EU_LOW if bucket == TokenBucket.BS16 else WAVES_PER_EU_DEFAULT,
    )
...
def _normalize_config_request(
    ...
    if p2p_quant == P2P_QUANT_AUTO:
        ...
        use_fp8 = mtpr >= P2P_FP8_MIN_MTPR if a_dtype == ACTIVATION_FP4 else mtpr > P2P_FP8_MIN_MTPR
...
def _apply_config_patch(config: MegaMoEConfig, patch: ConfigPatch) -> MegaMoEConfig:
    ...
    return replace(
        config,
        stage1=replace(config.stage1, **stage1_values) if stage1_values else config.stage1,
        stage2=replace(config.stage2, **stage2_values) if stage2_values else config.stage2,
    )
...
def _select_tuning_patches(
    ...
    if context.p2p_quant == P2P_QUANT_FP8_BLOCKWISE:
        if patch := _p2p_fp8_patch(context.bucket):
            patches.append(patch)

    if context.capacity_mode == CapacityMode.FIXED_SLOT:
        if patch := _fixed_slot_token_patch(context.a_dtype, context.tokens):
            patches.append(patch)
    ...
    if context.p2p_quant == P2P_QUANT_FP8_BLOCKWISE:
        if patch := _A4_FP8_P2P_BUCKET_PATCHES.get(context.bucket):
            patches.append(patch)

    if context.capacity_mode == CapacityMode.LARGE_COMPACT and context.mtpr == FIXED_LARGE_CAPACITY_MTPR:
...
def _tuning_token_key(tokens: int) -> int:
    if tokens in EXACT_TUNING_TOKENS:
        return tokens
    return WIDE_BATCH_MIN_TOKENS if tokens >= WIDE_BATCH_MIN_TOKENS else NO_EXACT_TOKEN_KEY
