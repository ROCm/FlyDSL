...
from kernels.mega_moe.group_gemm.atoms import (
    _PACK, AS2RLoader, AScaleLoader, ATileLoader, BScaleLoader, BWeightLoader,
    MfmaScaleGU, SiluQuantEpilogue, TileScheduler, wait_lds_barrier,
)
...
def compile_moe_group_gemm1(
    *,
    model_dim: int, inter_dim: int, experts: int, topk: int = 1,
    tile_m: int = 64, tile_n: int = 256, tile_k: int = 256, sort_block_m: int = 64,
    sched_nmajor: bool = False, pipe_weights: bool = True, mfma_amajor: bool = False, swizzle_a: bool = False,
    a_dtype: str = "fp8", out_dtype: str = "fp8", debug_gate: bool = False, use_xcd: bool = True,
    num_waves: int = 4, num_cu: int = 256, grid_mult: int = 2, wgm: int = 1,
):
    is_f8_a = a_dtype == "fp8"
    ...
    def kernel(
        ...
        a_gather = ATileLoader(x_rsrc=x_rsrc, row_bytes=a_row_bytes, sort_block_m=sort_block_m,
        ...
        a_scale = AScaleLoader(scale_rsrc=sx_rsrc, m_repeat=M_REPEAT, model_dim=model_dim,
        ...
        epi = SiluQuantEpilogue(out_rsrc=out_rsrc, out_scale_rsrc=os_rsrc, sorted_rsrc=sorted_rsrc, tokens=tokens,
                                inter_dim=inter_dim, m_repeat=M_REPEAT, num_acc_n=NUM_ACC_N, sort_block_m=sort_block_m,
                                tile_m=sort_block_m, tile_n=tile_n, num_waves=NUM_WAVES, lds_out=c_tile,
                                out_dtype=out_dtype, debug_gate=debug_gate)
        ...
        _while = scf.WhileOp([fx.Int32.ir_type], [arith._to_raw(it0)])
        before = _while.before.blocks.append(fx.Int32.ir_type)
        with ir.InsertionPoint(before):
            itv = arith.ArithValue(before.arguments[0])
            cond = arith.cmpi(CmpIPredicate.slt, arith._to_raw(_it_to_flat(itv)), arith._to_raw(total_work))
            scf.ConditionOp(arith._to_raw(cond), [arith._to_raw(itv)])
        after = _while.after.blocks.append(fx.Int32.ir_type)
        with ir.InsertionPoint(after):
            itv = arith.ArithValue(after.arguments[0])
            ...
            _compute_tile(
                m_tile=m_tile, sched=sched, a_gather=a_gather, a_s2r=a_s2r, b_loader=b_loader, b_scale=b_scale,
                a_scale=a_scale, mfma=mfma, epi=epi, a_buf=a_buf, a_scale_lds=a_scale_lds, a_lds_i32=a_lds_i32,
                n_tile_base=n_tile_base, K_ITERS=K_ITERS, STAGE=STAGE, SUB_I32=SUB_I32, M_REPEAT=M_REPEAT,
                NUM_ACC_N=NUM_ACC_N, A_K_STEP_BYTES=A_K_STEP_BYTES, sort_block_m=sort_block_m,
                pipe_weights=pipe_weights, mfma_amajor=mfma_amajor,
            )
            nxt = itv + it_stride
            scf.YieldOp([arith._to_raw(nxt)])
...
def _compute_tile(
    *,
    m_tile, sched, a_gather, a_s2r, b_loader, b_scale, a_scale, mfma, epi, a_buf, a_scale_lds, a_lds_i32,
    n_tile_base, K_ITERS, STAGE, SUB_I32, M_REPEAT, NUM_ACC_N, A_K_STEP_BYTES, sort_block_m,
    pipe_weights, mfma_amajor,
):
    """One (m_tile, n_tile): gather A->LDS, K-mainloop, epilogue (gate/up interleave)."""
    ...
    # Prime A super-step 0 and stage the whole tile's e8m0 A-scale to LDS once (K-loop reads it via ds_read).
    ...
    idx1 = arith.constant(1, index=True)
    idxN = arith.constant(K_ITERS, index=True)
    i32t = ir.IntegerType.get_signless(32)
    last = fx.Int32(K_ITERS - 1)

    if pipe_weights:
        # Software-pipelined weights: carry b_prev + acc; issue next-step B loads into this step's MFMA.
        assert STAGE == 1, "pipelined weight prefetch assumes STAGE==1 (fp8-A, tile_k=256)"
        b0 = b_loader.load_step(b_row, fx.Int32(0))
        init = [arith._to_raw(mfma.zero_value) for _ in range(N_ACC)]
        init += [arith._to_raw(h) for ni_list in b0 for h in ni_list]
        for_op = scf.ForOp(arith._to_raw(idx0), arith._to_raw(idxN), arith._to_raw(idx1), init)
        with ir.InsertionPoint(for_op.body):
            sp = arith.ArithValue(arith.index_cast(i32t, for_op.induction_variable))
            ia = for_op.inner_iter_args
            acc = [Vec(a) for a in ia[:N_ACC]]
            b_prev = [[Vec(ia[N_ACC + ni * _PACK + ks]) for ks in range(_PACK)] for ni in range(NUM_ACC_N)]
            cur_off = (sp & fx.Int32(1)) * fx.Int32(a_lds_i32)
            nxt_off = ((sp + fx.Int32(1)) & fx.Int32(1)) * fx.Int32(a_lds_i32)
            spn = arith.select(sp + fx.Int32(1) < last, sp + fx.Int32(1), last)
            ...
            wait_lds_barrier()
            yield_vals = [arith._to_raw(v) for v in acc]
            yield_vals += [arith._to_raw(h) for ni_list in b_next for h in ni_list]
        ...
        with ir.InsertionPoint(for_op.body):
            sp = arith.ArithValue(arith.index_cast(i32t, for_op.induction_variable))
            acc = [Vec(a) for a in for_op.inner_iter_args]
            cur_off = (sp & fx.Int32(1)) * fx.Int32(a_lds_i32)
            nxt_off = ((sp + fx.Int32(1)) & fx.Int32(1)) * fx.Int32(a_lds_i32)
            spn = arith.select(sp + fx.Int32(1) < last, sp + fx.Int32(1), last)
            a_regs = a_gather.load_regs(spn * fx.Int32(A_K_STEP_BYTES))
            for s in range_constexpr(STAGE):
                k = sp * fx.Int32(STAGE) + fx.Int32(s)
                a_base = cur_off + fx.Int32(s * SUB_I32)

                def a_load(mi, ks, _base=a_base):
                    return a_s2r.load_operand(a_buf, mi, ks, _base)

                b = b_loader.load_step(b_row, k)
                sa = a_scale.load_step(a_scale_lds, k)
                sb = b_scale.load_step(b_row, k)
                acc = mfma.call(a_load, b, acc, sa, sb)
            a_gather.store(a_buf, a_regs, nxt_off)
            wait_lds_barrier()
            scf.YieldOp([arith._to_raw(v) for v in acc])

    acc = [Vec(r) for r in for_op.results[:N_ACC]]
