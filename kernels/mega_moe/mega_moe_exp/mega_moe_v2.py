...
class MegaMoEV2(MegaMoE):
    ...
    def _build_fused_stage2(self, **kw):
        ...
        cu_num = torch.cuda.get_device_properties(torch.cuda.current_device()).multi_processor_count
        # shared compile kwargs; BM is the autotuned knob (compile cache keyed by BM).
        self._g2v2_kw = dict(
    ...
    def _g2v2_autotune_bm(self, run_tokens, run_call):
        """FlyDSL-core runtime autotune: bench each valid BM once per token bucket, cache the best.

        run_call(bm) launches the stage2 gemm2 for that BM. The P2P scatter epilogue is an idempotent
        overwrite, so repeated bench reps are safe. Skipped during CUDAGraph capture (falls back to the
        cached/default BM) since benching cannot happen inside a graph.
        """
        bucket = 1 << max(0, (int(run_tokens or 1) - 1)).bit_length()
        if bucket in self._g2v2_best_bm:
            return self._g2v2_best_bm[bucket]
        capturing = torch.cuda.is_current_stream_capturing()
        if capturing or len(self._g2v2_bm_candidates) == 1:
            return self._g2v2_best_bm.get(bucket, min(32, self._g2v2_bm_candidates[-1]))
        from flydsl.autotune import do_bench

        best_bm, best_t = None, float("inf")
        for bm in self._g2v2_bm_candidates:
            try:
                t = do_bench(lambda bm=bm: run_call(bm), warmup=3, rep=10)
            except Exception:
                continue
            if t < best_t:
                best_bm, best_t = bm, t
        best_bm = best_bm or min(32, self._g2v2_bm_candidates[-1])
        self._g2v2_best_bm[bucket] = best_bm
        return best_bm
    ...
    def _run_fused_stage2(self, s1, run_tokens, stream=None):
        ...
        def _run_call(bm):
            _run_compiled(self._g2v2_get_launch(bm), *args, fx.Int32(size_expert_ids), s_fx)

        bm = self._g2v2_autotune_bm(run_tokens, _run_call)  # FlyDSL runtime autotune over BM, cached per bucket
        _run_call(bm)
