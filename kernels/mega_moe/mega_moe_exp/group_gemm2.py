...
"""FP8xFP4 grouped-MoE GEMM2 compute shared by local and P2P-scatter epilogues."""
...
def scale_mma_atoms():
    return {
        (osa, osb): fx.make_mma_atom(
            fx.rocdl.cdna4.MFMA_Scale(16, 16, 128, fx.Float8E4M3FN, Float4E2M1FN, opsel_a=osa, opsel_b=osb)
        )
        for osa in range(4)
        for osb in range(4)
    }
...
def issue_a_ds_read_dt(s_aq_base, slot, slot_bytes, KH_TILE_A, lane_div_16, lane_mod_16, a_frags):
...
def issue_a_load_lds_dt(arg_aq, aq_num_records, s_aq_base, slot, kt, m_row, wave, lane, KH_TILE_A, K_BYTES):
    ...
    for h in range_constexpr(2):
        ...
        mask = lds_swizzle_mask_f8(lds_row + a_lane_row)
...
# fmt: off
def gemm2_compute(lds_base_i32, arg_ascale, arg_bq, arg_bscale, arg_eids, arg_aq, i32_max_m_blocks, bx_i32, lane,
    wave, *, N_OUT, D_INTER, aStages, use_nt, expert_offset=0):
# fmt: on
    """Compute one compact A2/W2 tile and return FP32 accumulators."""
    KH_TILE_A = BK
    K_BYTES = D_INTER
    ...
    e = fx.Int32(
        rocdl.readfirstlane(T.i32, buffer_ops.buffer_load(eids_rsrc, m_block_idx, vec_width=1, dtype=fx.Int32))
    )
    ...
    a_frags = [[fx.make_rmem_tensor(8, fx.Int32) for _ in range_constexpr(2)] for _ in range_constexpr(kMChunks)]
    ...
    def issue_a_ds_read(slot):
        issue_a_ds_read_dt(s_aq_base, slot, slot_bytes, KH_TILE_A, lane_div_16, lane_mod_16, a_frags)

    aq_num_records = fx.Int64(i32_max_m_blocks) * fx.Int64(BM * K_BYTES)

    def issue_a_load_lds(slot, kt):
        issue_a_load_lds_dt(arg_aq, aq_num_records, s_aq_base, slot, kt, m_row, wave, lane, KH_TILE_A, K_BYTES)
...
# fmt: off
def atomic_bf16_epilog(lds_acc_base, accm, arg_out, arg_stids, arg_sweights, m_row, n_block_idx, wave, lane, i32_M,
    N_OUT):
# fmt: on
...
def lds_bytes_for_gemm2(D_INTER, aStages=kAStages):
    s_aq_bytes = aStages * BM * BK
    lds_acc_bytes = BM * BN * 4
    return max(s_aq_bytes, lds_acc_bytes)


# fmt: off
def compile_group_gemm2(*, model_dim: int, inter_dim: int, experts: int, aStages: int=kAStages,
    use_nt: bool=True, num_cu: int=256, grid_mult: int=1):
# fmt: on
    N_OUT = model_dim
    D_INTER = inter_dim
    num_n_blocks = N_OUT // 256
    ...
    # fmt: off
    def kernel(addr_out: fx.Int64, addr_aq: fx.Int64, addr_ascale: fx.Int64, addr_bq: fx.Int64,
        addr_bscale: fx.Int64, addr_eids: fx.Int64, addr_stids: fx.Int64, addr_sweights: fx.Int64,
        addr_num_valid: fx.Int64, i32_M: fx.Int32, i32_max_m_blocks: fx.Int32):
    # fmt: on
        ...
        while itv < total_work:
            ...
            accm_vecs, m_row, n_block_idx = gemm2_compute(lds_base_i32, addr_ascale, addr_bq, addr_bscale,
                ...
                aStages=aStages, use_nt=use_nt)
            # fmt: on
            # fmt: off
            atomic_bf16_epilog(lds_base_i32, accm_vecs, addr_out, addr_stids, addr_sweights, m_row, n_block_idx,
                wave, lane, i32_M, N_OUT)
            # fmt: on
            itv = itv + fx.Int32(grid_x)

    @flyc.jit
    # fmt: off
    def launch(addr_out: fx.Int64, addr_aq: fx.Int64, addr_ascale: fx.Int64, addr_bq: fx.Int64,
        addr_bscale: fx.Int64, addr_eids: fx.Int64, addr_stids: fx.Int64, addr_sweights: fx.Int64,
        addr_num_valid: fx.Int64, i32_M: fx.Int32, i32_max_m_blocks: fx.Int32, stream: fx.Stream):
    # fmt: on
        kernel(
            addr_out,
            addr_aq,
