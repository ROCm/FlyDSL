...
from kernels.mfma_preshuffle_pipeline import _buffer_load_vec
# crd2idx / idx2crd: use the layout_utils versions (same as megaMoE kernels). They
# keep static-layout index math in index type and normalize coords before fly.crd2idx,
# matching the i32-typed layout strides produced by make_preshuffle_b_layout_packed4bit.
# The raw fx.idx2crd / mfma_preshuffle_pipeline.crd2idx do NOT normalize types, so
# index/i64 coords against i32 strides trigger an illegal i64->i32 extsi (APInt sext
# assertion) in fly-layout-lowering on the ghu core.
        ...
        def moe_gemm1(
            ...
            with _if_then(_if_blk):
                ...
                for ni in range_constexpr(num_acc_n):
                    ...
                    n_blk_gate.append(coord_gate[0])
                    n_intra_gate.append(coord_gate[1])
    
                    coord_up = idx2crd(row_up, layout_n_blk_intra)
                    n_blk_up.append(coord_up[0])
                    n_intra_up.append(coord_up[1])
                ...
                def load_b_tile_a8w4smooth(base_k, blk_list, intra_list):
                    ...
                    def _dequant_4pack(d0, d1, d2, d3, qs_lo, qs_hi, qz_lo, qz_hi, qz_lo_byte, qz_hi_byte):
                        ...
                        for di in range_constexpr(4):
                            ...
                            even = dw & c_0f0f0f0f
                            odd = (dw >> 4) & c_0f0f0f0f
                            if const_expr(not bool(overflow_guard)):
