...
def compile_fused_moe_gemm1(
    *,
    ...
    # DIAG (perf attribution): FUSED_MEGA_SKIP_GEMM=1 zeroes the consumer persist loop (tiles_per_block=0)
    # -> the launch times ONLY the dispatch (producer + block0).  FUSED_MEGA_PHASE_TS=1 enables the
    # per-phase s_memrealtime timestamps into addr_payload_done.  Both const_expr-gated (DCE when off).
    _skip_gemm = os.environ.get("FUSED_MEGA_SKIP_GEMM", "0") not in ("0", "", "false", "False")
    _phase_ts = os.environ.get("FUSED_MEGA_PHASE_TS", "0") not in ("0", "", "false", "False")
    # DIAG: FUSED_MEGA_SKIP_BODY=1 keeps the FULL consumer persist loop (all epr*cap tiles, per-tile
    # cnt_ready spin + acquire + address calc) but never calls _moe_gemm1_body -> isolates the per-tile
    # loop overhead from the GEMM compute.  (skip_gemm zeroes the loop; skip_body keeps it, drops GEMM.)
    _skip_body = os.environ.get("FUSED_MEGA_SKIP_BODY", "0") not in ("0", "", "false", "False")
    ...
    # G1 progressive-frontier overlap (docs/moe_stage1_overlap_design.md §7): expert-parallel producer
    # publishes per-(src,expert) readiness; consumer computes expert-ascending, gating each expert on
    # all-peer readiness instead of one global floor.  Non-compact fixedslot (P-static) ONLY; env-gated
    # and OFF by default -> the traced kernel is byte-identical to production when off.
    _frontier = bool(_static_tiles) and (os.environ.get("FUSED_MEGA_FRONTIER", "0") not in ("0", "", "false", "False"))
    # EXPERT-MAJOR consumer (design §7.2②): instead of a flat round-robin over ALL epr*cap padded tiles
    # (per-tile cnt_ready spin + sparse occupied tiles that break the weight L2/pipeline reuse), each
    # M-block owns a static stride of experts, spins cnt_ready ONCE per expert, then iterates ONLY that
    # expert's OCCUPIED k-tiles (dynamic inner bound = ceil(ll_count[e]/tile_m)) contiguously.  Restores
    # compact-path density (weight reuse) + drops the per-tile spin.  Requires _frontier; env-gated.
    _emajor = bool(_frontier) and (os.environ.get("FUSED_MEGA_EMAJOR", "0") not in ("0", "", "false", "False"))
    # SPLIT (design §6 / B): block-level producer/consumer split so dispatch (xGMI P2P write) and GEMM
    # (HBM weight read) run CONCURRENTLY on different CUs — no grid barrier between them.  First `_gp`
    # M-rounds (block_id.y < gp) = aggregator(flat 0, spins peers' expert_ready -> cnt_ready) + producers
    # (write payload, each warp owns a strided set of GLOBAL experts, writes them sequentially ->
    # progressive per-expert readiness).  block_id.y >= gp = consumers (expert-major gated GEMM, running
    # concurrently, computing expert e as soon as its cross-PE data lands).  Requires _emajor; env-gated.
    _split = bool(_emajor) and (os.environ.get("FUSED_MEGA_SPLIT", "0") not in ("0", "", "false", "False"))
    _gp = int(os.environ.get("FUSED_MEGA_GP", "1")) if _split else 0   # producer/aggregator M-rounds
    # Weight prefetch C (design §4.3 / §7.2②, the overlap "收益本体"): while a consumer block spins on
    # the per-expert cnt_ready[e] floor, warm the leading L2 lines of expert e's fp4 gate/up weights so
    # the following K-loop hits L2 (削 v4_pro 的 8x 冗余权重 BW 启动段).  Depth = number of 16B/thread
    # K-slices to prefetch; env FUSED_MEGA_PREFETCH_KTILES (0 = OFF, default -> pure A+B static-tile
    # frontier).  Frontier-only + const_expr-gated so default builds are byte-identical.
    try:
        _prefetch_kt = int(os.environ.get("FUSED_MEGA_PREFETCH_KTILES", "0")) if _frontier else 0
    except ValueError:
        _prefetch_kt = 0
    _prefetch_kt = max(0, _prefetch_kt)
    ...
    _fuse_tag += "_frt" if _frontier else ""   # G1 frontier -> distinct JIT kernel from production floor
    _fuse_tag += "_em" if _emajor else ""       # expert-major consumer -> distinct JIT kernel
    _fuse_tag += f"_sp{_gp}" if _split else ""   # producer/consumer split (gp M-rounds) -> distinct JIT kernel
    _fuse_tag += f"_pf{_prefetch_kt}" if _prefetch_kt > 0 else ""   # weight-prefetch depth -> distinct JIT kernel
    _fuse_tag += "_skg" if _skip_gemm else ""   # DIAG: skip-consumer variant -> distinct JIT kernel
    _fuse_tag += "_skb" if _skip_body else ""   # DIAG: skip-body variant -> distinct JIT kernel
    _fuse_tag += "_pts" if _phase_ts else ""    # DIAG: phase-timestamp variant -> distinct JIT kernel
    ...
    if True:
        ...
        def moe_gemm1(
            ...
            if const_expr(_fuse_fs and not _compact):
                ...
                # G1 frontier buffers (design §7): local expert_ready (consumer acquire + reset) and
                # p2p_expert_ready (producer publishes per-(src,expert) readiness to peers).
                if const_expr(_frontier):
                    _a_eready = _dp(26); _p_eready = _dp(27); _a_cntready = _dp(28)
                    if const_expr(_split):
                        _a_remaining = _dp(29)   # SPLIT countdown: per-src remaining[global_expert]
                    # Snapshot the launch epoch ONCE at kernel entry (== meta_flag = L-1), shared by the
                    # producer (publishes expert_ready=epoch), block0 (publishes cnt_ready=epoch) and the
                    # consumer (gates cnt_ready > L-1).  Read before block0's end-of-kernel meta update
                    # (which is ordered after the gb1 all-arrive), so every phase agrees on L-1.
                    _frt_em1 = buffer_ops.buffer_load(_crfa(_a_meta), 0, vec_width=1, dtype=T.i32)
                    _frt_ep = _frt_em1 + arith.constant(1, type=T.i32)
                ...
                # ---- SPLIT (B) role assignment (block_id.y based; block-uniform) ----
                if const_expr(_split):
                    _sp_by = arith.index_cast(T.i32, fx.block_idx.y)
                    _c_gp = arith.constant(_gp, type=T.i32)
                    _sp_flat_i = arith.index_cast(T.i32, _flat)
                    _sp_is_agg = arith.cmpi(CmpIPredicate.eq, _sp_flat_i, arith.constant(0))
                    _sp_is_cons = arith.cmpi(CmpIPredicate.sge, _sp_by, _c_gp)
                    _sp_is_prod = arith.andi(arith.cmpi(CmpIPredicate.slt, _sp_by, _c_gp),
                                             arith.cmpi(CmpIPredicate.ne, _sp_flat_i, arith.constant(0)))
                    # producer warps = flat[1 .. gp*gx-1]; each owns strided GLOBAL experts, written
                    # sequentially -> progressive readiness.  Non-producers skip (start=total_experts).
                    _sp_prodwn = (_c_gp * arith.index_cast(T.i32, _gdx) - arith.constant(1)) * arith.constant(num_waves)
                    _sp_prodwid = (_sp_flat_i - arith.constant(1)) * arith.constant(num_waves) + arith.index_cast(T.i32, _warp)
                    _prod_start_i = arith.select(_sp_is_prod, _sp_prodwid, arith.constant(_fz_total_experts, type=T.i32))
                    _prod_start = arith.index_cast(ir.IndexType.get(), _prod_start_i)
                    _prod_stride = arith.index_cast(ir.IndexType.get(), _sp_prodwn)
                ...
                # ---- payload write body for one (token,topk) pair -> fixed slot le*cap + atomic
                # (running[le]) on the DEST card.  Factored into a helper so both the default
                # (token,topk)-parallel writer AND the G1 expert-parallel writer emit the SAME stores.
                # ``_ge_match`` (frontier only) gates the write to a single owning expert.
                def _fz_write_pair(_wk, _ge_match=None):
                    ...
                    if _ge_match is not None:
                        # expert-parallel: this warp owns exactly one global expert -> only its rows.
                        _is_valid = arith.andi(_is_valid, arith.cmpi(CmpIPredicate.eq, _expert, _ge_match))
                ...
                if const_expr(_split):
                    # ---- SPLIT (B) CHEAP producer: token-parallel write O(tokens*topk) + COUNTDOWN
                    # readiness (replaces the O(experts*tokens) expert-parallel scan, which was CU/latency-
                    # bound and made the split's few producer CUs the bottleneck).  Only producer blocks
                    # (flat 1..gp*gx-1) run; each producer warp writes strided (token,topk) pairs, then
                    # after ONE fence decrements remaining[ge] per written row, then a per-expert finalize
                    # decrement publishes expert_ready[ge] on the 1->0 edge (remaining biased to count+1 by
                    # the host so 0-count experts are also published).  xGMI-BW-bound -> few producer CUs
                    # saturate the fabric, freeing the rest for the HBM-bound consumer (true overlap).
                    _epoch_frt = _frt_ep
                    _sp_prod_if = scf.IfOp(_sp_is_prod)
                    with ir.InsertionPoint(_sp_prod_if.then_block):
                        _pp_start = arith.index_cast(ir.IndexType.get(), _sp_prodwid)
                        _pp_stride = arith.index_cast(ir.IndexType.get(), _sp_prodwn)
                        for _wk in range(_pp_start, _wl, _pp_stride):     # write pass (pipelined)
                            _fz_write_pair(_wk)
                        _epk.fence_system_release()                       # drain this warp's writes ONCE
                        # PUBLISH-ON-1->0-EDGE (deadlock fix).  remaining[ge] is host-biased to
                        # count[ge]+1 (fused_moe_megakernel.py:505-521), so EXACTLY ONE decrement drives
                        # it 1->0 -- and that edge may be a ROW-decrement (below) OR the finalize.  The
                        # previous code published expert_ready ONLY in the finalize pass, so when a
                        # row-decrement hit the edge (finalize already ran) the expert was NEVER published
                        # -> aggregator int32_wait_until_greater_than spins forever -> DEADLOCK.  Publish
                        # on the edge in BOTH passes via the same helper.  Every producer warp already
                        # fence_system_release'd its writes (above) before ANY decrement, so the 1->0 edge
                        # implies all contributing warps' ge-rows are system-visible; the helper adds one
                        # release fence before the flag store (matches the non-split publisher, 1016-1022)
                        # to close the release->acquire chain to the aggregator.  Exactly one publish/expert.
                        def _sp_publish(_ge_v):   # lane0-only; _ge_v = i32 global expert that just hit 0
                            _dpe_p = _ge_v // arith.constant(_fz_epr)
                            _le_p = _ge_v % arith.constant(_fz_epr)
                            _epk.fence_system_release()
                            _er_b = buffer_ops.buffer_load(_crfa(_p_eready), _dpe_p, vec_width=1, dtype=T.i64)
                            _epk.store_i32_system(
                                _er_b, _le_p + arith.constant(_fz_rank * _fz_epr), _epoch_frt)
                        for _wk in range(_pp_start, _wl, _pp_stride):     # decrement pass (per written row)
                            if _lane == 0:
                                _ge_d = buffer_ops.buffer_load(_r_idx, _wk, vec_width=1, dtype=T.i32)
                                _vd_d = arith.cmpi(CmpIPredicate.ult, _ge_d, arith.constant(_fz_total_experts))
                                if _vd_d:
                                    _old_d = _epk.atomic_add_system(
                                        _a_remaining + _epk._to_i64(_ge_d) * 4, arith.constant(-1, type=T.i32))
                                    if arith.cmpi(CmpIPredicate.eq, _old_d, arith.constant(1)):
                                        _sp_publish(_ge_d)
                        for _ge in range(_pp_start, _fz_total_experts, _pp_stride):  # finalize (1 owner/expert)
                            _ge_i = arith.index_cast(T.i32, _ge)
                            if _lane == 0:
                                _old_r = _epk.atomic_add_system(
                                    _a_remaining + _epk._to_i64(_ge_i) * 4, arith.constant(-1, type=T.i32))
                                if arith.cmpi(CmpIPredicate.eq, _old_r, arith.constant(1)):
                                    _sp_publish(_ge_i)
                        scf.YieldOp([])
                elif const_expr(_frontier):
                    # ---- G1 expert-parallel writer (design §7.2 ①, non-split): each warp OWNS a strided
                    # set of global experts, scans ALL (token,topk), writes its rows, fences, publishes.
                    _epoch_frt = _frt_ep   # epoch = meta_flag(L-1) + 1, snapshotted at kernel entry
                    for _ge in range(_gwid, _fz_total_experts, _gwn):
                        _ge_i = arith.index_cast(T.i32, _ge)
                        for _wk in range(0, _wl):
                            _fz_write_pair(_wk, _ge_i)
                        _dpe_g = _ge_i // arith.constant(_fz_epr)
                        _le_g = _ge_i % arith.constant(_fz_epr)
                        _epk.fence_system_release()
                        if _lane == 0:
                            _er_base = buffer_ops.buffer_load(_crfa(_p_eready), _dpe_g, vec_width=1, dtype=T.i64)
                            _er_idx = _le_g + arith.constant(_fz_rank * _fz_epr)   # elem index in expert_ready[npes*epr]
                            _epk.store_i32_system(_er_base, _er_idx, _epoch_frt)
                else:
                    # ---- default: (token,topk)-parallel writer (strided over the whole grid) ----
                ...
                if _gwid == 0:
                  if const_expr(_frontier):
                    # ---- G1 block0 (design §7.2 ③): ride per-expert readiness ascending; NO monolithic
                    # done2 floor.  Per local expert: wait ALL peers' expert_ready[p][e] >= epoch (its
                    # cross-PE payload visible), snapshot ll_count[e]=running[e], reset running[e] (safe:
                    # e fully received), publish cnt_ready[e]=epoch so consumers compute e immediately.
                    # num_valid is a compile-time const (epr*cap rows) -> consumer loop bound needs no
                    # count reduction.  Each peer signals EVERY (p,e) (owner-warp runs all experts,
                    # even 0-token ones), so the per-expert wait never deadlocks.
                    _r_run = _crfa(_a_run); _r_cnt = _crfa(_a_cnt); _r_nv = _crfa(_a_nv)
                    _r_cr = _crfa(_a_cntready)
                    if _lane == 0:
                        buffer_ops.buffer_store(arith.constant(_fz_epr * _fz_cap), _r_nv, arith.constant(0))
                        buffer_ops.buffer_store(arith.constant(_fz_epr * _fz_cap), _r_nv, arith.constant(1))
                    for _ef in range(_lane, _fz_epr, 64):
                        for _pp in range_constexpr(_fz_npes):
                            _erx = _ef + arith.constant(_pp * _fz_epr)
                            mori_shmem.int32_wait_until_greater_than(
                                _a_eready + _epk._to_i64(_erx) * 4, _frt_em1)  # >= epoch (== _frt_em1+1)
                        _epk.fence_system_acquire()
                        _cc = buffer_ops.buffer_load(_r_run, _ef, vec_width=1, dtype=T.i32)
                        buffer_ops.buffer_store(_cc, _r_cnt, _ef)          # ll_count[e] = running[e]
                        buffer_ops.buffer_store(arith.constant(0), _r_run, _ef)  # reset (e fully received)
                        _epk.fence_agent_release()
                        # cnt_ready is LOCAL (no cross-card race): host memsets it to 0 each launch
                        # (in-graph, same stream) -> block0 sets 1 -> consumer waits >0.  No epoch needed.
                        buffer_ops.buffer_store(arith.constant(1, type=T.i32), _r_cr, _ef)
                    # meta = epoch for the NEXT launch.  Non-split: ordered after gb1 all-arrive so every
                    # block's kernel-entry meta snapshot (_frt_em1) sees L-1 before this store.  SPLIT: no
                    # gb1 (aggregator gates on expert_ready only) -> skip the arrival wait; kernel
                    # completion (stream) orders meta before the next launch's entry snapshot.
                    if const_expr(not _split):
                        _one2f = arith.constant(1, type=T.i64)
                        _gb1nowf = buffer_ops.buffer_load(_crfa(_a_gb1), 0, vec_width=1, dtype=T.i64)
                        _tg2f = ((arith.ArithValue(_gb1nowf, signed=False) - _one2f) // _bn + _one2f) * _bn
                        mori_shmem.int64_wait_until_equals(_a_gb1, _tg2f)
                    if _lane == 0:
                        _epk.fence_agent_release()
                        buffer_ops.buffer_store(_frt_ep, _crfa(_a_meta), arith.constant(0))
                    _epk.fence_system_release()
                  else:
                ...
                # frontier: NO single metadata gate -- the consumer gates each expert on cnt_ready[e]
                # (per-expert), so low experts compute while high experts are still being received.
                if const_expr(not _frontier):
                    if _tid == 0:
            ...
            if const_expr(_frontier):
                # FIRST-LAUNCH-CORRECTNESS FIX: frontier num_valid is the compile-time constant epr*cap
                # (ALL padded slots).  block0 writes it, but the frontier consumer has NO metadata gate
                # (the per-expert cnt_ready replaces the meta floor), so reading the buffer here RACES
                # block0's write: on a cold launch (num_valid buffer still 0) racing consumer blocks read
                # 0 -> _tiles_per_block=0 -> skip all tiles -> partial/zero a2 (only launch #1; later
                # launches read the previous launch's epr*cap, masking the bug).  Use the constant.
                num_valid_i32 = arith.constant(_fz_epr * _fz_cap, type=T.i32)
            else:
            ...
            if const_expr(_frontier):
                # G1 consumer: STATIC tile->(e,k) map (design §7.1/A) -> no ll_count prefix (which would
                # need ALL counts final = a floor).  ll_count read per-expert AFTER its cnt_ready gate.
                _st_llr = buffer_ops.create_buffer_resource_from_addr(addr_expected_real)  # ll_count[epr]
                _cr_base = buffer_ops.buffer_load(
                    buffer_ops.create_buffer_resource_from_addr(addr_disp),
                    arith.constant(28), vec_width=1, dtype=T.i64)  # cnt_ready[epr] base addr (disp 28)
                if const_expr(_emajor):
                    # EXPERT-MAJOR pre-loop: this M-block (block_id.y) owns experts {bx_persist + j*gy}
                    # for j in [0, NEB).  Spin cnt_ready ONCE per owned expert (agent-acquire), read
                    # ll_count -> ntiles=ceil(count/tile_m), build an EXCLUSIVE prefix -> the block's
                    # total OCCUPIED tiles _em_Tb.  The persist loop then iterates only occupied tiles,
                    # expert-contiguous (restores weight reuse), with the flat index mapped back to
                    # (e,k) via the prefix.  gy is the compile-time launch grid_y (_fz_gy).
                    # SPLIT: consumers are M-rounds [gp, gy) -> effective M-round = block_id.y - gp,
                    # gy_cons = gy - gp.  Non-consumer blocks (producer/aggregator) get _em_ok=False for
                    # every j -> _em_Tb=0 -> the persist loop runs 0 tiles (they only produce/aggregate).
                    _em_gy = (_fz_gy - _gp) if const_expr(_split) else _fz_gy
                    _NEB = (_fz_epr + _em_gy - 1) // _em_gy
                    if const_expr(_split):
                        _em_byp = _sp_by - _c_gp
                    else:
                        _em_byp = arith.index_cast(T.i32, bx_persist)
                    _em_ctm1 = arith.constant(_fz_tile_m - 1, type=T.i32)
                    _em_ctm = arith.constant(_fz_tile_m, type=T.i32)
                    _em_es = []   # per owned expert: (expert_id_i32, ll_count_i32)
                    _em_pref = [arith.constant(0, type=T.i32)]  # exclusive prefix of ntiles (i32)
                    for _emj in range_constexpr(_NEB):
                        _em_ei = _em_byp + arith.constant(_emj * _em_gy, type=T.i32)
                        _em_ok = arith.andi(arith.cmpi(CmpIPredicate.sge, _em_ei, arith.constant(0)),
                                            arith.cmpi(CmpIPredicate.slt, _em_ei, arith.constant(_fz_epr, type=T.i32)))
                        if const_expr(_split):
                            _em_ok = arith.andi(_em_ok, _sp_is_cons)  # only consumer blocks spin/consume
                        _em_eic = arith.select(_em_ok, _em_ei, arith.constant(0, type=T.i32))
                        # spin ONCE per owned expert (block-uniform _em_ok -> no divergence); skip for
                        # non-owned/non-consumer so producers/aggregator never enter the consumer gate.
                        _em_spif = scf.IfOp(_em_ok)
                        with ir.InsertionPoint(_em_spif.then_block):
                            mori_shmem.int32_wait_until_greater_than(
                                _cr_base + _epk._to_i64(_em_eic) * 4, arith.constant(0))
                            _epk.fence_agent_acquire()
                            scf.YieldOp([])
                        _em_cnt = buffer_ops.buffer_load(_st_llr, _em_eic, vec_width=1, dtype=T.i32)
                        _em_cnt = arith.select(_em_ok, _em_cnt, arith.constant(0, type=T.i32))
                        _em_nt = (_em_cnt + _em_ctm1) // _em_ctm
                        _em_es.append((_em_eic, _em_cnt))
                        _em_pref.append(_em_pref[_emj] + _em_nt)
                    _em_Tb = _em_pref[_NEB]
                    _em_Tb_idx = arith.index_cast(ir.IndexType.get(), _em_Tb)
            if const_expr(_static_tiles and _fz_epr <= 64 and not _frontier):
            ...
            _persist_bound = (arith.constant(0, index=True) if const_expr(_skip_gemm) else _em_Tb_idx) \
                if const_expr(_emajor) else _tiles_per_block
            _for_persist = scf.ForOp(_c0_p, _persist_bound, _c1_p)
            ...
            if const_expr(_emajor):
                # EXPERT-MAJOR: map the flat OCCUPIED-tile index _mi_p -> (e,k) via the per-block prefix
                # (pre-loop).  Tiles are expert-contiguous and all occupied -> no per-tile cnt_ready spin
                # (done once/expert), dense weight reuse.  _cnt_ef carried for the epilogue row mask.
                _mi_i32 = arith.index_cast(T.i32, _mi_p)
                _ef = arith.constant(0, type=T.i32)
                _kf = arith.constant(0, type=T.i32)
                _cnt_ef = arith.constant(0, type=T.i32)
                for _emj in range_constexpr(_NEB):
                    _in = arith.andi(
                        arith.cmpi(CmpIPredicate.sge, _mi_i32, _em_pref[_emj]),
                        arith.cmpi(CmpIPredicate.slt, _mi_i32, _em_pref[_emj + 1]))
                    _ef = arith.select(_in, _em_es[_emj][0], _ef)
                    _kf = arith.select(_in, _mi_i32 - _em_pref[_emj], _kf)
                    _cnt_ef = arith.select(_in, _em_es[_emj][1], _cnt_ef)
                _kf_row_i32 = _kf * arith.constant(_fz_tile_m, type=T.i32)
                blk_valid = arith.cmpi(CmpIPredicate.slt, _mi_i32, _em_Tb)
                bx_m_i32 = _ef * arith.constant(_fz_cap, type=T.i32) + _kf_row_i32
                ...
                # G1: static (e,k) = (bx // NT, bx % NT), NT = cap/tile_m tiles per expert.  Gate on the
                # per-expert cnt_ready[e] (LOCAL cacheline spin; block0 sets it once e's cross-PE payload
                # is in + ll_count[e] written), then read ll_count[e] for the row mask.  Low experts
                # compute while high experts are still arriving.
                _st_bx = arith.index_cast(T.i32, bx)
                _NT = arith.constant(_fz_cap // _fz_tile_m, type=T.i32)
                _ef = _st_bx // _NT
                _kf = _st_bx % _NT
                blk_valid = arith.cmpi(CmpIPredicate.ult, _st_bx, arith.index_cast(T.i32, _total_m_tiles))
                # RAGGED-EDGE FIX: the persist loop bound is ceil(total_m_tiles / grid_y), so the last
                # iteration of some blocks yields bx >= total_m_tiles -> _ef = bx//NT >= epr.  The per-
                # expert cnt_ready spin below reads cnt_ready via a RAW (unbounded) address
                # (_cr_base + _ef*4) BEFORE the blk_valid guard, so an out-of-range _ef reads past the
                # cnt_ready[epr] buffer and int32_wait_until_greater_than(>0) never satisfies -> deadlock.
                # Clamp _ef to an in-range, already-ready expert (0) for out-of-range tiles; blk_valid
                # masks the whole tile body, so the clamp never affects output (design §7.7 correctness).
                _ef = arith.select(blk_valid, _ef, arith.constant(0, type=T.i32))
                # ---- weight prefetch C (design §4.3/§7.2②): ISSUE the leading-K weight loads of expert
                # e BEFORE the cnt_ready[e] spin so the (long, per-expert) floor overlaps warming L2.
                # w_rsrc is fp4-packed, expert-major: expert e's block is _ef * (inter_dim*model_dim/4)
                # i32 words.  Each thread grabs a distinct 16B (v4 i32) slice; depth = prefetch K-slices.
                # The loaded values live in VGPRs across the spin, then are sunk to an OOB index (dropped
                # by the buffer resource) so the loads are DCE-safe WITHOUT touching LDS/occupancy.  Runs
                # for empty tiles too (occupancy unknown pre-floor) but bounded by depth -> minor waste
                # (design risk ②).  sched_barrier pins the loads ahead of the spin.
                if const_expr(_prefetch_kt > 0):
                    _pf_pe_i32 = arith.constant((inter_dim * model_dim) // 4, type=T.i32)  # i32 words / expert
                    _pf_base = _ef * _pf_pe_i32
                    _pf_tid = arith.index_cast(T.i32, tx) * arith.constant(4, type=T.i32)
                    _pf_vals = []
                    for _pfd in range_constexpr(_prefetch_kt):
                        _pf_idx = _pf_base + arith.constant(_pfd * total_threads * 4, type=T.i32) + _pf_tid
                        _pf_vals.append(buffer_ops.buffer_load(
                            w_rsrc, _pf_idx, vec_width=4, dtype=T.i32, cache_modifier=0))
                    rocdl.sched_barrier(0)
                # per-expert gate: spin on cnt_ready[e] > 0 (LOCAL cacheline), then acquire.
                mori_shmem.int32_wait_until_greater_than(_cr_base + _epk._to_i64(_ef) * 4,
                                                         arith.constant(0))
                _epk.fence_agent_acquire()
                if const_expr(_prefetch_kt > 0):
                    # DCE-safe sink: store the prefetched words to the FIRST out-of-bounds i32 word of
                    # w_rsrc (num_records = w_nbytes_s1 bytes) -> the buffer resource drops the OOB store,
                    # so nothing is clobbered but the loads/stores are not dead-code-eliminated.
                    _pf_oob = arith.constant((inter_dim * model_dim // 4) * int(experts_per_rank), type=T.i32)
                    for _pfd in range_constexpr(_prefetch_kt):
                        buffer_ops.buffer_store(_pf_vals[_pfd], w_rsrc, _pf_oob)
                _cnt_ef = buffer_ops.buffer_load(_st_llr, _ef, vec_width=1, dtype=T.i32)
                # EMPTY-TILE SKIP (design §7 "逐 tile 掩码"): num_valid is a compile-time const
                # (epr*cap = ALL padded slots) since the per-expert frontier can't compact a tile list
                # without a global floor.  So the static (e,k) loop iterates EVERY padded tile; gate the
                # whole tile on occupancy (k*tile_m < ll_count[e]) here so empty tiles skip the entire
                # weight-BW K-loop body (_if_blk) instead of running it on padding.  WITHOUT this, decode
                # (occupied ~= 1 of NT tiles/expert) runs NT x too many weight loads and frontier is
                # strictly slower than the non-frontier compacted static path -> the overlap benefit can
                # never show.  Row-level masking (_real_m) in the epilogue is unchanged.
                _kf_row_i32 = _kf * arith.constant(_fz_tile_m, type=T.i32)
                _tile_occ = arith.cmpi(CmpIPredicate.ult, _kf_row_i32, _cnt_ef)
                blk_valid = arith.andi(blk_valid, _tile_occ)
                bx_m_i32 = _ef * arith.constant(_fz_cap, type=T.i32) + _kf_row_i32
                bx_m = arith.index_cast(ir.IndexType.get(), bx_m_i32)
                expert_i32 = _ef + arith.constant(_fz_rank * _fz_epr, type=T.i32)
                _e_w_i32 = _ef
                expert_idx = arith.index_cast(ir.IndexType.get(), _e_w_i32)
                exp_valid = arith.cmpi(CmpIPredicate.ult, expert_i32, arith.constant(experts, type=T.i32))
            elif const_expr(_static_tiles and _fz_epr <= 64):
            ...
            # DIAG probe: skip_body -> always-false gate (loop + per-tile overhead run, GEMM body skipped)
            _blk_run = (arith.cmpi(CmpIPredicate.ne, arith.constant(0), arith.constant(0))
                        if const_expr(_skip_body) else blk_valid)
            _if_blk = scf.IfOp(_blk_run)
            with ir.InsertionPoint(_if_blk.then_block):
                ...
                with ir.InsertionPoint(_ifexpert_of.then_block):
                    ...
                    _moe_gemm1_body()
                    scf.YieldOp([])
                # End-of-tile workgroup barrier, GATED to occupied tiles (moved INSIDE _if_blk).
                # blk_valid is block-uniform (bx + ll_count[e] are wave-uniform), so all waves agree
                # -> no barrier divergence.  The frontier consumer visits epr*cap PADDED tiles (~NT per
                # expert, only ~1 occupied for decode); an UNCONDITIONAL end-of-tile barrier made every
                # empty tile pay a full workgroup sync -- the dominant consumer overhead (design §7.7①).
                # Empty tiles skip _moe_gemm1_body entirely (touch no LDS), so they need no barrier.
                gpu.barrier()
                scf.YieldOp([])
