...
│   │   │   └── rocdl/             # Target-specific ROCDL package (cdna4, cluster, inline_asm, tdm_ops, universal); lazy-loaded via __init__'s _LAZY_MODULES. Shadows the same-named legacy expr/rocdl.py (the package wins on import — see Kernel Authoring Conventions)
...
- **`expr/rocdl` is both a package and a same-named module.** `expr/rocdl/` (`__init__.py` + `cluster.py`, `tdm_ops.py`, `cdna4.py`, `universal.py`, `inline_asm.py`) coexists with a same-named `expr/rocdl.py`. CPython resolves the package over the `.py`, so `from flydsl.expr import rocdl` and `flydsl.expr.rocdl` always bind to `expr/rocdl/__init__.py`; `expr/rocdl.py` is unreachable via that import path. The two are kept mirrored by hand (e.g. PR #629 applied an identical edit to both) — when changing `expr/rocdl.py`, mirror the change into `expr/rocdl/__init__.py` or it has no effect on `flydsl.expr.rocdl`. Import submodules explicitly, e.g. `from flydsl.expr.rocdl import cluster`; `flydsl.expr.tdm_ops` is a lazy alias for `flydsl.expr.rocdl.tdm_ops` (see `expr/__init__.py` `_LAZY_MODULES`).
...
- Multi-GPU coverage (`tests/kernels/test_flydsl_shmem.py`, `tests/kernels/test_allreduce.py`) runs under `pytest -m multi_gpu`. The shmem regression skips below 2 GPUs; the allreduce accuracy/benchmark tests skip below 8 GPUs. It is gated in CI to a label-triggered job (PR label `multi-gpu` or manual dispatch) on the 8-GPU runner matrix (`linux-flydsl-mi325-8`, `linux-flydsl-mi355-8`); it does not run in the default `scripts/run_tests.sh` flow.
