...
| Run a hand-edited `.s` | `FLYDSL_HACK_UT_ASM` (path to a `.s` dumped by `FLYDSL_DUMP_IR`; assembled with the ROCm `clang` and substituted for the generated device object, bypassing the disk cache. Scoped by kernel symbol: kernels the `.s` does not name keep compiler codegen, and a `.s` that matched nothing warns at exit. Debug/UT only, rocm backend only, incompatible with `FLYDSL_COMPILE_LLVM_DIR` / `FLYDSL_RUNTIME_RUN_ONLY`. `FLYDSL_HACK_UT_ASM_SAVE=1` also writes the assembled code object next to the `.s` as `.hsaco`. See `python/flydsl/compiler/hack_asm.py`) |
