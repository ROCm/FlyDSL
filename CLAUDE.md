...
FlyDSL (Flexible Layout Python DSL) — a Python DSL and MLIR-based compiler stack for authoring high-performance GPU kernels with explicit layouts and tiling on AMD GPUs (MI300X/MI350/MI400).
...
│   ├── expr/               # DSL expression API (arith, vector, gpu, math, rocdl, buffer_ops, mem_ops, typing)
│   │   ├── primitive.py    # Layout algebra & tiled ops (make_layout, copy_atom_call, gemm, etc.)
│   │   ├── derived.py      # ThrCopy/ThrMma — per-thread partitioning views
│   │   └── rocdl/          # MFMA/WMMA atoms, make_buffer_tensor, BufferCopy, tdm_ops (gfx1250)
