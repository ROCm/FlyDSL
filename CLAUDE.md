...
- Use `range_constexpr` for compile-time unrolled Python loops. Use `fx.range(start, stop, step, init=[...])` for `scf.for` loops with loop-carried values, and `fx.range(..., unroll=N)` / `unroll_full=True` to attach an unrolling hint. Prefer the `fx.` form inside traced code: a bare `range` is rewritten the same way, but `fx.range` also works when the function is called without `@flyc.kernel` / `@flyc.jit`, where it falls back to the builtin `range` and rejects the keywords an untraced loop cannot honor.
