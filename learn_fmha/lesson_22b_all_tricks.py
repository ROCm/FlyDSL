...
"""Lesson 22b — The all-tricks kernel (tutorial style), end to end in ONE file.

This is the companion to the lesson_22 scoreboard: a single self-contained, heavily
commented fp8 causal attention kernel that FOLDS IN the wins from Part D that fit the
teaching style, so you can read the whole optimized kernel at once. It starts from
lesson_07 (fp8, 1 wave) and adds:

  * MULTIWAVE (Lesson 08): NWAVES waves/workgroup, each wave owns 16 q-rows -> BM = 16*NWAVES.
    A grid of workgroups now fills the machine.
  * COLUMN-V (Lesson 17): V is stored [d, kv] (kv contiguous). GEMM2 contracts over kv, so
    its V operand is now a single wide read -- the V transpose is GONE. (P still needs its
    transpose; we keep the register ds_bpermute path from Lesson 12 for that.)
  * CAUSAL KV-BOUND (Lesson 14): each q-tile stops its kv loop at the causal limit.
  * FAST EXP2 (Lesson 13): rocdl.exp2 in the softmax.
  * register-resident P transpose (Lesson 12): ds_bpermute, no P-LDS round trip.

Deliberately OMITTED (to stay readable, not because they don't matter): paged KV, GQA,
p_scale, diagonal-pair, ping-pong double-buffer. Those live in the production kernel
kernels/fmha_prefill_fp8_ck.py (61/70 TF). This file trades some of that last perf for
being a single ~250-line kernel you can hold in your head.

NOTE on layout vs lesson 07: lesson 07 used a 16x16x32 MFMA. To make the P register-
transpose (ds_bpermute, Lesson 12) clean, this capstone uses the 32x32x16 fp8 MFMA like
...
_alloc = SmemAllocator(None, arch="gfx942", global_sym_name="lesson22b_smem")
...
def attn_kernel(Q: fx.Tensor, K: fx.Tensor, V: fx.Tensor, O: fx.Tensor,
    ...
    # workgroup blk handles q-tile `blk`; wave owns its 32-row slice.
    ...
    for ks in fx.range_constexpr(KSTEPS):
        off = qrow_safe * fx.Int32(HD) + fx.Int32(ks * 16) + half * fx.Int32(8)
    ...
    for kt, st in range(fx.Index(0), fx.Index(n_kv), fx.Index(1), init=init):
        ...
        for p in fx.range_constexpr(NPASS):
            ...
            k_off = kvg_s * fx.Int32(HD) + cg * fx.Int32(16)
...
def _run_one(b, sq, sk, causal, pscale=1.0):
    torch.manual_seed(0)
    ...
    Kq, kd = _quant(Kf)
    Vq, vd = _quant(Vf)
    Vcol = Vq.t().contiguous()  # COLUMN-major V: [d, kv]
    O = torch.zeros(sq, HDV, dtype=torch.bfloat16).cuda()
    sm = 1.0 / HD**0.5
    grid = (sq + BM - 1) // BM
    run_attn(Qq.cuda(), Kq.cuda(), Vcol.cuda(), O, qd, kd, vd, sq, sk, sm, causal, grid, stream=torch.cuda.Stream())
    ...
    print(f"sq={sq} sk={sk} causal={causal} BM={BM}  err={err:.4f}  {'PASS' if err < 6e-2 else 'FAIL'}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) >= 5:
        ...
        for shp in [(1, 256, 256, 1), (1, 1024, 1024, 1), (1, 512, 768, 1), (1, 256, 256, 0)]:
