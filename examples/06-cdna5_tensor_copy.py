...
carries a ``K_TILES`` rest while the LDS tile carries a size-1 rest. This example uses
equal mode-0 sizes; the API also permits independent ITER counts within mode-0.
...
def tdm_copy_kernel(
    ...
    # LDS rest = 1, coordinate rest = K_TILES; each result preserves its own rest.
...
def tdm_tensor_copy(
    ...
    # One block per N-slab; the runtime M determines the number of tiles to stream.
    grid = ((n + TN - 1) // TN, 1, 1)
    tdm_copy_kernel(A, B, (m + TM - 1) // TM).launch(grid=grid, block=(32, 1, 1), stream=stream)
