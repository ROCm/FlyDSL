# ==========================================================================
# THIS FILE IS DEPLOYED AS:  flydsl/_mlir/_mlir_libs/__init__.py
#
# It is the package initializer for the `_mlir_libs` directory, NOT for the
# `flir_ir/` source directory where it lives.  CMake's post-build step and
# setup_python_package.sh both copy it to:
#
#   <build>/python_packages/flydsl/flydsl/_mlir/_mlir_libs/__init__.py
#
# When editing this file, keep in mind that at runtime it will be imported as
#   flydsl._mlir._mlir_libs
# and the `_flir_ir.so` extension module lives in the same directory.
# ==========================================================================
#
...
def append_load_on_create_dialect(dialect: str):
    """Register a dialect to be loaded when a new Context is created.

    Required by upstream MLIR's ir.py which imports this from _mlir_libs.
    """
...
def get_load_on_create_dialects():
    """Return the list of dialects to auto-load on Context creation.
