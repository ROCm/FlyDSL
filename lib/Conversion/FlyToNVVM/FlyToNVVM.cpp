...
namespace mlir {
#define GEN_PASS_DEF_FLYTONVVMCONVERSIONPASS
...
unsigned mapToLLVMAddressSpace(AddressSpace addrSpace) {
  ...
  case AddressSpace::Register:
    // NVVM has no dedicated register address space; register-backed scratch
    // is modelled with an alloca in the generic (0) address space.
    return 0;
  }
...
// Create a freshly named shared-memory global of `[nbytes x i8]` in `addrSpace`,
// inserted at the start of `moduleOp`. The symbol name is `prefix` followed by a
// counter chosen to avoid collisions with existing globals in the module. Shared
// by the static-shared `make_ptr` and dynamic-shared `get_dyn_shared` lowerings.
static LLVM::GlobalOp createSharedGlobal(ConversionPatternRewriter &rewriter,
  ...
  // Shared (addrspace 3) symbols are per-block hardware and can never resolve
  // across DSO.
...
public:
  ...
  LogicalResult matchAndRewrite(MakePtrOp op, OpAdaptor adaptor,
    ...
    if (isGenericAddressSpace<AddressSpace::Register>(addrSpaceAttr)) {
      ...
      // Static shared sub-allocation. Each op lowers to a freshly named
      // `@__shared_alloc_<n>` shared-memory global.
      auto dictAttrs = op.getDictAttrs();
      if (!dictAttrs)
      ...
      LLVM::GlobalOp global =
          createSharedGlobal(rewriter, moduleOp, loc, "__shared_alloc", allocBytesAttr.getInt(),
...
public:
  ...
  LogicalResult matchAndRewrite(GetDynSharedOp op, OpAdaptor adaptor,
    ...
    if (!sharedGlobal) {
      sharedGlobal = createSharedGlobal(rewriter, moduleOp, loc, "__dynamic_shared",
...
public:
  ...
  LogicalResult matchAndRewrite(ToLLVMPtrOp op, OpAdaptor adaptor,
    ...
    if (!convertedPtrTy)
      return op.emitError("pointer address space does not lower to a bare NVVM LLVM pointer: ")
    ...
    if (requestedPtrTy.getAddressSpace() != convertedPtrTy.getAddressSpace())
      return op.emitError("requested LLVM address space ")
             << requestedPtrTy.getAddressSpace() << " but NVVM conversion requires "
...
class FlyToNVVMConversionPass
    : public mlir::impl::FlyToNVVMConversionPassBase<FlyToNVVMConversionPass> {
public:
  using mlir::impl::FlyToNVVMConversionPassBase<
      FlyToNVVMConversionPass>::FlyToNVVMConversionPassBase;
  ...
  void runOnOperation() override {
    ...
    target.addLegalDialect<arith::ArithDialect, scf::SCFDialect, vector::VectorDialect,
                           ...
                           NVVM::NVVMDialect>();
    target.addIllegalDialect<fly::FlyDialect, fly_nvvm::FlyNVVMDialect>();
