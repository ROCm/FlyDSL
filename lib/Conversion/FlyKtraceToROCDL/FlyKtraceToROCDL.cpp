...
// Record layout, 32 bytes. Must match python/flydsl/expr/ktrace.py, which decodes
...
LogicalResult checkTargetArch(gpu::GPUModuleOp mod) {
  ArrayAttr targets = mod.getTargetsAttr();
  if (!targets || targets.empty())
    return mod.emitError("ktrace: gpu.module carries no target; cannot verify "
                         "that in-kernel timestamps are supported");
...
struct FlyKtraceToROCDLConversionPass
  ...
  void runOnOperation() override {
    ...
    // Event ids are process-global, not per-kernel: the device writes only the
    // integer and the host cannot tell which kernel a record came from, so two
    // kernels numbering from 1 would make id 1 ambiguous. One counter for the
    // whole module keeps every id decodable. The table is published as
    // fly_ktrace.event_names for the host to decode against.
    llvm::StringMap<uint32_t> ids;
    auto idFor = [&ids](StringRef name) {
      auto it = ids.find(name);
      if (it != ids.end())
        return it->second;
      uint32_t id = static_cast<uint32_t>(ids.size()) + 1;
      ids[name] = id;
      return id;
    };
    ...
    WalkResult result = root->walk([&](gpu::GPUFuncOp kernel) {
      ...
      if (!mod || failed(checkTargetArch(mod)))
