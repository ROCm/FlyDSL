...
//===----------------------------------------------------------------------===//
//
// An object exported by flyc.compile(...).export_to_c(...) owns one
  ...
  *slot = nullptr;
  ...
  for (auto &[device, entry] : aot->devices) {
    ...
    if (!err) {
      ...
      // kernels may still be executing asynchronously. Keep their code object
      // alive until the device has finished using it.
