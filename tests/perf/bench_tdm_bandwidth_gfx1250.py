...
def _tdm_load_tile(g_tensor, blk_sel, lds_ptr, mask, wave):
    ...
    atom, coord = fx.rocdl.cdna5.make_tiled_tdm_atom(
        fx.rocdl.TensorLoad(), g_tensor, box, (TILE, TILE), num_warps=NUM_WARPS
    )
    atom = fx.atom_set_value(atom, "workgroup_mask", mask)
    blk = fx.zipped_divide(coord, (TILE, TILE))[None, blk_sel]
    s = fx.make_view(lds_ptr, fx.make_layout(((TILE, TILE), 1), ((TILE, 1), TILE * TILE)))
...
def _compile_tdm_unique_add(grid_m, grid_n):
    ...
    def tdm_unique_add_kernel(arg_a: fx.Tensor, arg_b: fx.Tensor, arg_c: fx.Tensor):
        ...
        gA = fx.make_view(fx.get_iter(arg_a), fx.make_layout((total_wgs * TILE, TILE), (TILE, 1)))
        gB = fx.make_view(fx.get_iter(arg_b), fx.make_layout((total_wgs * TILE, TILE), (TILE, 1)))
