...
_triton_MetaData = None


def _load_triton_fa():
    global _triton_loaded, _triton_attention, _triton_MetaData
    if _triton_loaded:
        return
    _triton_loaded = True
    try:
        import triton  # noqa: F401
        _fa_path = Path(os.getenv(
            "TRITON_FA_PATH",
            "/home/yanguahe/code/triton_perf/triton/fa/flash-attention.py",
        ))
        if not _fa_path.exists():
            print(f"  [WARN] Triton FA not found: {_fa_path}", flush=True)
            return
        ...
        sys.path.remove(_fa_dir)
        _triton_MetaData = _fa.MetaData
        _triton_attention = _fa.attention
    except ImportError:
        print("  [WARN] triton not installed, --compare disabled", flush=True)


def run_triton_bench(
    batch, seq_len, nheads, head_dim, dtype, causal,
    warmup, iters, seed=DEFAULT_SEED, layout="bshd",
):
    """Run Triton flash attention kernel and return {tflops, max_err, us}."""
    _load_triton_fa()
    if _triton_attention is None:
        return {"err": "triton flash-attention not loaded"}
    ...
    sm_scale = D ** -0.5
    metadata = _triton_MetaData(sm_scale=sm_scale)
    ...
    try:
        tri_out, _, _ = _triton_attention(q, k, v, o, metadata)
        torch.cuda.synchronize()
    except Exception as e:
        import traceback; traceback.print_exc()
        return {"err": f"triton: {e}"}

    ref = pytorch_ref_attention(q.float(), k.float(), v.float(), causal=causal).to(dtype)
    max_err = (tri_out.float() - ref.float()).abs().max().item()
    results["max_err"] = max_err

    try:
        def bench_fn():
            _triton_attention(q, k, v, o, metadata)
...
def main():
    ...
    parser.add_argument(
        ...
        help="Compare FlyDSL vs Triton performance (requires triton)",
    )
    ...
    if args.compare:
        # ---- Comparison mode: FlyDSL vs Triton ----
        print("=" * 130)
        print(f"FlyDSL vs Triton  ({causal_desc}, {dtype_desc})")
        ...
        for dtype_key in dtypes_to_test:
            ...
            for causal in causals_to_test:
                for batch, seq_len, nh, hd in configs:
                    ...
                    tri_r = run_triton_bench(
                        batch, seq_len, nh, hd, dtype, causal,
                        warmup=args.warmup, iters=args.iters,
                        seed=args.seed,
                    )
                    rows.append((tag, fly_r, tri_r))

        col = f"{'Time(us)':>10s} {'TFLOPS':>8s} {'MaxErr':>8s}"
        cmp_col = f"{'TFLOPS':>7s} {'MaxErr':>6s}"
        hdr1 = (
            f"{'Config':>45s} | {'FlyDSL':^28s} | {'Triton':^28s}"
            f" | {'Fly/Tri':^14s}"
        )
        hdr2 = (
            f"{'':>45s} | {col} | {col}"
        ...
        print(hdr2)
        print(sep)
        for tag, fly_r, tri_r in rows:
            print(
                f"{tag:>45s} | {_fmt_result(fly_r)} | "
                f"{_fmt_result(tri_r)}"
                f" | {_fmt_cmp(fly_r, tri_r)}"
            )
        print("=" * len(hdr2))
