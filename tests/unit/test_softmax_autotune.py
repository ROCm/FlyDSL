...
def _identities(configs):
    return [(c.kwargs["BLOCK_THREADS"], c.waves_per_eu) for c in configs]


# ── candidate generation ─────────────────────────────────────────────────
@pytest.mark.parametrize("warp_size", WAVE_SIZES)
def test_candidate_set_is_the_exact_declared_product(warp_size):
    """The bounded Cartesian product of the two declared axes, nothing added or dropped:
    {64,128,256,512} threads x {no override, 1, 2} waves_per_eu."""
    expected = [(bt, wpe) for bt in (64, 128, 256, 512) for wpe in (None, 1, 2)]
    assert _identities(candidate_configs(warp_size)) == expected
...
def test_compatibility_default_is_always_retained(warp_size):
    ...
    assert (BLOCK_THREADS, None) in _identities(candidate_configs(warp_size))
...
def test_every_generated_candidate_is_legal(warp_size):
    assert all(is_legal(bt, warp_size) for bt, _ in _identities(candidate_configs(warp_size)))
...
def test_mixed_path_set():
    ...
    assert [uses_fast_path(2048, "bf16", bt) for bt in (64, 128, 256, 512)] == [True, True, True, False]
...
def _key(monkeypatch, *, m_in=64, N=4096, dtype_str="bf16", tuning_schema=TUNING_SCHEMA, arch="gfx950"):
    ...
    kwargs = {"N": N, "dtype_str": dtype_str, "tuning_schema": tuning_schema, "stream": None}
...
def test_default_serving_never_searches(monkeypatch):
    ...
    monkeypatch.setattr(_softmax_tuner, "configs", lambda *a, **k: pytest.fail("unexpected candidate generation"))
    served = []
    monkeypatch.setattr(_softmax_tuner, "_run_config", lambda config, args, kwargs: served.append(config))
...
def test_search_space_is_the_shared_candidate_owner():
    ...
    assert _identities(configs) == _identities(candidate_configs())
