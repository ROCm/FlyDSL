...
def test_every_replay_path_takes_its_abi_from_the_artifact():
    ...
    CallState from an artifact it did not just compile must pass ktrace_enabled, and must
    source it from that artifact rather than from the environment.

    Checks the invariant, not one spelling of it: each call site is required to pass the
    argument and to read it off an artifact, so the assertion survives a rename or a
    refactor that changes how the value is routed.
    """
    import inspect

    source = inspect.getsource(jit_function.JitFunction.__call__)
    calls = [ln for ln in source.splitlines() if "ktrace_enabled=" in ln]
    assert calls, "no _build_call_state call sites pass ktrace_enabled -- test is stale"

    # Every _build_call_state in this method must be handed the decision. Relying on the
    # default (None) means "re-derive from the env", which cannot see a ktrace compile
    # hint because the compile_hints scope has already closed.
    n_builds = source.count("_build_call_state(")
    assert n_builds == len(calls), (
        f"{n_builds} _build_call_state call(s) but only {len(calls)} pass ktrace_enabled: "
        "a replay path falls back to an env read"
    )

    # And the value must come from the artifact, not from the ambient environment.
    for ln in calls:
        assert "_ktrace_traced" in ln, f"ktrace_enabled is not sourced from an artifact: {ln.strip()}"
