...
def _compile_tdm_mcast_add(grid_m, grid_n, cluster_m, cluster_n):
    ...
    def tdm_mcast_add_kernel(
        ...
        bx_idx = gpu.block_id("x")
        by_idx = gpu.block_id("y")
        ...
        # --- TDM descriptors ---
        blk_m = bx_idx * arith.index(TILE)
        blk_n = by_idx * arith.index(TILE)
        ...
        bid = fx.block_idx.y * grid_n + fx.block_idx.x
        tid = fx.thread_idx.x
        ...
        base_elem = tid_idx * arith.index(ELEMS_PER_THREAD)
...
def _run_tdm_mcast_add(grid_m, grid_n, cluster_x, cluster_y, n_warmup=0, n_iters=1):
    ...
    c_dev = torch.zeros((M, N), device="cuda", dtype=torch.bfloat16)
    ...
    # Reference: C[bx*T:(bx+1)*T, by*T:(by+1)*T] = A[bx*T:(bx+1)*T, :] + B[:, by*T:(by+1)*T]
    ...
    for bx in range(grid_m):
        ...
        for by in range(grid_n):
            ...
            c_ref[bx * TILE : (bx + 1) * TILE, by * TILE : (by + 1) * TILE] = a_tile + b_tile
