...
@pytest.mark.parametrize(
    "arch,a_start,b_start,c_start,c_bank",
    [
        ("gfx942", 0, 64, 64, "VGPR"),
        ("gfx942", 32, 128, 128, "VGPR"),
        ("gfx950", 0, 64, 64, "VGPR"),
...
example.MMA_REG_C = (example.fx.rocdl.{c_bank}, {c_start})
example.main()
"""
...
def _compile_machine_module(
    tmp_path, body, *, vgpr_limit=None, opt_level=3, strip_placement=False, declarations="", arch="gfx942"
):
    attrs = f', passthrough = [["amdgpu-num-vgpr", "{vgpr_limit}"]]' if vgpr_limit else ""
    source = f"""module {{
      gpu.module @m [#rocdl.target<O = {opt_level}, chip = "{arch}">] {{
        {declarations}
...
with _create_mlir_context(), ir.Location.unknown():
    ...
    PassManager.parse("builtin.module(reconcile-unrealized-casts,convert-fly-to-rocdl,fly-serialize-register-kernels{{format=isa}},gpu-module-to-binary{{format=isa}})").run(module.operation)
