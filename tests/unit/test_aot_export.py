...
"""AOT compile + C object export: ``flyc.compile_aot(...).export_to_c(...)``.
...
out = Path(sys.argv[1])
compiled = flyc.compile_aot(axpy, _ptr(), _ptr(), 256, 1.0)
assert compiled.arch == "gfx950"
...
# ---------------------------------------------------------------------------
# compile_aot: ABI model
# ---------------------------------------------------------------------------


@pytest.mark.l0_backend_agnostic
def test_compile_aot_rejects_non_jit():
    with pytest.raises(TypeError, match="@flyc.jit"):
        flyc.compile_aot(lambda: None)
...
class TestCompileAot:
    def test_pointer_scalar_explicit_stream(self):
        compiled = flyc.compile_aot(axpy, _ptr(), _ptr(), 8, 1.0, fx.Stream(None))
    ...
    def test_implicit_stream_and_constexpr_removed(self):
        compiled = flyc.compile_aot(add_one_then_scale, _ptr(), 16, factor=3.0)
    ...
    def test_dynamic_tensor_layout_widths_from_lowered_abi(self):
        ...
        compiled = flyc.compile_aot(tensor_add, t, t, t)
    ...
    def test_32bit_stride_layout(self):
        ...
        compiled = flyc.compile_aot(tensor_add, a, t, t)
    ...
    def test_static_tensor_layout_has_no_layout_slot(self):
        ...
        compiled = flyc.compile_aot(tensor_add, s, s, s)
        assert [slot.kind for slot in compiled.abi] == ["tensor_data"] * 3 + ["stream"]

    def test_kwargs_defaults_and_methods(self):
        by_kw = flyc.compile_aot(axpy, _ptr(), Y=_ptr(), n=4, alpha=2.0)
        positional = flyc.compile_aot(axpy, _ptr(), _ptr(), 4, 2.0, fx.Stream(None))
        ...
        obj = _Scaler()
        bound = flyc.compile_aot(obj.launch, _ptr(), 4)
        unbound = flyc.compile_aot(_Scaler.launch, obj, _ptr(), 4)
    ...
    def test_struct_argument_expands_runtime_fields(self):
        compiled = flyc.compile_aot(axpy_struct, _ptr(), _ptr(), _AxpyParams(n=8, alpha=1.0, block=128))
    ...
    def test_does_not_dispatch_or_create_engine(self, monkeypatch):
        ...
        def boom(*_a, **_k):
            raise AssertionError("compile_aot must not execute anything")
        ...
        flyc.compile_aot(axpy, _ptr(), _ptr(), 8, 1.0)
    ...
    def test_matches_jit_lowering_including_compile_hints(self, monkeypatch):
        ...
        aot_ir = flyc.compile_aot(hinted, _ptr(), _ptr(), 4)._ir_text
        # The hint changes the GPU binary, so the comparison below is meaningful.
        assert flyc.compile_aot(plain, _ptr(), _ptr(), 4)._ir_text != aot_ir
    ...
    def test_unsupported_argument_reports_name_and_type(self):
        ...
        with pytest.raises(NotImplementedError, match=r"'x'.*Opaque"):
            flyc.compile_aot(takes_opaque, Opaque())
...
class TestExport:
    ...
    @classmethod
    def compiled(cls):
        return flyc.compile_aot(axpy, _ptr(), _ptr(), 8, 1.0)
    ...
    def test_header_matches_abi(self, tmp_path):
        compiled = flyc.compile_aot(scalar_echo, _ptr(), *[t(1) for t in _SCALAR_TYPES])
        ...
        t = torch.empty_strided((2, 3), (5, 1), device="meta")
        tensor = flyc.compile_aot(tensor_add, t, t, t).export_to_c(
            tmp_path / "t.o", "tadd", header_file_path=tmp_path / "t.h"
        )
    ...
    def test_header_parameter_names_avoid_c_keywords(self, tmp_path):
        ...
        compiled = flyc.compile_aot(keywords, _ptr(), 4, 1.0, 0, 0)
...
class TestEndToEnd:
    ...
    def test_pointer_scalar_explicit_stream(self, tmp_path):
        k = _export_and_load(tmp_path, flyc.compile_aot(axpy, _ptr(), _ptr(), 8, 1.0, fx.Stream(None)), "e2e_axpy")
    ...
    def test_implicit_stream_multi_kernel(self, tmp_path):
        k = _export_and_load(tmp_path, flyc.compile_aot(add_one_then_scale, _ptr(), 8, factor=3.0), "e2e_two")
        assert k.init() == 0 and k.load(-1) == 0
    ...
    def test_dynamic_non_contiguous_tensor(self, tmp_path):
        t = torch.empty_strided((2, 3), (5, 1), device="meta")
        k = _export_and_load(tmp_path, flyc.compile_aot(tensor_add, t, t, t), "e2e_tensor")
        assert k.init() == 0 and k.load(-1) == 0
    ...
    def test_scalar_abi_values(self, tmp_path):
        k = _export_and_load(
            tmp_path, flyc.compile_aot(scalar_echo, _ptr(), *[t(1) for t in _SCALAR_TYPES]), "e2e_echo"
        )
        assert k.init() == 0 and k.load(-1) == 0
    ...
    def test_status_propagation(self, tmp_path):
        k = _export_and_load(tmp_path, flyc.compile_aot(add_one_then_scale, _ptr(), 8), "e2e_status")
    ...
    def test_two_artifacts_in_one_library(self, tmp_path):
        first = flyc.compile_aot(add_one_then_scale, _ptr(), 8, factor=2.0)
        second = flyc.compile_aot(add_one_then_scale, _ptr(), 8, factor=10.0)
    ...
    def test_concurrent_launches(self, tmp_path):
        k = _export_and_load(tmp_path, flyc.compile_aot(axpy, _ptr(), _ptr(), 8, 1.0, fx.Stream(None)), "e2e_mt")
        assert k.init() == 0
    ...
    def test_unload_racing_with_launches(self, tmp_path):
        ...
        k = _export_and_load(tmp_path, flyc.compile_aot(add_one_then_scale, _ptr(), 8), "e2e_race")
    ...
    def test_extern_bitcode_launcher(self, tmp_path):
        ...
        k = _export_and_load(tmp_path, flyc.compile_aot(triple_launcher, _ptr(), 8), "e2e_extern")
        assert k.init() == 0 and k.load(-1) == 0
    ...
    def test_two_devices(self, tmp_path):
        k = _export_and_load(tmp_path, flyc.compile_aot(add_one_then_scale, _ptr(), 8), "e2e_devs")
        assert k.init() == 0
