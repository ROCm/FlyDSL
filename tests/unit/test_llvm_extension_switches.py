...
def test_every_shipped_extension_satisfies_the_rule():
    """Anything that is not a required patch must carry a switch."""
...
def test_required_patches_are_read_from_the_build_script():
    """The exemption list is the build's own array, not a second copy of it.

    A separate list would drift: a patch could be exempt here while the build
    treats it as an extension, or the reverse.
    """
    ext_dir = _REPO_ROOT / "thirdparty" / "llvm-extensions"
    on_disk = {p.name for p in ext_dir.glob("*.patch")}
    required = _mod.required_patches()

    assert required, "expected at least one required patch"
