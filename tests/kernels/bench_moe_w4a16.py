"""
Benchmark: FlyDSL vs sglang MoE kernels (W4A16 group-quant, bf16, fp8_bf16, hybrid)
...
Usage:
  ...
  HIP_VISIBLE_DEVICES=4 python -m tests.kernels.bench_moe_w4a16 --fp8-bf16
...
Notes:
  ...
  - fp8_bf16: weights quantized per-channel to fp8, cast back to bf16 for kernel;
    per-channel scale applied in epilogue.  Simulates fp8 weight storage + bf16 compute.
...
def main():
    parser = argparse.ArgumentParser(
        description="Benchmark FlyDSL vs sglang MoE kernels (W4A16 group-quant, bf16, fp8_bf16)",
    ...
    if is_hybrid:
        ...
        if not HAS_SGLANG:
            ...
            sglang_label = "W4A16" if sglang_w4a16 else "bf16"
            print(f"Running sglang {sglang_label}...")
            sglang_ms = benchmark_sglang(
                tokens, model_dim, inter_dim, experts, topk,
                w4a16=sglang_w4a16, group_size=group_size,
                num_warmup=args.num_warmup, num_iters=args.num_iters,
            )
        torch.cuda.empty_cache()
        gc.collect()
        ...
        print(f"{'sglang ' + sglang_label:<20s} {sglang_ms:>8.2f} ms {sglang_tflops:>8.2f}")
