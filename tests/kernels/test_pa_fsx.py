...
def run_pa_gluon_test(
    ...
    if not skip_flydsl:
        ...
        if query_scale_factors is not None and query_scale_factors.numel() == 1:
            ...
            fd_q_scale = query_scale_factors.reshape(1).to(torch.float32)
        else:
            # Query not quantized or per-token — do per-tensor quant
            fd_query, fd_q_scale = per_tensor_quant(query, quant_dtype=aiter.dtypes.fp8)
            fd_q_scale = fd_q_scale.reshape(1).to(torch.float32)
        fd_k_scale = key_scale_original.reshape(1).to(torch.float32) if key_scale_original is not None and key_scale_original.numel() == 1 else torch.tensor([0.0], device=device, dtype=torch.float32)
        fd_v_scale = value_scale_original.reshape(1).to(torch.float32) if value_scale_original is not None and value_scale_original.numel() == 1 else torch.tensor([0.0], device=device, dtype=torch.float32)
        ...
        _, flydsl_time = run_flydsl_kernel(
            flydsl_output,
            fd_query,
            quantized_keys,
            flydsl_value_cache,
