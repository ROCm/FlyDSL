...
Mirrors tests/kernels/test_pa_mqa_logits_fp4_qfp8_kvfp8.py but KV is also
FP4 (host-side FP4-quantized + contiguous-K preshuffle). Both MFMA
operands are native FP4 (cbsz=4, blgp=4); no in-kernel dequant.
"""
...
from kernels.pa_mqa_logits_fp4_qfp4_kvfp4 import (
...
print("[test] using pa_mqa_logits_fp4_qfp4_kvfp4 kernel "
...
def create_paged_preshuffle_kv_fp4(kv_bf16, kv_block_size, num_blocks, block_tables):
    ...
    kv_cache = torch.zeros(num_blocks, k_tiles, 4, kv_block_size, 16,
                           dtype=torch.uint8, device=dev)
    kv_scale = torch.zeros(num_blocks, k_tiles, 4, kv_block_size,
                           dtype=torch.uint8, device=dev)
...
def test_pa_mqa_logits_fp4_qfp4_kvfp4(
    ...
    print("=" * 96)
    print(f"MQA Logits (Q FP4, KV FP4) varctx: batch={batch_size}, heads={heads}, "
    ...
    ref_logits = ref_mqa_logits_mixed(
        q_packed, q_e8m0, kv_fp4_dense, kv_e8m0_dense, weights, context_lens,
        next_n=next_n)
    ...
    _build_kwargs = dict(block_k=block_k, kv_block_size=kv_block_size,
                         ...
                         next_n=next_n, heads=heads, head_dim=head_dim)
    ...
    err_ratio = checkAllclose(valid_ref.float(), valid_out.float(),
                              rtol=0.05, atol=0.05,
                              msg="flydsl-qfp4-kvfp4 vs ref", printLog=False)
    ...
    kv_dq_bf16 = fp4_dequant_e2m1_with_e8m0(
        kv_fp4_dense, kv_e8m0_dense).to(torch.bfloat16)
    ...
    _PERF_SUMMARY.append((
        batch_size, heads, head_dim, max_ctx, next_n, block_k, cos.item(),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--heads", type=int, default=HEADS,
                        help=f"Number of Q heads (multiple of 16, <= 64). Default {HEADS}.")
    parser.add_argument("--head_dim", type=int, default=HEAD_DIM,
                        help=f"Per-head dim (multiple of 128). Default {HEAD_DIM}.")
