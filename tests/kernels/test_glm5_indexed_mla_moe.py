...
"""GLM-5 indexed sparse MLA+MoE layer monokernel vs the torch golden.

Single GPU (TP1 view of one shard, the peer reduce is a 1-rank loopback)::

    python3 tests/kernels/test_glm5_indexed_mla_moe.py --npes 1

TP8, one process per GPU::

    python3 tests/kernels/test_glm5_indexed_mla_moe.py --npes 8
"""
...
def _check(name, got, ref, report, fp8_flips=False):
...
def run_rank(rank, npes, S, cur_pos, iters, group=None, seed=1234, moe_mode=MoeMode.W8A8):
    ...
    sparse_indices = torch.stack(
        [
            torch.randperm(max(cur_pos + s + 1, sparse_attention_topk), generator=gen, device=dev)[
    ...
    if cur_pos + 1 > sparse_attention_topk:  # the reused selection always holds the newest token
        sparse_indices[:, -1] = torch.arange(cur_pos, cur_pos + S, device=dev, dtype=torch.int32)
    op = Glm5IndexedMlaMoeBlock(
        W,
        S,
        rank=rank,
        npes=npes,
    ...
    for it in range(iters):
        ...
        out = op.forward(h, pos_t, kv, pe, sparse_indices, cos, sin)
        ...
        # ordered FP32 sum. Model that communication contract here;
...
def bench_rank(rank, npes, S, cur_pos, iters=320, group=None, seed=1234, moe_mode=MoeMode.W8A8):
    ...
    sparse_indices = torch.stack(
        [torch.randperm(max(cur_pos + s + 1, 2048), device=dev)[:2048] for s in range(S)]
    ).int()
    op = Glm5IndexedMlaMoeBlock(
        W,
        S,
    ...
    pos_t = torch.tensor([cur_pos], dtype=torch.int32, device=dev)
    for _ in range(10):
        op.forward(h, pos_t, kv, pe, sparse_indices, cos, sin, x_out=x)
    torch.cuda.synchronize()
    import torch.distributed as dist
    ...
    if os.environ.get("MLA_MOE_TIMELINE"):
        ...
        for _ in range(3):
            top.forward(h, pos_t, kv, pe, sparse_indices, cos, sin, x_out=x)
        torch.cuda.synchronize()
        if rank == 0:
    ...
    with torch.cuda.graph(graph):
        for layer in range(launches_per_step):
            op.forward(h, pos_t, kv, pe, sparse_indices, cos, sin, x_out=x, layer=layer, advance=False)
        op.advance_step()
    for _ in range(3):
        graph.replay()
    torch.cuda.synchronize()
    ...
    t1.record()
    torch.cuda.synchronize()
    latency = t0.elapsed_time(t1) * 1e3 / (iters // launches_per_step * launches_per_step)
    op.close()
    return latency
