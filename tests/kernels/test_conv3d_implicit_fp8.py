...
# Same 2D-degenerate shapes as test_conv3d_implicit.test_qwenimage_vae_*.
...
def test_qwenimage_vae_t1_res3_fp8(c_in, c_out, h, w):
    torch.manual_seed(8900 + c_in + c_out + h + w)
    x2 = _fp8(torch.randn((1, c_in, h, w), device="cuda", dtype=torch.bfloat16))
    weight5 = _fp8(torch.randn((c_out, c_in, 3, 3, 3), device="cuda", dtype=torch.bfloat16))
    weight2 = weight5[:, :, 2, :, :]
    bias = torch.randn((c_out,), device="cuda", dtype=torch.float32)

    y = conv3d_implicit_fp8(x2, weight2, bias=bias, stride=1, padding=1)
    y_ref = F.conv2d(
    ...
    rel = (y.float() - y_ref.float()).abs().mean() / y_ref.float().abs().mean().clamp_min(1e-6)
    assert rel.item() < 2e-2, f"Qwen-Image FP8 rel_err {rel.item():.3e}"


@_skip_no_fp8
@pytest.mark.parametrize("c,h,w", _QWENIMAGE_DOWN2D)
def test_qwenimage_vae_downsample2d_fp8(c, h, w):
    torch.manual_seed(9200 + c + h + w)
    x2 = _fp8(torch.randn((1, c, h, w), device="cuda", dtype=torch.bfloat16))
    x_pad = F.pad(x2, (0, 1, 0, 1))
    weight = _fp8(torch.randn((c, c, 3, 3), device="cuda", dtype=torch.bfloat16))
    bias = torch.randn((c,), device="cuda", dtype=torch.float32)

    y = conv3d_implicit_fp8(x_pad, weight, bias=bias, stride=2, padding=0)
    y_ref = F.conv2d(
        x_pad.to(torch.bfloat16),
        weight.to(torch.bfloat16),
        bias=bias.to(torch.bfloat16),
        stride=2,
        padding=0,
    ...
    assert y.shape == y_ref.shape
    rel = (y.float() - y_ref.float()).abs().mean() / y_ref.float().abs().mean().clamp_min(1e-6)
    assert rel.item() < 2e-2, f"Qwen-Image FP8 downsample rel_err {rel.item():.3e}"


@_skip_no_fp8
@pytest.mark.parametrize("c_in,c_out,h,w", _QWENIMAGE_UP2D)
def test_qwenimage_vae_upsample2d_fp8(c_in, c_out, h, w):
    torch.manual_seed(9500 + c_in + c_out + h + w)
    x2 = _fp8(torch.randn((1, c_in, h, w), device="cuda", dtype=torch.bfloat16))
    weight = _fp8(torch.randn((c_out, c_in, 3, 3), device="cuda", dtype=torch.bfloat16))
    bias = torch.randn((c_out,), device="cuda", dtype=torch.float32)

    y = conv3d_implicit_fp8(x2, weight, bias=bias, stride=1, padding=1)
    y_ref = F.conv2d(
        x2.to(torch.bfloat16),
        weight.to(torch.bfloat16),
        bias=bias.to(torch.bfloat16),
    ...
    assert y.dtype == torch.bfloat16
    rel = (y.float() - y_ref.float()).abs().mean() / y_ref.float().abs().mean().clamp_min(1e-6)
    assert rel.item() < 2e-2, f"Qwen-Image FP8 upsample rel_err {rel.item():.3e}"
