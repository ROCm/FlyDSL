...
Single GPU (TP1 view of one shard, the peer reduce is a 1-rank loopback)::
    ...
    python3 tests/kernels/test_shared_reuse_mla_moe_layer.py --npes 1
...
if _ARCH != "gfx950":
    pytest.skip(f"GLM-5 shared/reuse MLA + MoE requires gfx950, got {_ARCH}", allow_module_level=True)
...
def bench_rank(
    ...
    t1.record()
    torch.cuda.synchronize()
    latency = t0.elapsed_time(t1) * 1e3 / (iters // LAYERS * LAYERS)
    op.close()
    return latency
