...
Kernel implementation lives in `kernels/gemm/gemm_a16w16_gfx1250.py`.
...
from kernels.gemm.gemm_a16w16_gfx1250 import gemm_a16w16  # noqa: E402
...
@pytest.mark.parametrize("num_buffers", [2, 3])
@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16])
def test_gemm_a16w16_compute_bound(M, N, K, num_buffers, dtype):
    """compute_bound variant (in-place fragment rotation): Y = X @ W^T.

    Held to the SAME correctness bar as the default bandwidth_bound path —
    identical reference and tolerance. num_buffers 2 and 3 exercise both the
    nb==2 tensor_wait(0) branch and the nb>2 tensor_wait((nb-2)*2) branch of
    the rotation loop.
    """
    _check_gfx1250()
    ...
    out = gemm_a16w16(x, w, dtype=dtype, num_buffers=num_buffers, variant="compute_bound")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--activation", type=str, default=None, choices=["relu", "silu", "silu_exp2", "gelu", "gelu_tanh"]
    )
    ...
    out = gemm_a16w16(x, w, bias=bias, dtype=dtype, activation=args.activation, num_buffers=args.num_buffers)

    torch.cuda.synchronize()
    passed = verify_output(out.cpu().to(torch.float32), ref.cpu().to(torch.float32), rtol=3e-2, atol=3e-2)
