...
from kernels.mha_pipeline_bf16_gfx1250 import attn_fwd_pipelined_kernel as kernel
from kernels import mha_pipeline_bf16_gfx1250 as mha_mod
...
@flyc.jit
def launch_mha_fa(q, k, v, o, sq_z, sq_h, sq_m, sk_z, sk_h, sk_n, sv_z, sv_h, sv_n, so_z, so_h, so_m, seqlen_k):
    if hasattr(mha_mod, "_lds_allocator"):
        mha_mod._lds_allocator.finalized = False
        ctx = CompilationContext.get_current()
        with ir.InsertionPoint(ctx.gpu_module_body):
            mha_mod._lds_allocator.finalize()
    launcher = kernel(q, k, v, o, sq_z, sq_h, sq_m, sk_z, sk_h, sk_n, sv_z, sv_h, sv_n, so_z, so_h, so_m, seqlen_k)
    mha_mod._set_kernel_attrs()
    launcher.launch(grid=[B, H, (SQ + BLOCK_M - 1) // BLOCK_M], block=[BLOCK_THREADS, 1, 1])

print("FlyDSL MHA kernel:")
print(f"  variant='pipeline' B={B} H={H} SQ={SQ} SK={SK}")
