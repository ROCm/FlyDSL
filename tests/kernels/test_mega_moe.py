...
def _prepare(dev, *, quant, tokens, model_dim, inter_dim, experts, topk, seed, rank=0,
    ...
    w_kernel_gui = gemm_common_utils.shuffle_weight_w4(
        w1_fp4.view(experts, 2 * inter_dim, model_dim // 2),
        NLane=16, gate_up=True, moe_gemm=True).view(torch.uint8).contiguous()
    scale_gui = gemm_common_utils.shuffle_scale_w4(
        w1_scale_raw.view(experts * 2 * inter_dim, model_dim // 32),
        experts_cnt=experts, gate_up=True).view(torch.uint8).contiguous()
    ...
    return dict(
        ...
        scale_w1_1d=scale_w1_1d, w_kernel_gui=w_kernel_gui, scale_gui=scale_gui,
        topk_ids=topk_ids, wts=wts,
...
def _run_full_e2e(args, rank, world, dev, *, model_dim, inter_dim, experts, epr, topk,
                  ...
                  w_kernel_gui=None, scale_gui=None):
    ...
    cfg_a = FlyDSLDispatchCombineConfig(
        ...
        dispatch_dtype=torch.bfloat16, combine_dtype=torch.bfloat16,
...
def run_one(args, rank, world, dev):
    ...
    return _run_full_e2e(
        ...
        w_kernel_gui=T.get("w_kernel_gui"), scale_gui=T.get("scale_gui"))
...
def main():
    ...
    p.add_argument("--mega-gate-mode", type=str, default="auto",
