...
def _build_dispatch_inputs(rank, world_size, dev, args, cfg):
    ...
    if cfg.dispatch_dtype == torch.float4_e2m1fn_x2:
        ...
        ).to(cfg.dispatch_dtype)
    else:
        inp = torch.randn(
            cur_tok, cfg.hidden_dim, dtype=cfg.dispatch_dtype, device=dev,
        )
...
def _build_gemm2_static_inputs(rank, world_size, dev, args, cfg, *, disp_op,
    ...
    #                       expanded layout.
    out_dtype = cfg.combine_dtype if cfg.combine_dtype in (torch.bfloat16, torch.float16) else torch.bfloat16
...
def _build_mori_op(rank, world_size, cfg, block_num=None, warp_per_block=None):
    ...
    if cfg.dispatch_dtype not in _MORI_SUPPORTED_DTYPES:
        raise RuntimeError(
            ...
            f"got cfg.dispatch_dtype={cfg.dispatch_dtype}; rerun with --dtype bf16/fp16/fp8_*."
        )
...
def _snapshot_combine_out(disp_op):
    ...
    out_tok = (
        disp_op.shmem_comb_out_tok.view(torch.int8)[:mt * cfg.combine_token_bytes]
        .view(cfg.combine_dtype).view(mt, cfg.combine_token_view_dim)
    )
