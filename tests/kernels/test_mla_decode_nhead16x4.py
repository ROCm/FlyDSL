#!/usr/bin/env python3
"""Test for MLA decode attention kernel (MFMA 16x16x16, GQA, seqlen_q <= 4).
...
def create_paged_kv(kv_4d, page_block_size, shuffle=False):
    ...
    Returns:
        kv_paged:    [total_blocks, page_block_size, Hkv, D_QK] f16 tensor.
...
def run_config(
    ...
    setup_seed(seed)
    q_4d = torch.randn(B, Sq, Hq, D_QK, dtype=dtype, device=device)
    kv_4d = torch.randn(B, Skv, Hkv, D_QK, dtype=dtype, device=device)

    kv_paged, block_table = create_paged_kv(kv_4d, page_block_size, shuffle=shuffle)

    q_flat = q_4d.contiguous().view(-1)
    kv_paged_flat = kv_paged.contiguous().view(-1)
    ...
    output = torch.empty(total_q, Hq, D_V, dtype=dtype, device=device)
    ...
    out_bytes = total_q * Hq * D_V * elem_bytes
...
def main():
    ...
    print("=" * 110)
    print(f"MLA Decode Attention ({'causal' if causal else 'non-causal'}, fp16)")
