...
"""Correctness test for the FP8 8-wave (fp8_gemm-pipeline) implicit-GEMM conv3d.

The kernel consumes FP8 (E4M3FN) inputs natively and requires the rigid 8-wave
...
from kernels.conv.conv3d_implicit_fp8_gemm8w import conv3d_implicit_fp8_gemm8w
...
def test_conv3d_fp8_gemm8w_vs_reference(n, c, t, h, w, k, kt, kh, kw, stride, padding):
    ...
    y = conv3d_implicit_fp8_gemm8w(x, weight, stride=stride, padding=padding)
    ...
    assert rel.item() < 2e-2, f"FP8 gemm8w conv rel_err {rel.item():.3e} too high vs FP8-cast reference"


@_skip_no_fp8
def test_conv3d_fp8_gemm8w_bias():
    ...
    y = conv3d_implicit_fp8_gemm8w(x, weight, bias=bias, stride=1, padding=(0, 1, 1))
    ref = F.conv3d(
    ...
    assert y.shape == ref.shape
    rel = (y.float() - ref.float()).abs().mean() / ref.float().abs().mean().clamp_min(1e-6)
    assert rel.item() < 2e-2, f"FP8 gemm8w conv+bias rel_err {rel.item():.3e}"


# 2D via the depth-1 wrapper (aligned: c=256, k=256, 3x3 -> crs=2304).
@_skip_no_fp8
def test_conv2d_fp8_gemm8w():
    ...
    y = conv3d_implicit_fp8_gemm8w(x, weight, padding=1)
    ref = F.conv2d(x.to(torch.bfloat16), weight.to(torch.bfloat16), padding=1)
    torch.cuda.synchronize()
    assert y.shape == ref.shape
    rel = (y.float() - ref.float()).abs().mean() / ref.float().abs().mean().clamp_min(1e-6)
    assert rel.item() < 2e-2, f"FP8 gemm8w conv2d rel_err {rel.item():.3e}"
