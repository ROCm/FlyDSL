...
def test_fmoe(
    ...
    if use_flir:
        ...
        tile_m = 64 if token > 1024 else 32 if token > 256 else 16
        tile_n1 = 128
        tile_k1 = 128
        tile_n2 = 256
        tile_k2 = 128
        ...
        # Allow overriding stage2 output dtype for perf comparisons:
        #   - FLIR_MOE_STAGE2_OUT_DTYPE=bf16 (default when dtype==bf16)
        #   - FLIR_MOE_STAGE2_OUT_DTYPE=f32  (scalar f32 atomics, slower but different behavior)
        stage2_out_override = os.environ.get("FLIR_MOE_STAGE2_OUT_DTYPE", "").strip().lower()
        if stage2_out_override not in ("", "bf16", "f16", "f32"):
            raise ValueError(
                f"FLIR_MOE_STAGE2_OUT_DTYPE must be one of '', 'bf16', 'f16', 'f32' (got {stage2_out_override!r})"
            )
        if stage2_out_override:
            stage2_out_dtype = stage2_out_override
        else:
            stage2_out_dtype = ("bf16" if dtype == torch.bfloat16 else "f16")

        # Optional experiment: avoid atomics by writing per-(token,slot) output and reducing over topk outside.
        # FLIR_MOE_STAGE2_ACCUMULATE=0 enables scatter stores (no atomics) into [token, topk, model_dim].
        _acc_env = os.environ.get("FLIR_MOE_STAGE2_ACCUMULATE", "1").strip()
        if _acc_env.isdigit():
            stage2_accumulate = int(_acc_env) != 0
        else:
            stage2_accumulate = _acc_env.lower() not in ("0", "false", "no")
        ...
        exe2 = compile_moe_gemm2(
            ...
            in_dtype="fp8",
            out_dtype=stage2_out_dtype,
            accumulate=bool(stage2_accumulate),
        )
        w2_flat = w2_shuf.contiguous().view(E * model_dim, inter_dim).contiguous()
        w2_scale_1d = w2_scale.view(-1).contiguous()
        # Kernel output buffer matches stage2 out_dtype and accumulation mode.
        if stage2_accumulate:
            out2_kernel = torch.empty(
                (token, model_dim),
                device="cuda",
                dtype=(torch.float32 if stage2_out_dtype == "f32" else dtype),
            )
            out2_reduce = None
        else:
            # Scatter output: [token, topk, model_dim] flattened as [(token*topk), model_dim]
            out2_kernel = torch.empty((token, topk, model_dim), device="cuda", dtype=dtype)
            out2_reduce = torch.empty((token, model_dim), device="cuda", dtype=dtype)
        ...
        out2_ck, us2_stage2 = run_perftest(launch_stage2, num_iters=num_iters, num_warmup=num_warmup)
        us2_reduce = 0.0
        if not stage2_accumulate:
            assert out2_reduce is not None

            def launch_reduce():
                # Use out= to avoid per-iter allocations.
                torch.sum(out2_ck, dim=1, out=out2_reduce)
                return out2_reduce

            _, us2_reduce = run_perftest(
                launch_reduce, num_iters=num_iters, num_warmup=num_warmup
            )

        us2 = us1 + us2_stage2 + us2_reduce
        print(
            f"[aiter] flir_moe_gemm1: {us1:8.2f} us, flir_moe_gemm2: {us2_stage2:8.2f} us, "
            f"reduce_topk: {us2_reduce:8.2f} us (accumulate={int(stage2_accumulate)})"
        )
        # Return output in the requested dtype (bf16/f16). Cast is outside timing.
        if not stage2_accumulate:
            out2_ck = out2_reduce
    ...
    mode = "flir" if use_flir else "ck"
    err = checkAllclose(
        out2_ref,
        out2_ck,
        msg=f"{mode}_moe_2stages:{us2:>8.2f} us, {token*model_dim*inter_dim*3*topk*2/us2/1000/1000:>8.2f} tflops......(quant:{AQDType})",
    )
