...
def _bench_us_cuda_events(fn, *, num_iters: int, num_warmup: int) -> float:
    """Return average latency in microseconds using CUDA events (works on ROCm)."""
    ...
    torch.cuda.synchronize()
    # elapsed_time: milliseconds between two recorded events
    ms = start.elapsed_time(end) / float(num_iters)
    return ms * 1000.0


def _bench_us_cudagraph_replay(make_graph, *, num_iters: int, num_warmup: int) -> float:
    """Return avg latency (us) of CUDAGraph replay measured by CUDA events.

    `make_graph()` must return a captured `torch.cuda.CUDAGraph` ready for replay.
    """
    num_iters = int(num_iters)
    num_warmup = int(num_warmup)
    if num_iters <= 0:
...
def test_perf_torch_vs_flydsl_bf16_gemm_8k_cubed():
    """Perf-only benchmark: compare FlyDSL preshuffle GEMM vs torch.mm on 8192^3.

    Disabled by default since it is long-running and uses lots of VRAM.
    Enable with: FLYDSL_BENCH_TORCH_8K=1
    """
    if os.environ.get("FLYDSL_BENCH_TORCH_8K", "0") not in ("1", "true", "True", "YES", "yes"):
        pytest.skip("Set FLYDSL_BENCH_TORCH_8K=1 to enable 8k^3 torch-vs-flydsl perf comparison.")
    ...
    torch.manual_seed(0)
    a = torch.randn(M, K, dtype=torch_in_dtype, device=device)
    b = torch.randn(N, K, dtype=torch_in_dtype, device=device)

    # FlyDSL expects preshuffled B.
    b_input = shuffle_weight(b.contiguous(), layout=(16, 16)).contiguous().view(-1)
    sa_flat = torch.empty(0, dtype=torch.float32, device=device)
    sb_flat = torch.empty(0, dtype=torch.float32, device=device)
    c_out = torch.empty(M, N, dtype=torch_out_dtype, device=device)
    ...
    compiled_fn = flyc.compile(launch_fn, *_args())

    def _flydsl():
        compiled_fn(*_args())

    def _torch():
        # Match the reference path used elsewhere in this file: accumulate in fp32.
        # (torch may pick a fast kernel; we keep output bf16 to reduce bandwidth.)
        out = torch.mm(a.to(torch.float32), b.to(torch.float32).T).to(torch_out_dtype)
        # Prevent dead-code elimination (shouldn't happen, but keep it explicit).
        c_out.view(-1)[:1].copy_(out.view(-1)[:1])

    def _torch_scaled_mm():
        # Optional: scaled GEMM path (fp8/int8 style). Not available in all torch builds.
        scaled_mm = getattr(torch, "_scaled_mm", None)
        if scaled_mm is None:
            raise RuntimeError("torch._scaled_mm not available in this torch build")
        # Use unity scales so numerically it matches mm (modulo internal kernel differences).
        # This torch build expects RowWise scaling: scale_a=(M,1), scale_b=(1,N) when B is (K,N).
        sa = torch.ones((M, 1), device=device, dtype=torch.float32)
        sb = torch.ones((1, N), device=device, dtype=torch.float32)
        out = scaled_mm(a, b.T, sa, sb, out_dtype=torch_out_dtype)
        c_out.view(-1)[:1].copy_(out.view(-1)[:1])

    # Use fewer iters by default; 8k^3 is expensive.
    bench_iters = int(os.environ.get("FLYDSL_BENCH_ITERS_8K", "10"))
    bench_warmup = int(os.environ.get("FLYDSL_BENCH_WARMUP_8K", "3"))

    fly_us = _bench_us_cuda_events(_flydsl, num_iters=bench_iters, num_warmup=bench_warmup)
    torch_us = _bench_us_cuda_events(_torch, num_iters=max(2, bench_iters // 2), num_warmup=bench_warmup)
    torch_scaled_us = None
    try:
        torch_scaled_us = _bench_us_cuda_events(
            _torch_scaled_mm, num_iters=max(2, bench_iters // 2), num_warmup=bench_warmup
        )
    except Exception as e:
        print(f"[8k^3 bf16] torch scaled_mm skipped: {str(e).splitlines()[0] if str(e) else repr(e)}")

    flops = 2 * M * N * K
    fly_tflops = flops / (fly_us / 1e6) / 1e12
    torch_tflops = flops / (torch_us / 1e6) / 1e12
    torch_scaled_tflops = flops / (torch_scaled_us / 1e6) / 1e12 if torch_scaled_us is not None else None

    print("=" * 80)
    print(f"[8k^3 bf16] M=N=K={M}, tile={tile_m}x{tile_n}x{tile_k}, arch={arch}")
    print(f"torch.mm (fp32 accum -> bf16 out): {torch_us:.1f} us  {torch_tflops:.2f} TFLOPS")
    if torch_scaled_us is not None:
        print(f"torch._scaled_mm (unity scales):   {torch_scaled_us:.1f} us  {torch_scaled_tflops:.2f} TFLOPS")
    print(f"FlyDSL preshuffle GEMM:            {fly_us:.1f} us  {fly_tflops:.2f} TFLOPS")
    print(f"Speedup (FlyDSL / torch):          {torch_us / fly_us:.3f}x")
    print("=" * 80)


@pytest.mark.large_shape
def test_perf_torch_scaled_mm_vs_flydsl_fp8_gemm_8k_async():
    """Perf-only benchmark: compare FlyDSL fp8 preshuffle GEMM (async copy) vs torch._scaled_mm on 8192^3.

    Disabled by default since it is long-running and uses lots of VRAM.
    Enable with: FLYDSL_BENCH_FP8_8K_ASYNC=1
    """
    if os.environ.get("FLYDSL_BENCH_FP8_8K_ASYNC", "0") not in ("1", "true", "True", "YES", "yes") and os.environ.get(
        "FLYDSL_BENCH_FP8_ASYNC_SCALEDMM_COMPARE", "0"
    ) not in ("1", "true", "True", "YES", "yes"):
        pytest.skip(
            "Set FLYDSL_BENCH_FP8_ASYNC_SCALEDMM_COMPARE=1 (or legacy FLYDSL_BENCH_FP8_8K_ASYNC=1) "
            "to enable fp8 async scaled_mm comparison."
        )
    ...
    scaled_mm = getattr(torch, "_scaled_mm", None)
    if scaled_mm is None:
        pytest.skip("torch._scaled_mm not available in this torch build")

    device = "cuda:0"
    M = int(os.environ.get("FLYDSL_BENCH_M", "8192"))
    N = int(os.environ.get("FLYDSL_BENCH_N", "8192"))
    K = int(os.environ.get("FLYDSL_BENCH_K", "8192"))
    # Default tile matches scripts/run_benchmark.sh async sweep defaults.
    tile_m = int(os.environ.get("FLYDSL_BENCH_TILE_M", "128"))
    tile_n = int(os.environ.get("FLYDSL_BENCH_TILE_N", "256"))
    tile_k = int(os.environ.get("FLYDSL_BENCH_TILE_K", "128"))
    waves_per_eu = int(os.environ.get("FLYDSL_BENCH_WAVES_PER_EU", "2"))
    in_dtype = "fp8"
    out_dtype = "bf16"
    torch_out_dtype = torch.bfloat16
    ...
    b_fp32_t = torch.randn(N, K, dtype=torch.float32, device=device)

    # Quantize to fp8 with per-token scaling (row-wise, length=M/N).
    a_q, scale_a = pertoken_quant(a_fp32, quant_dtype=DTYPE_FP8)
    b_q, scale_b = pertoken_quant(b_fp32_t, quant_dtype=DTYPE_FP8)
    a_q = a_q.contiguous()
    b_q = b_q.contiguous()

    # Precompute row-wise scales in the format expected by torch._scaled_mm.
    # When B is (K, N), torch expects: scale_a=(M,1), scale_b=(1,N).
    sa_2d = scale_a.to(torch.float32).contiguous().view(M, 1)
    sb_2d = scale_b.to(torch.float32).contiguous().view(1, N)

    # FlyDSL expects preshuffled B (int8 view for fp8 storage).
    b_shuffled = shuffle_weight(b_q, layout=(16, 16))
    b_input = b_shuffled.contiguous().view(torch.int8).view(-1)
    a_input = a_q.contiguous().view(torch.int8).view(-1)
    ...
    # Capture both paths under the same stream to avoid CPU overhead.
    ...
    compiled_fn = flyc.compile(launch_fn, *_fly_args())

    def _flydsl():
        compiled_fn(*_fly_args())

    def _make_graph_flydsl():
        g = torch.cuda.CUDAGraph()
        with torch.cuda.stream(capture_stream):
            # Warmup on capture stream (outside capture) to ensure kernels are initialized.
            _flydsl()
    ...
    def _make_graph_torch():
        g = torch.cuda.CUDAGraph()
        with torch.cuda.stream(capture_stream):
            _ = scaled_mm(a_q, b_q.T, sa_2d, sb_2d, out_dtype=torch_out_dtype)
        torch.cuda.current_stream().wait_stream(capture_stream)
        torch.cuda.synchronize()
    ...
    # Measure graph replay latency (kernel-dominant, minimal CPU overhead).
    fly_us = _bench_us_cudagraph_replay(_make_graph_flydsl, num_iters=bench_iters, num_warmup=bench_warmup)
    torch_us = _bench_us_cudagraph_replay(
        _make_graph_torch, num_iters=max(2, bench_iters // 2), num_warmup=bench_warmup
    )
    ...
    print("=" * 80)
    print(
        f"[fp8 async scaled_mm compare] M={M}, N={N}, K={K}, "
        f"tile={tile_m}x{tile_n}x{tile_k}, wpe={waves_per_eu}, arch={arch}"
    )
    print(f"torch._scaled_mm (fp8 -> bf16 out): {torch_us:.1f} us  {torch_tflops:.2f} TFLOPS")
    print(f"FlyDSL preshuffle GEMM (fp8 async): {fly_us:.1f} us  {fly_tflops:.2f} TFLOPS")
    print(f"Speedup (FlyDSL / torch):           {torch_us / fly_us:.3f}x")
    print("=" * 80)
