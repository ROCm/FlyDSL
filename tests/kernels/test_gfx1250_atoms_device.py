...
These exercise the new high-level atom APIs end-to-end on hardware:
  * ``make_tiled_tdm_atom`` + ``tdm_partition`` + ``fx.copy`` (Global<->LDS TDM DMA)
...
def _compile_tdm_roundtrip(M: int, N: int, num_warps: int):
    ...
    def tdm_roundtrip_kernel(A: fx.Tensor, C: fx.Tensor):
        ...
        smem = fx.make_view(lds.ptr, fx.make_layout(((M, N), 1), ((N, 1), M * N)))

        # Whole-tile Global->LDS->Global via the new tiled-TDM atoms; num_warps waves split the tile.
        load_atom, coordA = fx.rocdl.cdna5.make_tiled_tdm_atom(
            fx.rocdl.TensorLoad(), A, box, (M, N), num_warps=num_warps
        )
        store_atom, coordC = fx.rocdl.cdna5.make_tiled_tdm_atom(
            fx.rocdl.TensorStore(), C, box, (M, N), num_warps=num_warps
        )
        ...
        # Warp selection preserves ((ATOM, ITER), rest) for every warp count.
