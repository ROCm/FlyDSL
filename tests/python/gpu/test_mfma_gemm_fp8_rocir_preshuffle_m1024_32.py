...
# Allow running this file directly (`python tests/python/gpu/...py`).
# We need:
# - repo root: so `from tests.*` imports resolve
# - (optional fallback) embedded build python packages: for environments where `pyflir` isn't installed yet
_repo_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", ".."))
if _repo_root not in sys.path:
    sys.path.insert(0, _repo_root)

# Prefer the normal editable install path (`pip install -e .`).
# If `pyflir.dialects` isn't importable (common before install), fall back to the
# embedded build outputs under `.flir/build/python_packages`.
try:
    import pyflir.dialects  # noqa: F401
except Exception:
    _build_python_packages = os.path.join(_repo_root, ".flir", "build", "python_packages")
    _pyflir_pkg = os.path.join(_build_python_packages, "pyflir")
    _mlir_pkg = os.path.join(_build_python_packages, "_mlir")
    for _p in (_mlir_pkg, _pyflir_pkg):
        if os.path.isdir(_p) and _p not in sys.path:
            sys.path.insert(0, _p)

import pyflir as rocdsl
import pyflir.dialects.ext.flir as rocir
