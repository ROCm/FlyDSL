...
def test_lane_operations():
    ...
    class _M(flir.MlirModule):
        ...
        def lane_test(self: flir.T.i64):
            ...
            rocdl.readlane(i32, arith_ext.unwrap(src), arith_ext.unwrap(lane))
            rocdl.readfirstlane(i32, arith_ext.unwrap(src))
            offset = arith_ext.constant(0x1F, type=i32)
            rocdl.ds_swizzle(i32, arith_ext.unwrap(src), arith_ext.unwrap(offset))
            return []
