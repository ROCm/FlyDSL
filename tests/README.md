...
| Run a hand-edited `.s` instead of generated ISA | `FLYDSL_HACK_UT_ASM` (path to a dumped/edited `.s`; debug/UT only, bypasses the disk cache; only the kernels the `.s` names are overridden) |
| Keep the code object assembled from that `.s` | `FLYDSL_HACK_UT_ASM_SAVE` (`1` writes it next to the `.s` as `.hsaco`; off by default) |
