...
from coop_warp_movement_utils import (
    check_compile_movement_policies,
    check_guarded_cross_dtype_io,
    check_guarded_io,
    check_movement_io_policies,
)
...
def test_guarded_io(
    ...
    check_guarded_io(policy, valid, block, count, entry, universal, width, dynamic)
...
def test_runtime_valid_items(policy, valid):
    check_guarded_io(policy, valid, dynamic=True)
...
def test_warp_valid_items_are_per_warp(policy, valid):
    ...
    def apply(a, out):
        ...
        items = warp_load(
            a, count, width=width, offset=warp_offset, algorithm=policy, valid_items=fx.Int32(valid), default=-7
        )
        warp_store(
            out,
            items,
            width=width,
            offset=warp_offset,
            algorithm=WarpStoreAlgorithm[policy.name],
...
def test_movement_io_policies(warp, policy, count, valid):
    check_movement_io_policies(warp, policy, count, valid)
...
def test_io_dtypes(entry, count, policy, universal):
    check_guarded_io(policy, 128 * count - 1, block=128, count=count, entry=dtype_entry(entry), universal=universal)
...
def test_tile_addresses(policy, addressing, scope, universal):
    ...
    def apply(a, out):
        ...
        if addressing == "view":
            ...
            src, dst = a, out
        items = ns.warp_load(
            src, count, width=width, offset=source_offset, algorithm=policy, valid_items=valid, default=-7
        )
        ns.warp_store(
            dst,
            items,
