...
# FlyDSL PA decode import (optional) — sliding-window variant
try:
    sys.path.insert(0, '/FlyDSL')
    from kernels.pa_decode_sw_fp8 import (
        pa_decode_sw_launch as flydsl_pa_decode_launch,
        QUERY_GROUP_SIZE as FLYDSL_QGS,
        HEAD_SIZE as FLYDSL_HS,
    )
    HAS_FLYDSL = False
    print("FlyDSL PA decode (sliding window) available")
except ImportError as e:
    HAS_FLYDSL = False
    FLYDSL_HS = 128
    FLYDSL_QGS = 16
    print(f"Warning: FlyDSL PA decode not available: {e}")

# FlyDSL PS (Persistent Scheduling) kernel import
try:
    from kernels.pa_decode_sw_fp8_ps import (
        pa_decode_ps_launch as flydsl_ps_launch,
        get_pa_metadata as flydsl_get_pa_metadata,
    )
    HAS_FLYDSL_PS = True
    print("FlyDSL PA decode PS (persistent scheduling) available")
except ImportError as e:
    HAS_FLYDSL_PS = False
    print(f"Warning: FlyDSL PA decode PS not available: {e}")

# FlyDSL Tile PA decode import (f16) -- DISABLED: replaced by pa_decode_tile_sw
try:
    from kernels.pa_decode_tile import pa_decode_tile_launch as flydsl_tile_launch
    HAS_FLYDSL_TILE = False  # DISABLED: use pa_decode_tile_sw instead
    print("FlyDSL PA decode tile (f16) available")
except ImportError as e:
    HAS_FLYDSL_TILE = False
    print(f"Warning: FlyDSL PA decode tile not available: {e}")

# FlyDSL Tile PA decode import (bf16) -- DISABLED: replaced by pa_decode_tile_sw
try:
    from kernels.pa_decode_tile_bf16 import pa_decode_tile_launch as flydsl_tile_bf16_launch
    HAS_FLYDSL_TILE_BF16 = False  # DISABLED: use pa_decode_tile_sw instead
    print("FlyDSL PA decode tile (bf16) available")
except ImportError as e:
    HAS_FLYDSL_TILE_BF16 = False
    print(f"Warning: FlyDSL PA decode tile bf16 not available: {e}")

# FlyDSL Tile PA decode import (unified f16/bf16 with sliding window)
try:
    from kernels.pa_decode_tile_sw import pa_decode_tile_sw_launch as flydsl_tile_sw_launch
    HAS_FLYDSL_TILE_SW = False
    print("FlyDSL PA decode tile SW (f16/bf16, sliding window) available")
except ImportError as e:
    HAS_FLYDSL_TILE_SW = False
...
def run_flydsl_kernel(
    ...
    """Run FlyDSL PA decode kernel (sliding-window variant)."""
    for _ in range(10): 
        flydsl_pa_decode_launch(
...
def run_pa_gluon_test(
    ...
    # Save un-shuffled value cache for FlyDSL before shuffling
    quantized_values_unshuffled = quantized_values
    if trans_v:
    ...
    if not skip_flydsl:
        import sys
        print(f"TRACE: FlyDSL not skipped, about to prepare")
        sys.stdout.flush()
        # FlyDSL expects: output [batch, nkv, qg, head_size]
        # query [batch, nh, head_size]  (same as quantized_query)
        # key_cache [N, nkv, hd//x, bs, x]  (FP8, same as quantized_keys)
        # value_cache [N, nkv, hd, bs]  (FP8, original 4D — FlyDSL handles trans_v internally)
        # context_lengths [batch]
        # block_tables [batch, bps]
        # query_scale, key_scale, value_scale are scalar floats
        flydsl_value_cache = quantized_values_unshuffled

        # FlyDSL requires FP8 query with scalar scale — quantize if needed
        if query_scale_factors is not None and query_scale_factors.numel() == 1:
            fd_query = quantized_query
            fd_q_scale = query_scale_factors.item()
        else:
            # Query not quantized or per-token — do per-tensor quant
            fd_query, fd_q_scale_t = per_tensor_quant(query, quant_dtype=aiter.dtypes.fp8)
            fd_q_scale = fd_q_scale_t.item()
        fd_k_scale = key_scale_original.item() if key_scale_original is not None and key_scale_original.numel() == 1 else 0.0
        fd_v_scale = value_scale_original.item() if value_scale_original is not None and value_scale_original.numel() == 1 else 0.0

        flydsl_output = torch.empty(
            batch_size * query_length, num_query_heads, head_size,
            dtype=data_type, device=device,
        )
        flydsl_time = 1
        import sys
        print(f"TRACE: flydsl_output.shape={flydsl_output.shape} dtype={flydsl_output.dtype}")
        print(f"TRACE: fd_query.shape={fd_query.shape} dtype={fd_query.dtype}")
        print(f"TRACE: quantized_keys.shape={quantized_keys.shape} dtype={quantized_keys.dtype}")
        print(f"TRACE: flydsl_value_cache.shape={flydsl_value_cache.shape} dtype={flydsl_value_cache.dtype} ndim={flydsl_value_cache.ndim}")
        print(f"TRACE: context_lengths.shape={context_lengths.shape} max={context_lengths.max().item()}")
        print(f"TRACE: block_tables.shape={block_tables.shape}")
        print(f"TRACE: softmax_scale={softmax_scale}")
        print(f"TRACE: fd_q_scale={fd_q_scale} fd_k_scale={fd_k_scale} fd_v_scale={fd_v_scale}")
        print(f"TRACE: kv_block_size={block_size} sliding_window={sliding_window} max_context_partition_num={max_context_partition_num}")
        sys.stdout.flush()
        torch.cuda.synchronize()
        print("TRACE: sync before FlyDSL done, launching...")
        sys.stdout.flush()
        _ = run_flydsl_kernel(
            flydsl_output,
            fd_query,
        ...
        flydsl_output_flat = flydsl_output.reshape(batch_size, num_query_heads, head_size)

        # Compare FlyDSL against original reference (torch_mha_extend).
        # FlyDSL uses FP8 probability requantization in PV MFMA, which adds
        # ~0.03-0.04 error vs bf16 references. Use 5e-2 tolerance (same as
        # FlyDSL's own test_pa.py).
        flydsl_ref = reference_output_quant
        flydsl_ref_name = "Original Ref"
        flydsl_diff_tolerance = 5e-3
        torch.testing.assert_close(flydsl_ref, flydsl_output_flat, atol=flydsl_diff_tolerance, rtol=flydsl_diff_tolerance)
        err_flydsl = checkAllclose(
            flydsl_ref,
            flydsl_output_flat,
            atol=flydsl_diff_tolerance,
            rtol=flydsl_diff_tolerance,
            msg=f"[FlyDSL vs {flydsl_ref_name}][{quant_mode}]: {flydsl_time:>8.2f} us......",
        )
        if err_flydsl > 0:
            err_flydsl = 1

        print(f"\nFlyDSL vs {flydsl_ref_name}:")
        flydsl_diff_result = compare_arrays(
            flydsl_output_flat.to(torch.float32).detach().cpu().numpy(),
            flydsl_ref.to(torch.float32).detach().cpu().numpy(),
        )
        if flydsl_diff_result["max_diff_thr"] < flydsl_diff_tolerance:
            print("flydsl_vs_ref PASSED")
        else:
            print("flydsl_vs_ref FAILED")

        # Also compare FlyDSL vs Gluon directly
        print("\nFlyDSL vs Gluon:")
        compare_arrays(
            flydsl_output_flat.to(torch.float32).detach().cpu().numpy(),
            final_output_gluon.to(torch.float32).detach().cpu().numpy(),
        ...
        results["us_flydsl"] = flydsl_time
        results["err_flydsl"] = err_flydsl
        flydsl_bw = pa_rw_bytes / (flydsl_time * 1e6 * 1.024**4)
        results["flydsl_bandwith(TB/s)"] = flydsl_bw
        results["perf_flydsl_vs_gluon"] = f'{gluon_time / flydsl_time:.0%}'
        print(f"\nFlyDSL: {flydsl_time:.2f} us, Gluon: {gluon_time:.2f} us, perf: {gluon_time / flydsl_time:.0%}")
    else:
        if not HAS_FLYDSL:
            results["us_flydsl"] = "N/A (not installed)"
        else:
            results["us_flydsl"] = "N/A (unsupported config)"
        results["err_flydsl"] = "N/A"
        results["flydsl_bandwith(TB/s)"] = "N/A"
        results["perf_flydsl_vs_gluon"] = "N/A"


    # ============================================================
    # FlyDSL Tile PA decode test (f16, CuTe-style tile programming)
    # ============================================================
    query_group_size_tile = num_query_heads // num_kv_heads
    skip_flydsl_tile = (
        not HAS_FLYDSL_TILE
        or query_length != 1
        or head_size != 128
        or query_group_size_tile != 16
        or compute_type not in [torch.float16]
        or quant_kv
        or use_sinks
        or sliding_window > 0
    )

    if not skip_flydsl_tile:
        # FlyDSL tile kernel uses f16 KV cache directly (no quantization)
        # key_cache: [N, nkv, hd//x, bs, x] f16
        # value_cache (unshuffled): [N, nkv, hd, bs] f16
        tile_output = torch.empty(
            batch_size, num_query_heads, head_size,
            dtype=torch.float16, device=device,
        )

        # Query: reshape from [batch*ql, nq, hd] to [batch, nq, hd] (ql=1)
        tile_query = query.reshape(batch_size, num_query_heads, head_size).contiguous()

        # Warm up + benchmark (10 iterations like other FlyDSL tests)
        torch.cuda.synchronize()
        for _ in range(10):
            flydsl_tile_launch(
                tile_output,
                tile_query,
                key_cache,                        # original f16 key cache
                quantized_values_unshuffled,       # original f16 value cache [N, nkv, hd, bs]
                context_lengths,
                block_tables,
                softmax_scale,
                kv_block_size=block_size,
        ...
        tile_output_flat = tile_output.reshape(batch_size, num_query_heads, head_size)

        # Compare against reference
        tile_ref = reference_output_quant
        tile_diff_tolerance = 5e-3
        err_tile = checkAllclose(
            tile_ref,
            tile_output_flat,
            atol=tile_diff_tolerance,
            rtol=tile_diff_tolerance,
            msg=f"[FlyDSL Tile vs Ref]: ......",
        )
        if err_tile > 0:
            err_tile = 1

        print(f"\nFlyDSL Tile vs Reference:")
        tile_diff_result = compare_arrays(
            tile_output_flat.to(torch.float32).detach().cpu().numpy(),
            tile_ref.to(torch.float32).detach().cpu().numpy(),
        )
        if tile_diff_result["max_diff_thr"] < tile_diff_tolerance:
            print("flydsl_tile_vs_ref PASSED")
        else:
            print("flydsl_tile_vs_ref FAILED")

        results["err_flydsl_tile"] = err_tile
    else:
        if not HAS_FLYDSL_TILE:
            results["err_flydsl_tile"] = "N/A (not installed)"
        else:
            results["err_flydsl_tile"] = "N/A (unsupported config)"


    # FlyDSL Tile PA decode test (bf16, CuTe-style tile programming)
    # ==============================================================
    query_group_size_tile_bf16 = num_query_heads // num_kv_heads
    skip_flydsl_tile_bf16 = (
        not HAS_FLYDSL_TILE_BF16
        or query_length != 1
        or head_size != 128
        or query_group_size_tile_bf16 != 16
        or compute_type not in [torch.bfloat16]
        or quant_kv
        or use_sinks
        or sliding_window > 0
    )

    if not skip_flydsl_tile_bf16:
        # FlyDSL bf16 tile kernel uses bf16 KV cache directly
        tile_bf16_output = torch.empty(
            batch_size, num_query_heads, head_size,
            dtype=torch.bfloat16, device=device,
        )
        tile_bf16_query = query.reshape(batch_size, num_query_heads, head_size).contiguous()

        torch.cuda.synchronize()
        for _ in range(10):
            flydsl_tile_bf16_launch(
                tile_bf16_output,
                tile_bf16_query,
                key_cache,
        ...
        tile_bf16_output_flat = tile_bf16_output.reshape(batch_size, num_query_heads, head_size)
        tile_bf16_ref = reference_output_quant
        tile_bf16_diff_tolerance = 5e-3
        err_tile_bf16 = checkAllclose(
            tile_bf16_ref,
            tile_bf16_output_flat,
            atol=tile_bf16_diff_tolerance,
            rtol=tile_bf16_diff_tolerance,
            msg=f"[FlyDSL Tile BF16 vs Ref]: ......",
        )
        if err_tile_bf16 > 0:
            err_tile_bf16 = 1

        print(f"\nFlyDSL Tile BF16 vs Reference:")
        tile_bf16_diff_result = compare_arrays(
            tile_bf16_output_flat.to(torch.float32).detach().cpu().numpy(),
            tile_bf16_ref.to(torch.float32).detach().cpu().numpy(),
        )
        if tile_bf16_diff_result["max_diff_thr"] < tile_bf16_diff_tolerance:
            print("flydsl_tile_bf16_vs_ref PASSED")
        else:
            print("flydsl_tile_bf16_vs_ref FAILED")

        results["err_flydsl_tile_bf16"] = err_tile_bf16
    else:
        if not HAS_FLYDSL_TILE_BF16:
            results["err_flydsl_tile_bf16"] = "N/A (not installed)"
        else:
            results["err_flydsl_tile_bf16"] = "N/A (unsupported config)"


    # ============================================================
    # FlyDSL Tile SW PA decode test (unified f16/bf16, sliding window)
    # ============================================================
    query_group_size_tile_sw = num_query_heads // num_kv_heads
    skip_flydsl_tile_sw = (
        not HAS_FLYDSL_TILE_SW
        or query_length != 1
        or head_size != 128
        or query_group_size_tile_sw != 16
        or compute_type not in [torch.float16, torch.bfloat16]
        or quant_kv
    )

    if not skip_flydsl_tile_sw:
        tile_sw_dtype = compute_type
        tile_sw_output = torch.empty(
            batch_size, num_query_heads, head_size,
            dtype=tile_sw_dtype, device=device,
        )
        tile_sw_query = query.reshape(batch_size, num_query_heads, head_size).contiguous()

        # Sinks tensor (None if not used)
        tile_sw_sinks = None
        if use_sinks:
            # Sinks: pre-computed per-query-head attention scores (f32)
            # For testing, create dummy sinks (zeros -- no contribution)
            tile_sw_sinks = torch.zeros(num_query_heads, dtype=torch.float32, device=device)

        torch.cuda.synchronize()
        for _ in range(10):
            flydsl_tile_sw_launch(
                tile_sw_output,
                tile_sw_query,
                key_cache,
                quantized_values_unshuffled,
        ...
        tile_sw_output_flat = tile_sw_output.reshape(batch_size, num_query_heads, head_size)
        tile_sw_ref = reference_output_quant
        tile_sw_diff_tolerance = 5e-2
        err_tile_sw = checkAllclose(
            tile_sw_ref,
            tile_sw_output_flat,
            atol=tile_sw_diff_tolerance,
            rtol=tile_sw_diff_tolerance,
            msg=f"[FlyDSL Tile SW vs Ref]: ......",
        )
        if err_tile_sw > 0:
            err_tile_sw = 1

        print(f"\nFlyDSL Tile SW ({tile_sw_dtype}) vs Reference:")
        tile_sw_diff_result = compare_arrays(
            tile_sw_output_flat.to(torch.float32).detach().cpu().numpy(),
            tile_sw_ref.to(torch.float32).detach().cpu().numpy(),
        )
        if tile_sw_diff_result["max_diff_thr"] < tile_sw_diff_tolerance:
            print("flydsl_tile_sw_vs_ref PASSED")
        else:
            print("flydsl_tile_sw_vs_ref FAILED")

        results["err_flydsl_tile_sw"] = err_tile_sw
    else:
        if not HAS_FLYDSL_TILE_SW:
            results["err_flydsl_tile_sw"] = "N/A (not installed)"
        else:
            results["err_flydsl_tile_sw"] = "N/A (unsupported config)"


    # ============================================================
    # FlyDSL PS (Persistent Scheduling) test
    # ============================================================
    ...
    if not skip_flydsl_ps:
        import math
        # Build kv_page_indices and kv_indptr from block_tables + context_lengths
        actual_blocks_ps = (context_lengths + block_size - 1) // block_size
        kv_indptr_ps = torch.zeros(batch_size + 1, dtype=torch.int32, device=device)
        kv_indptr_ps[1:batch_size + 1] = torch.cumsum(actual_blocks_ps, dim=0)
        kv_indices_ps_lst = []
        for i in range(batch_size):
            kv_indices_ps_lst += block_tables_list[i][:actual_blocks_ps[i]]
        kv_page_indices_ps = torch.tensor(kv_indices_ps_lst, dtype=torch.int32, device=device)

        # Compute PA metadata via get_pa_metadata
        _ps_metadata = flydsl_get_pa_metadata(
            quantized_query, quantized_keys, context_lengths, kv_indptr_ps,
            num_query_heads, num_kv_heads)
        _total_work_ps = _ps_metadata["work_indptr"][-1].item()
        _winfo_flat = _ps_metadata["work_info_flat"]
        _has_splits_ps = (_winfo_flat.reshape(-1, 8)[:_total_work_ps, 1] >= 0).any().item()
        if _has_splits_ps:
            print(f"\nFlyDSL PS: split-reduce mode ({_total_work_ps} work items)")
        else:
            print(f"\nFlyDSL PS: direct mode ({_total_work_ps} work items, no splits)")

        if True:
            # PS kernel uses un-shuffled value cache (4D)
            flydsl_ps_value_cache = quantized_values_unshuffled

            # Query: same as FlyDSL baseline
            if query_scale_factors is not None and query_scale_factors.numel() == 1:
                fdps_query = quantized_query
                fdps_q_scale = query_scale_factors.item()
            else:
            ...
            print(f"\nTRACE: FlyDSL PS launch: query={fdps_query.shape} kv_page_indices={kv_page_indices_ps.shape} kv_indptr={kv_indptr_ps.shape}")
            torch.cuda.synchronize()

            _ = run_flydsl_ps_kernel(
                flydsl_ps_output,
                fdps_query,
                quantized_keys,
                flydsl_ps_value_cache,
                context_lengths,
                kv_page_indices_ps,
                kv_indptr_ps,
                softmax_scale,
            ...
            err_flydsl_ps = checkAllclose(
                ...
                atol=flydsl_ps_diff_tolerance,
                rtol=flydsl_ps_diff_tolerance,
                msg=f"[FlyDSL_PS vs Ref][{quant_mode}]: ......",
            )
            if err_flydsl_ps > 0:
                err_flydsl_ps = 1

            print(f"\nFlyDSL PS vs Ref:")
            flydsl_ps_diff_result = compare_arrays(
                flydsl_ps_output_flat.to(torch.float32).detach().cpu().numpy(),
                flydsl_ps_ref.to(torch.float32).detach().cpu().numpy(),
            )
            if flydsl_ps_diff_result["max_diff_thr"] < flydsl_ps_diff_tolerance:
                print("flydsl_ps_vs_ref PASSED")
            else:
                print("flydsl_ps_vs_ref FAILED")

            # Compare FlyDSL PS vs Gluon
            print("\nFlyDSL PS vs Gluon:")
            compare_arrays(
                flydsl_ps_output_flat.to(torch.float32).detach().cpu().numpy(),
                final_output_gluon.to(torch.float32).detach().cpu().numpy(),
            ...
            # Compare FlyDSL PS vs FlyDSL baseline (should be bit-identical)
            if not skip_flydsl:
                print("\nFlyDSL PS vs FlyDSL baseline:")
                flydsl_ps_vs_base = compare_arrays(
                    flydsl_ps_output_flat.to(torch.float32).detach().cpu().numpy(),
                    flydsl_output_flat.to(torch.float32).detach().cpu().numpy(),
                )
                if flydsl_ps_vs_base["max_diff_thr"] < 1e-6:
                    print("flydsl_ps_vs_baseline BIT-IDENTICAL")
                else:
                    print(f"flydsl_ps_vs_baseline max_diff_thr={flydsl_ps_vs_base['max_diff_thr']:.6e}")
...
def parse_arg_and_run_test(sample_rate0: float = None):
    ...
    columns_to_print_mean = [
        ...
        "gluon_bandwith(TB/s)",
        "us_flydsl",
        "flydsl_bandwith(TB/s)",
        "perf_flydsl_vs_gluon",
        "err_flydsl_ps",
        "us_asm",
        "asm_bandwith(TB/s)",
        "perf_gluon_vs_asm",
    ...
    if total_errors > 0:
        print(
            f"\nTests failed! {total_errors} Gluon test case(s) exceeded the error threshold. "
        )
        print(f"Please check rows with non-zero err_gluon in {output_file}.")
        assert False, f"{total_errors} Gluon test case(s) exceeded the error threshold"

    # Check FlyDSL errors if present
    if "err_flydsl" in results_df.columns:
        flydsl_errors = results_df["err_flydsl"].apply(
            lambda x: x if isinstance(x, (int, float)) and x > 0 else 0
        ).sum()
        if flydsl_errors > 0:
            print(
                f"\nFlyDSL tests: {int(flydsl_errors)} test case(s) exceeded the error threshold."
            )
        else:
            flydsl_ran = results_df["err_flydsl"].apply(
                lambda x: isinstance(x, (int, float))
            ).sum()
            print(f"\nFlyDSL: {flydsl_ran} test(s) ran, all passed!")
