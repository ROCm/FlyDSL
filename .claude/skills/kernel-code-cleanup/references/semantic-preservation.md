...
- Record the base and final commits, FlyDSL source and native binding versions,
  imported module paths, GPU target and relevant dispatch flags. Keep the
  baseline immutable in a separate checkout. Select the intended kernel tree and
  Python source explicitly. When porting between aiter and FlyDSL, validate each
  against its own supported version.
...
Use `kernels/common/act.py` for shared activation components and `LOG2E`. A
sign-restored tanh and `2 * sigmoid(2 * x) - 1` can round differently; retain distinct variants
...
Prefer `fx.ceildiv` for its matching typed division operation. A host/helper
ceildiv expression `(n + d - 1) // d` preserves the caller's fixed-width additions
when used with DSL integers. Those formulas can
...
- Typed pointer access is suitable only if it preserves address space, cache
  ...
  local LLVM boundary when the checkout API cannot express those properties.
