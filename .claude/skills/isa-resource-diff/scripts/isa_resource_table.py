...
``FLYDSL_DUMP_IR``:
    ...
    python3 .claude/skills/isa-resource-diff/scripts/isa_resource_table.py diff /tmp/before /tmp/after
    python3 .claude/skills/isa-resource-diff/scripts/isa_resource_table.py summarize <dump-dir> --json snapshot.json
    python3 .claude/skills/isa-resource-diff/scripts/isa_resource_table.py diff before.json after.json
...
if sys.version_info < (3, 10):  # pragma: no cover - exercised via subprocess
    sys.stderr.write(
        ".claude/skills/isa-resource-diff/scripts/isa_resource_table.py requires Python 3.10+ (running %s).\n"
        "This repository targets 3.10+ (CONTRIBUTING.md; ruff target-version=py310).\n"
        "Try: python3.10 .claude/skills/isa-resource-diff/scripts/isa_resource_table.py ...\n" % sys.version.split()[0]
    )
    raise SystemExit(2)
...
class Cell:
    ...
    def from_json(raw: object, where: str) -> "Cell":
        ...
        if state == VALUE:
            ...
            if value < 0:
                # A snapshot is just the other way to build a `Cell`, and the invariant
                # belongs to the cell rather than to the assembly parser: every metric
                # here is a count or a byte size.  Enforcing it on one path only lets a
                # saved snapshot carry a value the tool would never have written, and a
                # diff down to it reads as an improvement.
                raise SnapshotError(f"{where}.value is negative: {value}")
...
class Arch:
    ...
    def from_json(raw: object) -> "Arch":
        ...
        # Sorted here as well as in `parse_target_id`: the invariant is the field's, and
        # applying it on only one of the two paths that build an `Arch` makes two
        # snapshots of one target differ on the order alone.
        return Arch(str(raw.get("target_id") or ""), processor, gen, tuple(sorted(features)))
...
_CATEGORIES = (
    ...
    # `smfmac` is CDNA3/4 sparse MFMA; without it a kernel on the sparse cores reads 0.
    ("matrix_ops", re.compile(r"^v_(?:mfma|wmma|smfmac)")),
)
...
class _Scan:
    ...
    symbol_problems: list = field(default_factory=list)
    target_id: str | None = None
    md_target_id: str | None = None
    saw_metadata: bool = False


def _record_set(line: str, symbols: dict, problems: list) -> None:
    m = RE_SET.match(line)
    if m:
    ...
    if base.startswith(".L"):
        ...
        base = base[2:]
    known = symbols.setdefault(base, {})
    if suffix in known:
        # Stripping `.L` is what puts the two spellings in one key space, so last-wins
        # here would silently discard a value this normalization created a rival for.
        # `private_seg_size` has teeth: it feeds `_cross_check_scratch`, and a collided
        # value invents a scratch-bytes disagreement.  Keep the first, say so.
        if known[suffix] != value:
            problems.append(f".set {base}.{suffix} declared twice with different values ({known[suffix]} vs {value})")
        return
    known[suffix] = value
...
def _scan_text(text: str) -> _Scan:
    ...
    for index, line in enumerate(text.splitlines()):
        ...
        if stripped.startswith(".set"):
            _record_set(line, scan.symbols, scan.symbol_problems)
            continue
...
def parse_isa(path: str | Path) -> tuple:
    ...
    file_problems.extend(scan.symbol_problems)
...
def load_snapshot(path: str | Path) -> Snapshot:
    ...
    # `collect()` folds each record's problems into the snapshot's; a snapshot that did
    # not would make `diff a.json b.json` strictly less diagnosable than the `diff dirA
    # dirB` it was written from, for the same data and the same verdict.
    problems = [f"{key}: {problem}" for key, record in kernels.items() for problem in record.problems]
    if not problems and raw.get("trustworthy") is False:
