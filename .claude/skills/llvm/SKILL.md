...
This project has been burned by false negatives repeatedly; §8 is not optional.
...
A flag appearing in a shipped kernel is **not** evidence that it exists — see §9.
...
the `rocdl.waves_per_eu` MLIR attribute. `maxnreg` had no such second path — see §9.
...
| ~~`maxnreg`~~ | **removed** — now raises; see §9 | raises | n/a |
| `fast_fp_math` | `rocdl-attach-target{fast=true}` **+** ambient tracing fastmath | truthiness | `fastmath<...>` in IR |
| `unsafe_fp_math` | `rocdl-attach-target{unsafe-math=true}` only | truthiness | pass options in IR |
| `fastmath` | explicit fastmath flags on traced ops | normalised | `fastmath<...>` in IR |
...
| `fast/unsafe_fp_math`, `fastmath` | `*_llvm_ir.ll` | `fastmath<...>` on arith ops |
...
- MLIR emits `f0x`-prefixed 32-bit hex float literals that **no** `llc` will parse.
  Rewrite them before feeding.
...
2. **Flag does not exist** — differs between embedded and external mode, and
   between LLVM trees. Check: `llvm_knob_check.py`.
...
4. **Type confusion** — `llvm_options` dispatches on the Python type;
   `boolOrDefault` options mis-route. Prefer external mode, or test first.
