...
| Mode | Mechanism | Failure behaviour |
|---|---|---|
| embedded (default) | in-process `cl::opt` mutation via `addOccurrence()` | raises on unknown option / type mismatch |
| external (`FLYDSL_COMPILE_LLVM_DIR` set) | `--name=value` argv on a subprocess `mlir-opt` | no name or type checking at all |

⚠ So a dict that raises a clean "Unknown LLVM option" in one mode can produce an
opaque subprocess failure in the other.
...
`4` is invalid. Prefer external mode for anything that is not a plain bool.
