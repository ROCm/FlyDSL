...
def get_mlir_type_size(mlir_type: ir.Type) -> int:
    ...
    if (
        mlir_type == T.i8()
        or mlir_type == ir.IntegerType.get_unsigned(8)
        or mlir_type == ir.IntegerType.get_signed(8)
    ):
        return 1
    # FP8 types
    if isinstance(
...
class SmemPtr:
    ...
    def get(self) -> ir.Value:
        ...
        target_type = T.memref(
            *target_shape, self.element_type, memory_space=lds_space()
        )

        # memref.view(source, byte_shift, sizes)
        # sizes are needed for dynamic dimensions. Since we use static shapes here, sizes=[]
        self._view_cache = memref.view(
            target_type, self.base_memref, offset_val, sizes=[]
        )
        return self._view_cache
...
class SmemStructInstance:
    ...
    def __init__(
        self, base_memref, start_offset, field_layout: Dict[str, Tuple[int, ir.Type]]
    ):
...
class SmemAllocator:
    def __init__(
        self, ctx, arch: Optional[str] = None, global_sym_name: Optional[str] = None
    ):
        self.ctx = ctx
        self.ptr = 0
        self.max_size = 0
    ...
    def allocate(
        self, size_or_type_or_struct: Union[int, ir.Type, Any], alignment=None
    ):
        """
        ...
        # Mode 3: Struct (Python Class decorated with @dataclass or similar)
        elif is_dataclass(size_or_type_or_struct) or hasattr(
            size_or_type_or_struct, "__annotations__"
        ):
            ...
            for name, type_hint in hints.items():
                ...
                if not isinstance(field_dtype, ir.Type):
                    raise ValueError(
                        f"Field '{name}' in struct '{cls.__name__}' must be typed with an MLIR Type"
                    )
            ...
            raise ValueError(
                f"Unsupported argument to allocate: {size_or_type_or_struct}"
            )
...
SMEM_CAPACITY_MAP = {
    ...
    "gfx950": 163840,  # MI350: 160KB LDS per CU
    # ===================== AMD RDNA Architectures (Consumer/Workstation GPUs) =====================
    # RDNA 4 (Radeon RX 9000 Series) - 64KB LDS per workgroup (from HSA GROUP segment)
    "gfx1201": 65536,  # Radeon RX 9070/9700: 64KB LDS per workgroup
}
