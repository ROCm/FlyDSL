...
This module provides high-level Python wrappers for AMD GPU buffer operations (CDNA and RDNA).
    ...
    if byte_offset is None:
        ...
        if static_byte_offset != 0:
            if not isinstance(offset_val.type, ir.IntegerType):
                raise TypeError(
                    f"dynamic byte_offset must be integer-typed, got {offset_val.type}"
                )
...
class BufferResourceDescriptor:
    ...
    def from_memref(
        ...
        # DWORD3 flags differ between GFX9 (CDNA) and GFX10+ (RDNA):
        #   GFX9:  data_format=7 (float) at bits[14:12], num_format=4 (32-bit) at bits[17:15]
        #   GFX12: FORMAT at bits[21:14], OOB_SELECT at bits[29:28]
        #          OOB_SELECT must be >= 1 (0 = structured mode, requires stride != 0)
        from flydsl.runtime.device import get_rocm_arch as _get_arch

        _arch = _get_arch()
        if (
            _arch.startswith("gfx12")
            or _arch.startswith("gfx11")
            or _arch.startswith("gfx10")
        ):
            # RDNA: OOB_SELECT=2 (disable OOB check) | FORMAT bit 13 (raw 32-bit)
            flags_val = (2 << 28) | (1 << 13)
        else:
            # CDNA (GFX9xx): data_format=7 (float), num_format=4 (32-bit)
            flags_val = (7 << 12) | (4 << 15)
...
def buffer_load_f32x4(
    rsrc: ir.Value, offset: ir.Value, mask: Optional[ir.Value] = None
) -> ir.Value:
    """Load vector<4xf32> using buffer operation."""
    return buffer_load(rsrc, offset, vec_width=4, dtype=ir.F32Type.get(), mask=mask)


def buffer_load_f16x4(
    rsrc: ir.Value, offset: ir.Value, mask: Optional[ir.Value] = None
) -> ir.Value:
    """Load vector<4xf16> using buffer operation (stored as 2xi32)."""
    # For f16, we load 4 elements but they're packed into 2xi32
    i32_data = buffer_load(
        rsrc, offset, vec_width=2, dtype=ir.IntegerType.get_signless(32), mask=mask
    )
    # TODO: Add bitcast to 4xf16 if needed
    return i32_data


def buffer_store_f32x4(
    data: ir.Value, rsrc: ir.Value, offset: ir.Value, mask: Optional[ir.Value] = None
):
...
def buffer_load_2d(
    rsrc, row, col, stride, vec_width=4, dtype=None, mask=None
) -> ir.Value:
