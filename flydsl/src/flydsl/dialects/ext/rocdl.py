...
# fp16 MFMA (K32)
def mfma_f32_16x16x32_f16_op(result_type, operands, *, loc=None, ip=None):
    """Return the op view (original behavior)."""
    if _ods_mfma_f32_16x16x32_f16 is None:
        raise AttributeError("ROCDL op not found: mfma_f32_16x16x32_f16")
    ops = [_unwrap_mfma_operand(v, loc=loc) for v in operands]
    return _ods_mfma_f32_16x16x32_f16(result_type, ops, loc=loc, ip=ip)


def mfma_f32_16x16x32_f16(result_type, operands, *, loc=None, ip=None):
    """Return the op result directly (no `.result` needed at call sites)."""
    return mfma_f32_16x16x32_f16_op(result_type, operands, loc=loc, ip=ip).result
...
__all__ = [
    ...
    'mfma_f32_16x16x16f16_op', 'mfma_f32_16x16x32_f16_op', 'mfma_f32_16x16x32_fp8_fp8_op',
