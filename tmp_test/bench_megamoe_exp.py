...
def _locate_cudagraph_hang(args, rank, world, dev, s1_body, e2e_body):
    """One-shot CUDAGraph back-to-back deadlock LOCATOR (--locate).

    The hang only shows up under CUDAGraph replay (back-to-back launches with NO
    inter-replay ``shmem_barrier_all``), never on a single launch.  ``_mega_body``
    bundles BOTH stage-1 (megakernel: fused dispatch+GEMM1+merged GEMM2+combine-Stage1)
    AND stage-2 combine (``comb_op.combine_no_stage1`` == ``make_combine_kernel``), so a
    plain e2e hang can't tell which kernel deadlocks.  This probes them in ESCALATING
    order and the FIRST probe to wedge is the culprit:

      * HANG at 'stage1-only'            => deadlock is inside the stage-1 megakernel's
                                            cross-PE barriers / claim scheduler.
      * 'stage1-only' OK, HANG at 'e2e'  => deadlock is in the combine kernel
                                            (dispatch_combine_intranode_kernel.make_combine_kernel).

    Each probe captures ONE CUDAGraph, fires a host watchdog thread, then does
    ``--locate-reps`` back-to-back replays + a single ``synchronize()``.  If that does
    not finish within ``--locate-timeout`` s the watchdog prints this rank's stuck probe
    and hard-exits (the wedged persistent kernel is unrecoverable, so a clean teardown
    would itself hang).  ``synchronize()`` releases the GIL, so the watchdog runs."""
    to = float(getattr(args, "locate_timeout", 30.0))
    reps = max(2, int(getattr(args, "locate_reps", 20)))

    def _probe(name, body):
        ms.shmem_barrier_all()
        # single launch first (matches the earlier warmup that already PASSED)
        ...
        with torch.cuda.graph(g, stream=_cap):
            body()
        _info(rank, f"[LOCATE] probe '{name}': captured; {reps} back-to-back replays "
                    f"(NO inter-replay barrier), watchdog={to:.0f}s ...")
        done_ev = threading.Event()

        def _wd():
            if not done_ev.wait(to):
                print(f"[LOCATE][rank{rank}] *** HANG in probe '{name}' *** "
                      f"(no completion after {to:.0f}s of {reps} back-to-back CUDAGraph replays)",
                      flush=True)
                sys.stdout.flush(); sys.stderr.flush()
                os._exit(7)

        _t = threading.Thread(target=_wd, daemon=True); _t.start()
        for _ in range(reps):
            g.replay()
        torch.cuda.synchronize()   # blocks here on a deadlock -> watchdog fires
        done_ev.set()
        ms.shmem_barrier_all()
        _info(rank, f"[LOCATE] probe '{name}': COMPLETED {reps} back-to-back replays (no deadlock)")

    if rank == 0:
        print(f"[LOCATE] escalating CUDAGraph back-to-back probes "
              f"(reps={reps}, timeout={to:.0f}s): stage1-only -> e2e(stage1+combine)", flush=True)
    _probe("stage1-only", s1_body)
    _probe("e2e (stage1+combine)", e2e_body)
    if rank == 0:
        print("[LOCATE] ALL probes COMPLETED -- no CUDAGraph back-to-back deadlock reproduced "
              "(check bs / try more --locate-reps).", flush=True)
...
def _run(args, rank, world, dev, *, model_dim, inter_dim, experts, epr, topk, run_tokens, mtpr,
    ...
    def _cg_time(body):
        ...
        if _eager:
            # Per-iteration shmem_barrier_all: MegaMoEExp's fused Plan-A has the megakernel P2P-write
            # into comb_op's buffers + self-clear total_recv each launch, so back-to-back launches
            # WITHOUT a cross-rank barrier let one rank's next launch overwrite another rank's not-yet
            # -consumed combine buffers -> cross-PE deadlock.  The barrier is included in the timed
            # window for BOTH mega and baseline, so the speedup ratio stays fair.
            for _ in range(10):
                ms.shmem_barrier_all(); body()
    ...
    if getattr(args, "locate", False):
        _info(rank, "[stage] oracle+relL2 done; entering --locate CUDAGraph deadlock probes ...")
        _locate_cudagraph_hang(args, rank, world, dev, _mega_s1_body, _mega_body)
        return None

    _info(rank, f"[stage] oracle+relL2 done (megaexp={_rm_w:.3e} atom={_ra8_w:.3e} vs-base={_rma:.3e}); "
                f"timing ({'eager+barrier' if _eager else 'CUDAGraph'}) ...")
...
def main():
    ...
    p.add_argument("--locate", action="store_true",
                   help="one-shot CUDAGraph deadlock LOCATOR: escalating back-to-back replay probes "
                        "(stage1-only -> e2e) with a host watchdog; the first probe to HANG names the "
                        "deadlocking kernel (stage-1 megakernel vs combine). Skips normal timing.")
    p.add_argument("--locate-timeout", type=float, default=30.0,
                   help="--locate: seconds a probe's back-to-back replays may run before it is "
                        "declared a hang (default 30).")
    p.add_argument("--locate-reps", type=int, default=20,
                   help="--locate: number of back-to-back CUDAGraph replays per probe (default 20).")
    args = p.parse_args()
